...
dOrderTotal OlineBrowse ItemBrowse cStatus 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS OlineBrowse dsOrderWin _STRUCTURED
  QUERY OlineBrowse NO-LOCK DISPLAY
      ...
      ttOLine.Qty FORMAT "->>>>9":U WIDTH 8.8
      ttOLine.ExtendedPrice FORMAT "->>>,>>9.99":U WIDTH 18.4
...
DEFINE FRAME dsFrame
     ...
     BtnSave AT ROW 11.95 COL 4
     cStatus AT ROW 11.95 COL 29 NO-LABEL
...
"Price" ? ? "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[5]   > Temp-Tables.ttOLine.Qty
"Qty" ? ? "integer" ? ? ? ? ? ? yes ? no no "8.8" yes no no "U" "" ""
     _FldNameList[6]   > Temp-Tables.ttOLine.ExtendedPrice
"ExtendedPrice" ? ? "decimal" ? ? ? ? ? ? no ? no no "18.4" yes no no "U" "" ""
     _FldNameList[7]   > Temp-Tables.ttOLine.Discount
"Discount" ? ? "integer" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
...
DO:
  ...
  /* All this code is replaced by MERGE-CHANGES: 
  CREATE QUERY hQuery.
...
------------------------------------------------------------------------------*/
  DISPLAY iOrderNum iCustNum cCustName cRepName dOrderTotal cStatus 
