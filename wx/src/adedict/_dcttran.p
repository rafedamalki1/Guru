...
repeat transaction ON ERROR UNDO, RETRY:
   ...
   */
   ...
   do:
	 ...
	 else if answer = ? then  /* User chose "Cancel to continue" */
	 do:
	    /* Don't do the action that caused this question to be asked.  
	       e.g., if user tried to switch databases, we shouldn't. */
	    s_ActionProc = "".

      	    /* CurrDb is still the old value - set the list box selection
      	       back to that. */
