    ...
    THEN DO: /* Text is selected  */      
        ...
        ELSE DO:
             ...
             /* If we are going to comment the section, then surround the 
                text with '/**/'. Also, add ' * ' to the start of each line.
                Reverse the process when we uncomment */
             IF pl_comment EQ yes
             THEN
               ASSIGN New_Text = "/*" + REPLACE(New_Text,CHR(10), CHR(10) + " * ") + "*/" NO-ERROR.
             ELSE DO:
               ASSIGN SUBSTRING( New_Text , Comment_End , LENGTH( "*/":u, "CHARACTER":u ) ) = "" NO-ERROR.
               ASSIGN SUBSTRING( New_Text , Comment_Start, LENGTH( "/*":u, "CHARACTER":u ) ) = "" NO-ERROR.
               ASSIGN New_Text = REPLACE(New_Text,CHR(10) + " * ", CHR(10)).          
             END.
