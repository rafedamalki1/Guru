...
Define var hide_it as logical NO-UNDO.
   ...
   case (p_Obj):
      ...
      do:
      	 assign
           s_lst_Doms:hidden in frame browse = hide_it
      	    s_DomFill:hidden  in frame browse = hide_it.
      	 
      	    if p_Show then 
      	       s_Lvl1Lbl:screen-value in frame browse = " Domains".   
...
Define var hide_it as logical NO-UNDO.
   ...
   if p_Level = 1 then
      assign
      	 ...
      	 */
      	 /*---
      	 s_icn_Doms:hidden   in frame browse = hide_it 
      	 /* WIN95
      	    s_DomLbl:hidden     in frame browse = hide_it
      	 */
      	 ---*/
...
case (p_Obj_Num):
   ...
   otherwise /* {&OBJ_TBL}, {&OBJ_DOM}, {&OBJ_SEQ} */
   do:
	 ...
	 fill_widget =
	    ...
	    else ? /* s_DomFill:HANDLE in frame browse */ )
      	 msg = 
	    (if p_Obj_Num = {&OBJ_TBL} then
	       "(no tables)"
	    else if p_Obj_Num = {&OBJ_SEQ} then
