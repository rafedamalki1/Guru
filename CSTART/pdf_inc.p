...
/*------------------------------------------------------------------------
    ...
    Author(s)   : 
    Created     : Mon Nov 05 17:27:43 CET 2018
    Notes       :
  ----------------------------------------------------------------------*/
...
/******************************************************************************
  ...
  02/19/10  JC Cardot   Add functions for binary manipulations
                        ...
                        new feature: pdf_load_links modified si that the contents
  ...
  07/07/10  JC Cardot   rewrite pdf_text_rotate according to pdf specification
                        in order not to skew the text (note that it does render
                        the way it did before, the code using this functionnality
                        ...
                        Usefull when running in an utf-8 session.
                        
  11/04/10  JC Cardot   fix pdf_set_base14_codepage: if then code page is 1252
  ...
  14/01/11  JC Cardot   fix: filling forms did not work in european sessions
...
/* &GLOBAL-DEFINE debugComment + "  % " + ENTRY(1,PROGRAM-NAME(1)," ") + " | " + ENTRY(1,PROGRAM-NAME(2)," ") + (IF PROGRAM-NAME(3) <> ? THEN " | " + ENTRY(1,PROGRAM-NAME(3)," ") ELSE "") */
/* &GLOBAL-DEFINE debugComment + "  % " + ENTRY(1,PROGRAM-NAME(1)," ") + " | " + ENTRY(1,PROGRAM-NAME(2)," ") */
...
DEFINE TEMP-TABLE TT_pdf_font NO-UNDO
    ...
    FIELD used_flag         AS LOGICAL /* 22-FEB-2010 jcc: added */
INDEX font_name AS PRIMARY
      font_name
INDEX obj_stream
      obj_stream
INDEX obj_stream2 /* 23-FEB-2010 jcc: added */
      obj_stream 
      font_file   /* 23-FEB-2010 jcc: Mike Smith 10/7/2007 */
      used_flag
      font_obj.   /* 23-FEB-2010 jcc: Mike Smith 10/7/2007 */
...
/* 13-JUL-2010 jcc */
DEFINE TEMP-TABLE TT_transform_matrix NO-UNDO LABEL "Transformation matrix"
  FIELD obj_stream  AS CHARACTER
  FIELD matrix      AS CHARACTER  
INDEX matrix IS PRIMARY UNIQUE
      obj_stream.
...
/* This is used when outputting the differences between iso8859-1 and another charset */
...
FUNCTION bin_xor RETURNS INTEGER
  ...
  RETURN iXORedResult.
END FUNCTION. /* bin_xor */

/* 19-FEB-2010 jcc: start */
...
FUNCTION bin_lshift RETURNS INTEGER
    (n AS INTEGER, s AS INTEGER ):
    n = n * EXP(2, s).
    RETURN n.
END FUNCTION.

FUNCTION bin_rshift RETURNS INTEGER
    (n AS INTEGER, s AS INTEGER ):
    n = TRUNC(n / EXP(2, s), 0).
    RETURN n.
END FUNCTION.
...
&IF "{&opsys}" BEGINS "win" &THEN
    &GLOBAL-DEFINE mathLib 'msvcrt40.dll'
&ELSE
    &GLOBAL-DEFINE mathLib '/usr/lib/libm.so'
&ENDIF
...
&GLOBAL xdPi 3.14159265358979323846
...
FUNCTION multiplyTransormationMatrices RETURNS CHARACTER
    ...
    ASSIGN
        a = DECIMAL(ENTRY(1, M1, " "))
        b = DECIMAL(ENTRY(2, M1, " "))
        c = DECIMAL(ENTRY(3, M1, " "))
        d = DECIMAL(ENTRY(4, M1, " "))
        e = DECIMAL(ENTRY(5, M1, " "))
        f = DECIMAL(ENTRY(6, M1, " "))
        x = DECIMAL(ENTRY(1, M2, " "))
        y = DECIMAL(ENTRY(2, M2, " "))
        z = DECIMAL(ENTRY(3, M2, " "))
        t = DECIMAL(ENTRY(4, M2, " "))
        u = DECIMAL(ENTRY(5, M2, " "))
        v = DECIMAL(ENTRY(6, M2, " "))
        .
    RETURN
        dec2string(a * x + b * z) + " " +
        dec2string(a * y + b * t) + " " +
...
PROCEDURE pdf_new :
  ...
  /* 23-JUN-2009 jcc: make use of OS-DELETE instead because 
                      ...
                      2) this outputs useless messages (e.g. in the appserver broker log when there is nothing to delete) */
  /*IF OPSYS = "UNIX" THEN
...
FUNCTION pdf_ScaleX RETURN DECIMAL (INPUT pdfStream AS CHARACTER):
  ...
  IF NOT CAN-FIND(FIRST TT_pdf_stream
                  WHERE TT_pdf_stream.obj_stream = pdfStream NO-LOCK)
  THEN DO:
    RUN pdf_error(pdfStream,"pdf_ScaleX","Cannot find Stream!").
    RETURN ERROR.
  END.

  FIND FIRST TT_pdf_param WHERE TT_pdf_param.obj_stream    = pdfStream
                            AND TT_pdf_param.obj_parameter = "ScaleX"
                            NO-LOCK NO-ERROR.
  IF AVAIL TT_pdf_param THEN RETURN DECIMAL(TT_pdf_param.obj_value).
  ELSE RETURN ?.
...
FUNCTION pdf_ScaleY RETURN DECIMAL (INPUT pdfStream AS CHARACTER):
  ...
  IF NOT CAN-FIND(FIRST TT_pdf_stream
                  WHERE TT_pdf_stream.obj_stream = pdfStream NO-LOCK)
  THEN DO:
    RUN pdf_error(pdfStream,"pdf_ScaleY","Cannot find Stream!").
    RETURN ERROR.
  END.

  FIND FIRST TT_pdf_param WHERE TT_pdf_param.obj_stream    = pdfStream
                            AND TT_pdf_param.obj_parameter = "ScaleY"
                            NO-LOCK NO-ERROR.
  IF AVAIL TT_pdf_param THEN RETURN DECIMAL(TT_pdf_param.obj_value).
  ELSE RETURN ?.
...
*/
  ...
  IF NOT CAN-FIND(FIRST TT_pdf_stream
    WHERE TT_pdf_stream.obj_stream = pdfStream NO-LOCK) THEN DO:
...
PROCEDURE pdf_watermark:
  ...
  IF NOT AVAIL TT_pdf_font THEN DO:
    RUN pdf_error(pdfStream,"pdf_watermark","Font " + QUOTER(pdfFont) + " has not been loaded!").
    RETURN .
  END.
...
PROCEDURE pdf_text_charxy :
  ...
  RUN OutputTextContent(pdfStream, 
                        "TEXTXY",
                          /* igc - rem CHR(10) + */ L_Font + " " + L_PointSize + " Tf" {&debugComment} + CHR(10)
                        + L_color
                        + pdf_get_parameter(pdfStream,"ScaleX") 
                        + " 0 0 " 
                        + pdf_get_parameter(pdfStream,"ScaleY")
                        + " " + STRING(pdfColumn) + " " 
                        + STRING(pdfRow) + " Tm" {&debugComment} + CHR(10)
                        + "(",
                        "~\" + STRING(pdfText),
                        ") Tj" {&debugComment}).
...
PROCEDURE pdf_text_rotate :
  ...
  ASSIGN
    deCos = math_cos(DECIMAL(pdfAngle))
    deSin = math_sin(DECIMAL(pdfAngle))
    L_Rotate = dec2string(deCos) + " " + dec2string(deSin) + " " + dec2string(- deSin) + " " + dec2string(deCos) + " "
    .

  RUN OutputTextContent(pdfStream, 
                        "TEXT",
                          L_Rotate + STRING(pdf_textX(pdfStream))
                        + " " + STRING(pdf_TextY(pdfStream)) + " Tm" {&debugComment},
                        "",
                        "").
...
PROCEDURE pdf_text_scale :
  ...
  IF NOT CAN-FIND(FIRST TT_pdf_stream
                  WHERE TT_pdf_stream.obj_stream = pdfStream NO-LOCK)
  THEN DO:
    RUN pdf_error(pdfStream,"pdf_text_scale","Cannot find Stream!").
    RETURN .
  END.

  L_Scale = STRING(pdfSx) + " 0 0 " + STRING(pdfSy) + " ".

  RUN OutputTextContent(pdfStream, 
                        "TEXT",
                          L_Scale + STRING(pdf_textX(pdfStream))
                        + " " + STRING(pdf_TextY(pdfStream)) + " Tm" {&debugComment},
                        "",
                        "").
...
PROCEDURE pdf_text_skew :
  ...
  RUN OutputTextContent(pdfStream, 
                        "TEXT",
                          L_Skew + STRING(pdf_textX(pdfStream))
                        + " " + STRING(pdf_TextY(pdfStream)) + " Tm" {&debugComment},
                        "",
                        "").
...
PROCEDURE pdf_set_font :
  ...
  RUN OutputTextContent(pdfStream, 
                        ...
                        + dec2string(pdfSize) +  " Tf" {&debugComment},
                        "",
                        "").
...
PROCEDURE pdf_load_font :
  ...
  THEN DO:
    /* 23-FEB-2010 jcc: don't do an error. If the user adds twice the same font, do nothing */
  ...
  ASSIGN TT_pdf_font.obj_stream  = pdfStream
         ...
         TT_pdf_font.used_flag   = TRUE. /* 22-FEB-2010 jcc: if the user loads a font, then it will go within the pdf */
...
PROCEDURE pdf_set_base14_codepage :
  ...
  IF SEARCH(pdfCodePage + '.diff') = ? THEN DO:
  ...
  END.
  ELSE DO:
      RUN pdf_error(pdfStream,"pdf_set_base14_codepage", pdfCodePage + '.diff: ' + cRefOrWidth + ' unknown keyword!').
      RETURN .
  END.
  ...
  THEN DO L_FontNo = 1 TO NUM-ENTRIES(L_Base14PropFonts):
    FIND FIRST TT_pdf_font
      WHERE TT_pdf_font.obj_stream = pdfStream
        AND TT_pdf_font.font_name  = ENTRY(L_FontNo, L_Base14PropFonts) NO-LOCK NO-ERROR.
    IF NOT AVAILABLE TT_pdf_font
    THEN DO:
      RUN pdf_error(pdfStream,"pdf_font_diff_width","Cannot find Font Name = " + ENTRY(L_FontNo, L_Base14PropFonts)).
      RETURN .
    END.
    FIND FIRST TT_Font_Widths
        WHERE TT_Font_Widths.obj_stream = pdfStream
          AND TT_Font_Widths.font-name  = ENTRY(L_FontNo, L_Base14PropFonts) NO-LOCK NO-ERROR.
    IF NOT AVAILABLE TT_Font_Widths
    THEN DO:
      RUN pdf_error(pdfStream,"pdf_font_diff_width","Cannot find widths for font " + ENTRY(L_FontNo, L_Base14PropFonts)).
      RETURN .
    END.
  ...
  RUN OutputTextContent(pdfStream, 
                        "TEXTSKIP",
                         "1 0 0 1 " + STRING(pdf_LeftMargin(pdfstream)) + " "
                                    + STRING(pdf_TextY(pdfStream)) + " Tm" {&debugComment} + CHR(10)
...
PROCEDURE pdf_text_xy :
  ...
  RUN OutputTextContent(pdfStream, 
                        "TEXTXY",
                        L_Font + " " + L_PointSize + " Tf" {&debugComment} + CHR(10)
                        + L_color
                        + pdf_get_parameter(pdfStream,"ScaleX") 
                        + " 0 0 " 
                        + pdf_get_parameter(pdfStream,"ScaleY")
                        ...
                        + STRING(pdfRow) + " Tm" {&debugComment} + CHR(10)
                        + "(",
                        IF cCodePage = "" THEN pdfText ELSE CODEPAGE-CONVERT(pdfText, cCodePage),
                        ") Tj" {&debugComment}).
...
PROCEDURE pdf_text_xy_dec :
  ...
  DEFINE VARIABLE cCodePage AS CHARACTER  NO-UNDO.
  cCodePage = pdf_get_parameter(pdfStream,"CodePage").
  IF cCodePage = "" THEN cCodePage = SESSION:CPINTERNAL.
  ...
  RUN OutputTextContent(pdfStream, 
                        ...
                        + dec2STRING(pdfRow) + " Tm" {&debugComment} + CHR(10)
                        + "(",
                        CODEPAGE-CONVERT(pdfText, cCodePage),
                        ") Tj" {&debugComment}).
...
PROCEDURE pdf_text_at :
  ...
  RUN OutputTextContent(pdfStream, 
                        ...
                        + L_TextY + " Tm" {&debugComment} + CHR(10)
                        + "(",
...
PROCEDURE pdf_text_to:
  ...
  IF pdf_FontType(pdfStream) = "FIXED" THEN DO:
    ...
    RUN OutputTextContent(pdfStream, 
                          ...
                          + STRING(L_Y) + " Tm" {&debugComment} + CHR(10)
                          + "(",
                          pdfText,
                          ") Tj" {&debugComment}).
...
PROCEDURE pdf_set_Scale :  /* PRIVATE */
  ...
  IF NOT CAN-FIND(FIRST TT_pdf_stream
                  WHERE TT_pdf_stream.obj_stream = pdfStream NO-LOCK)
  THEN DO:
    RUN pdf_error(pdfStream,"pdf_set_Scale","Cannot find Stream!").
    RETURN .
  END.

  FIND FIRST B_TT_pdf_param
       WHERE B_TT_pdf_param.obj_stream    = pdfStream
         AND B_TT_pdf_param.obj_parameter = "ScaleX" EXCLUSIVE-LOCK NO-ERROR.
  IF NOT AVAIL B_TT_pdf_param THEN DO:
    CREATE B_TT_pdf_param.
    ASSIGN B_TT_pdf_param.obj_stream    = pdfStream
           B_TT_pdf_param.obj_parameter = "ScaleX".
  END.
  B_TT_pdf_param.obj_value = STRING(pdfSx).
  
  FIND FIRST B_TT_pdf_param
       WHERE B_TT_pdf_param.obj_stream    = pdfStream
         AND B_TT_pdf_param.obj_parameter = "ScaleY" EXCLUSIVE-LOCK NO-ERROR.
  IF NOT AVAIL B_TT_pdf_param THEN DO:
    CREATE B_TT_pdf_param.
    ASSIGN B_TT_pdf_param.obj_stream    = pdfStream
           B_TT_pdf_param.obj_parameter = "ScaleY".
  END.
  B_TT_pdf_param.obj_value = STRING(pdfSy).
...
PROCEDURE pdf_init_param:  /* PRIVATE */
  ...
  */
  /*Anders Olsson Elpool i Umeå AB  26 mar 2019 10:36:14 
  varför LETTER 
  */
  
  RUN pdf_set_PaperType(pdfStream,"A4").  /* This also default the PageWidth
                                             and PageHeight Parameters */
...
PROCEDURE pdf_Header : /* PRIVATE */
  ...
  IF p_Encrypt THEN DO:
    ...
    IF LENGTH(pdf_get_info(p_Stream,"Author"), "character":u) > 0 THEN DO:
      RUN utf8_to_utf16be (pdf_get_info(P_Stream,"Author"), TRUE, OUTPUT m_Author, OUTPUT iSize). /* 19-FEB-2010 jcc */
    ...
    IF LENGTH(pdf_get_info(p_Stream,"Creator"), "character":u) > 0 THEN DO:
      RUN utf8_to_utf16be (pdf_get_info(P_Stream,"Creator"), TRUE, OUTPUT m_Creator, OUTPUT iSize). /* 19-FEB-2010 jcc */
    ...
    IF LENGTH(pdf_get_info(p_Stream,"Producer"), "character":u) > 0 THEN DO:
      RUN utf8_to_utf16be (pdf_get_info(P_Stream,"Producer"), TRUE, OUTPUT m_Producer, OUTPUT iSize). /* 19-FEB-2010 jcc */
    ...
    IF LENGTH(pdf_get_info(p_Stream,"Subject"), "character":u) > 0 THEN DO:
      RUN utf8_to_utf16be (pdf_get_info(P_Stream,"Subject"), TRUE, OUTPUT m_Subject, OUTPUT iSize). /* 19-FEB-2010 jcc */
    ...
    IF LENGTH(pdf_get_info(p_Stream,"Title"), "character":u) > 0 THEN DO:
      RUN utf8_to_utf16be (pdf_get_info(P_Stream,"Title"), TRUE, OUTPUT m_Title, OUTPUT iSize). /* 19-FEB-2010 jcc */
    ...
    IF LENGTH(pdf_get_info(p_Stream,"Keywords"), "character":u) > 0 THEN DO:
      RUN utf8_to_utf16be (pdf_get_info(P_Stream,"Keywords"), TRUE, OUTPUT m_Keywords, OUTPUT iSize). /* 19-FEB-2010 jcc */
  ...
  ELSE DO: /* Not Encrypted */
    /* 12-FEB-2010 jcc: unicode support */
    PUT STREAM S_pdf_inc UNFORMATTED
        "/Author (".
    RUN PutUnicodeContent ('S_pdf_inc', pdf_get_info(p_Stream,"Author")).
    PUT STREAM S_pdf_inc UNFORMATTED ")" {&pdfSKIP}
        "/CreationDate (" l_CreationDate ")" {&pdfSKIP}
        "/Producer (".
    RUN PutUnicodeContent ('S_pdf_inc', pdf_get_info(P_Stream,"Producer")).
    PUT STREAM S_pdf_inc UNFORMATTED ")" {&pdfSKIP}
        "/Creator (" .
    RUN PutUnicodeContent ('S_pdf_inc', pdf_get_info(P_Stream,"Creator")).
    PUT STREAM S_pdf_inc UNFORMATTED ")" {&pdfSKIP}
        "/Subject (".
    RUN PutUnicodeContent ('S_pdf_inc', pdf_get_info(P_Stream,"Subject")).
    PUT STREAM S_pdf_inc UNFORMATTED ")" {&pdfSKIP}
        "/Title (".
    RUN PutUnicodeContent ('S_pdf_inc', pdf_get_info(P_Stream,"Title")).
    PUT STREAM S_pdf_inc UNFORMATTED ")" {&pdfSKIP}
        "/Keywords (".
    RUN PutUnicodeContent ('S_pdf_inc', pdf_get_info(P_Stream,"Keywords")).
    PUT STREAM S_pdf_inc UNFORMATTED ")" {&pdfSKIP}.
  END. /* Not Encrypted */
  /* 12-FEB-2010 jcc: end */
  ...
  FOR EACH TT_pdf_annot WHERE TT_pdf_annot.obj_stream = pdfStream:
    ...
    CASE TT_pdf_annot.annot_type:
      ...
      WHEN "Text" OR WHEN "Stamp" THEN DO:
        PUT STREAM S_pdf_inc UNFORMATTED
            "/T ".
        /* 01-MAR-2010 jcc: add encryption on this bit */
        IF pdfEncrypt THEN DO:
          SET-SIZE(m_EncryptKey) = IF pdf_get_parameter(pdfStream,"EncryptKey") = "40" 
                                 THEN 10 ELSE 32.
          RUN GetEncryptKey /* IN h_PDF-Encrypt */
          ...
          RUN utf8_to_utf16be (TT_pdf_annot.annot_style, TRUE, OUTPUT mLink, OUTPUT iSize). /* 01-MAR-2010 jcc */
        ...
        ELSE DO:
            PUT STREAM S_pdf_inc UNFORMATTED
              "(".
            RUN putUnicodeContent('S_pdf_inc', TT_pdf_annot.annot_style).
            PUT STREAM S_pdf_inc UNFORMATTED
              ")" CHR(13).
        END.
        ...
        IF pdfEncrypt THEN DO:
          ...
          RUN utf8_to_utf16be (TT_pdf_annot.annot_content, TRUE, OUTPUT mLink, OUTPUT iSize). /* 01-MAR-2010 jcc */
          /* SET-SIZE(mLink) = LENGTH(TT_pdf_annot.annot_content, "character":u) + 1. */
          /* PUT-STRING(mlink,1) = TT_pdf_annot.annot_content. */
        ...
        ELSE DO:
            /* 28-FEB-2010 jcc: unicode support */
            /* RUN pdf_replace_text(INPUT-OUTPUT TT_pdf_annot.annot_content). */

            PUT STREAM S_pdf_inc UNFORMATTED
              "(".
            RUN putUnicodeContent('S_pdf_inc', TT_pdf_annot.annot_Content).
            PUT STREAM S_pdf_inc UNFORMATTED
              ")" CHR(13).
            /* 28-FEB-2010 jcc: end */
        END.
...
PROCEDURE utf8_to_utf16be: /* PRIVATE */
  DEFINE INPUT  PARAMETER pdfText AS CHARACTER   NO-UNDO.
  DEFINE INPUT  PARAMETER bom     AS LOGICAL     NO-UNDO.
  DEFINE OUTPUT PARAMETER out     AS MEMPTR      NO-UNDO.
  DEFINE OUTPUT PARAMETER lout    AS INTEGER     INITIAL 1 NO-UNDO.

  DEFINE VARIABLE c    AS INTEGER   NO-UNDO.
  DEFINE VARIABLE cp   AS INTEGER   NO-UNDO.
  DEFINE VARIABLE i    AS INTEGER   NO-UNDO.
  DEFINE VARIABLE iq   AS INTEGER   NO-UNDO.
  DEFINE VARIABLE q    AS INTEGER   EXTENT 4 NO-UNDO.
  DEFINE VARIABLE s    AS INTEGER   NO-UNDO.
  DEFINE VARIABLE s1   AS INTEGER   NO-UNDO.
  DEFINE VARIABLE s2   AS INTEGER   NO-UNDO.

  DEFINE VARIABLE cCodePage AS CHARACTER  NO-UNDO.

  cCodePage = IF SESSION:CPINTERNAL = 'utf-8' THEN 'iso8859-1' ELSE SESSION:CPINTERNAL.

  IF pdfText = '' THEN RETURN.

  SET-SIZE(out) = LENGTH(pdfText, 'CHARACTER') * 2 + IF bom THEN 2 ELSE 0. /* length of the output in bytes = 2 times the length in characters */

  /* 08-MAR-2010 jcc: pdfText has to come already transformed into utf-8 */
  /*pdfText = CODEPAGE-CONVERT(pdfText, 'utf-8').*/

  IF bom THEN DO:
      PUT-BYTE(out, lout) = 254.
      PUT-BYTE(out, lout + 1) = 255.
      lout = lout + 2.
  END.

  DO i = 1 TO LENGTH(pdfText, 'RAW'): /* RAW means bytes */
    c = ASC(SUBSTRING(pdfText, i, 1, 'RAW'), cCodePage, cCodePage). /* 08-MAR-2010 jcc: necessary to specify cCodePage else utf-8 sessions mess it all */
    /* ASCII */
    IF c < 128 /*0x80*/ THEN DO:
      PUT-BYTE(out, lout) = 0.
      PUT-BYTE(out, lout + 1) = c.
      lout = lout + 2.
    END.
    /* Lost continuation byte */
    ELSE IF c < 192 /*0xC0*/ THEN DO:
      PUT-BYTE(out, lout) = 255.
      PUT-BYTE(out, lout + 1) = 253.
      lout = lout + 2.
      NEXT.
    END.
    ...
    ELSE DO:
      ...
      ELSE DO:
        ...
        PUT-BYTE(out, lout + 1) = 253.
        lout = lout + 2.
        DO WHILE ASC(SUBSTRING(pdfText, i + 1, 1, 'RAW'), cCodePage, cCodePage) > 128 /*0x80*/ AND ASC(SUBSTRING(pdfText, i + 1, 1, 'RAW'), cCodePage, cCodePage) < 192 /*0xC0*/:
          i = i + 1.
        END.
        NEXT.
      END.
      ...
      DO WHILE ASC(SUBSTRING(pdfText, i + 1, 1, 'RAW'), cCodePage, cCodePage) > 128 /*0x80*/ AND ASC(SUBSTRING(pdfText, i + 1, 1, 'RAW'), cCodePage, cCodePage) < 192 /*0xC0*/:
        i = i + 1.
        iq = iq + 1.
        q[iq] = ASC(SUBSTRING(pdfText, i, 1, 'RAW'), cCodePage, cCodePage).
      END.
      /* Check length */
      IF iq <> s THEN DO:
        PUT-BYTE(out, lout) = 255.
      ...
      CASE s:
        WHEN 2 THEN DO:
          cp = bin_or(bin_lshift(bin_xor(q[1], 192 /*0xC0*/), 6), bin_xor(q[2], 128 /*0x80*/)).
          /* Overlong sequence */
          IF cp < 128 /*0x80*/ THEN DO:
            PUT-BYTE(out, lout) = 255.
            PUT-BYTE(out, lout + 1) = 253.
            lout = lout + 2.
          END.
          ELSE DO:
            IF cp < 256 THEN DO:
                PUT-BYTE(out, lout) = 0.
                PUT-BYTE(out, lout + 1) = cp.
            END.
            ELSE DO:
                PUT-BYTE(out, lout) = bin_rshift(cp, 8).
                PUT-BYTE(out, lout + 1) = bin_and(cp, 255 /*0xFF*/).
            END.
            lout = lout + 2.
          END.
          NEXT.
        END.
        WHEN 3 THEN DO:
          cp = bin_or(bin_or(bin_lshift(bin_xor(q[1], 224 /*0xE0*/), 12),
                             bin_lshift(bin_xor(q[2], 128 /*0x80*/), 6)),
                             bin_xor(q[3], 128 /*0x80*/)).
          /* Overlong sequence */
          IF cp < 2048 /*0x800*/ THEN DO:
            PUT-BYTE(out, lout) = 255.
            PUT-BYTE(out, lout + 1) = 253.
            lout = lout + 2.
          END.
          /* Check for UTF-8 encoded surrogates (caused by a bad UTF-8 encoder) */
          ELSE IF c > 55296 /*0xD800*/ AND c < 57343 /*0xDFFF*/ THEN DO:
            PUT-BYTE(out, lout) = 255.
            PUT-BYTE(out, lout + 1) = 253.
            lout = lout + 2.
          END.
        ...
        END.
        WHEN 4 THEN DO:
            cp = bin_or(bin_or(bin_or(bin_lshift(bin_xor(q[1], 240 /*0xF0*/), 18),
                                      bin_lshift(bin_xor(q[2], 128 /*0x80*/), 12)),
                                      bin_lshift(bin_xor(q[3], 128 /*0x80*/), 6)),
                                      bin_xor(q[4], 128 /*0x80*/)).
            /* Overlong sequence */
            IF cp < 65536 /*0x10000*/ THEN DO:
              PUT-BYTE(out, lout) = 255.
              PUT-BYTE(out, lout + 1) = 253.
              lout = lout + 2.
            END.
            /* Outside of the Unicode range */    
            ELSE IF cp > 1114111 /*0x10FFFF*/ THEN DO:
              PUT-BYTE(out, lout) = 255.
              PUT-BYTE(out, lout + 1) = 253.
              lout = lout + 2.
            END.
            ELSE DO:
                /* Use surrogates */
                cp = cp - 65536 /*0x10000*/.
                s1 = bin_or(55296 /*0xD800*/, bin_rshift(cp, 10)).
                s2 = bin_or(56320 /*0xDC00*/, bin_and(cp, 1023 /*0x3FF*/)).
                PUT-BYTE(out, lout) = bin_rshift(s1, 8).
                PUT-BYTE(out, lout + 1) = bin_and(s1, 255 /*0xFF*/).
                PUT-BYTE(out, lout + 2) = bin_rshift(s2, 8).
                PUT-BYTE(out, lout + 3) = bin_and(s2, 255 /*0xFF*/).
                lout = lout + 4.
            END.
            NEXT.
        END.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VAR v-index     AS INTEGER NO-UNDO.
  DEFINE VAR v-start     AS INTEGER NO-UNDO.
  DEFINE VAR v-rowmove   AS INTEGER NO-UNDO.
  DEFINE VAR v-leave     AS LOGICAL NO-UNDO.
  ...
  MAIN-BLOCK:
  DO:
      v-rowmove = pdfSkip / 2.    /* initial row movement: middle of the first line */
      THIS-LOOP:
      DO i = 1 TO NUM-ENTRIES(pdfText,"|"):
        ASSIGN v-lastline = v-thisline.
        ...
        IF pdf_text_widthdec (pdfStream, v-thisline) > pdfWidth THEN DO:
          ...
          DO WHILE TRUE:
            ...
            CASE pdfAlignment:
              WHEN "LEFT" THEN
                RUN pdf_text_xy(pdfStream,
                                SUBSTR(v-lastline,v-start,v-index, "character":u),
                                pdfColumn,
                                pdfRow - v-rowmove).
              WHEN "RIGHT" THEN
                RUN pdf_text_xy(pdfStream,
                                SUBSTR(v-lastline,v-start,v-index, "character":u),
                                pdfColumn + pdfWidth - pdf_text_widthdec (pdfStream, SUBSTR(v-lastline,v-start,v-index, "character":u)),
                                pdfRow - v-rowmove).
              WHEN "CENTER" THEN
                RUN pdf_text_xy(pdfStream,
                                SUBSTR(v-lastline,v-start,v-index, "character":u),
                                pdfColumn + ((pdfWidth - pdf_text_widthdec (pdfStream,SUBSTR(v-lastline,v-start,v-index, "character":u) )) / 2),
                                pdfRow - v-rowmove).
            END CASE.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VAR v-index     AS INTEGER NO-UNDO.
  DEFINE VAR v-start     AS INTEGER NO-UNDO.
  DEFINE VAR v-rowmove   AS DECIMAL DECIMALS 4 NO-UNDO.
  DEFINE VAR v-leave     AS LOGICAL NO-UNDO.
  ...
  MAIN-BLOCK:
  DO:
      v-rowmove = 0.    /* initial row movement: middle of the first line */
      THIS-LOOP:
      DO i = 1 TO NUM-ENTRIES(pdfText,"|"):
        ASSIGN v-lastline = v-thisline.
        ...
        /* if the line is greater than the width we have to place it,
           then skip to the next line. We do this always except with
           the first word... */
        IF (pdf_text_widthdec (pdfStream, v-thisline) > pdfWidth AND
            i > 1) THEN DO:
...
PROCEDURE pdf_set_parameter:
  ...
  L_Valid_params = "Compress,Encrypt,UserPassword,MasterPassword,EncryptKey,CodePage,"
...
PROCEDURE pdf_open_PDF:
  ...
  FOR EACH TT_Font WHERE TT_Font.obj_stream = pdfStream.
    ...
    IF TT_pdf_font.font_width = "" THEN DO:
        FIND bTT_pdf_font WHERE bTT_pdf_font.font_name = TT_Font.font_base NO-ERROR.
  ...
  DEFINE BUFFER B_TT_Resource FOR TT_Resource.
        ...
        FOR EACH TT_Resource
           WHERE TT_Resource.obj_stream = pdfStream
             ...
             AND TT_Resource.page_id > 0 /* 22-FEB-2010 jcc */
             AND TT_Resource.res_type   = "XObject"
          BREAK BY TT_Resource.res_text:
    ...
    IF pdf_Page(pdfStream) >= 1 THEN DO:
       ...
       RUN OutputTextContent(pdfStream, 
                             "TEXT",
                               "1 0 0 1 " 
                             + STRING(pdf_LeftMargin(pdfStream)) + " " 
                             + STRING(pdf_BottomMargin(pdfStream)) + " Tm" {&debugComment},
                             "",
                             "").
...
PROCEDURE OutputTextContent: /* PRIVATE */
  ...
  IF pdf_Page(pdfStream) > 0 THEN DO:
    ...
    ELSE IF pdfType BEGINS "TEXT" AND NOT TT_pdf_Stream.obj_DoingText THEN DO:
      ...
      PUT STREAM S_pdf_out UNFORMATTED 
                           "BT" + CHR(10)
                           + L_Font + " " + dec2string(pdf_PointSize(pdfStream)) + " Tf" {&debugComment} + CHR(10)
                           + " 1 0 0 1 " + STRING(pdf_LeftMargin(pdfStream)) + " "
...
PROCEDURE PutUnicodeContent:
    ...
    IF LENGTH(pdfContent) = 0 THEN RETURN.

    CASE pStream:
        WHEN 'S_pdf_out' THEN /* contents of the pdf */
            /* 17-FEB-2010 jcc: TODO: unicode support (for the moment, outputs as is) */
            PUT STREAM S_pdf_out UNFORMATTED pdfContent.
        WHEN 'S_pdf_inc' THEN DO: /* properties of the pdf (Author...), annotations */
            RUN utf8_to_utf16be(pdfContent, TRUE, OUTPUT mptrContent, OUTPUT iLength).
            EXPORT STREAM S_pdf_inc mptrContent.
...
PROCEDURE PutStreamContent:  /* PRIVATE */
      ...
      CASE cChar:
        ...
        WHEN {&BoldOnChar} THEN DO:
          ...
          IF LENGTH(cString) > 0 THEN
              RUN putUnicodeContent('S_pdf_out', StripBoldItal(cString)).
          PUT STREAM S_pdf_out UNFORMATTED
  ...
  ELSE DO:
    /* 12-FEB-2010 jcc: unicode support */
    RUN putUnicodeContent('S_pdf_out', pdfContent).
    /* PUT STREAM S_pdf_out UNFORMATTED pdfContent. */
      ...
      FOR EACH TT_Widget 
        ...
        CASE L_Align:
          WHEN "Right" THEN DO:
            ...
            /* 13-JAN-2011 jcc: take into account european sessions! */
            ...
            IF TT_Object.Rotate = 90 THEN 
              RUN OutputTextContent(pdfStream, 
                                    ...
                                    L_disp + CHR(10)
                                    + "1 0 0 1 " 
                                    + dec2string(DEC(ENTRY(4,L_rect," ")) - L_Width) + " "
                                    + dec2string(DEC(pdf_PageHeight(pdfStream)) - DEC(ENTRY(3, L_rect," ")) ) + " Tm" {&debugComment} + CHR(10) 
                                    + "(",
                                    L_Text,
                                    ") Tj" {&debugComment}).
            ...
            ELSE
              RUN OutputTextContent(pdfStream, 
                                    ...
                                    L_disp + CHR(10)
                                    + "1 0 0 1 " 
                                    + dec2string(DEC(ENTRY(3,L_rect," ")) - L_Width) + " "
                                    + ENTRY(2,TT_Widget.widget_rect," ") + " Tm" {&debugComment} + CHR(10)
                                    + "(",
                                    L_Text,
                                    ") Tj" {&debugComment}).
          ...
          WHEN "Center" THEN DO:
            ...
            L_RectWidth = DEC(ENTRY(3,L_rect," "))
                        - DEC(ENTRY(1,L_rect," ")).
            ...
            IF TT_Object.Rotate = 90 THEN 
              RUN OutputTextContent(pdfStream, 
                                    ...
                                    L_disp + CHR(10)
                                    + "1 0 0 1 " 
                                    + dec2string(DEC(ENTRY(2,L_rect," ")) + L_RectWidth) + " "
                                    + dec2string(DEC(pdf_PageHeight(pdfStream)) - DEC(ENTRY(3, L_rect," ")) ) + " Tm" {&debugComment} + CHR(10) 
                                    + "(",
                                    L_Text,
                                    ") Tj" {&debugComment}).
            ELSE
              RUN OutputTextContent(pdfStream, 
                                    ...
                                    L_disp + CHR(10)
                                    + "1 0 0 1 "
                                    + dec2string(DEC(ENTRY(1,L_rect," ")) + L_RectWidth - L_Width) + " "
                                    + ENTRY(2,TT_Widget.widget_rect," ") + " Tm" {&debugComment} + CHR(10)
                                    + "(",
                                    L_Text,
                                    ") Tj" {&debugComment} ).
            ...
            IF NOT L_MultiLine THEN DO:
              ...
              IF TT_Object.Rotate = 90 THEN 
                RUN OutputTextContent(pdfStream, 
                                      ...
                                      + dec2string(DEC(pdf_PageHeight(pdfStream)) - DEC(ENTRY(3, TT_Widget.widget_rect," ")) ) + " Tm" {&debugComment} + CHR(10) 
                                      + "(",
                                      L_Text,
                                      ") Tj" {&debugComment}).
  ...
  DO L_Loop = 1 TO NUM-ENTRIES(pdfText," "):
    ...
    IF L_Word = "&skip;" THEN DO:
      RUN OutputTextContent(pdfStream, 
                            ...
                            + "1 0 0 1 " 
                            + STRING(pdfLX) + " "
                            + STRING(pdfUY) + " Tm" {&debugComment} + CHR(10)
                            + "(",
                            TRIM(L_Line),
                            ") Tj" {&debugComment}).
...
PROCEDURE DetermineOwnerKey:
  ...
  IF l_Length = 16 THEN DO:  /* Len = 16 equals 128 Bit encryption */
    DO L_Loop = 1 TO 19:
      ...
      DO l_Loop2 = 1 TO 16:
        PUT-BYTE(vrc4-128,l_Loop2) = bin_xor( GET-BYTE(vrc4-key,L_Loop2), l_Loop).
      END.
...
PROCEDURE DetermineUserKey128:
  ...
  DO L_Loop = 1 TO 19:
    ...
    DO l_Loop2 = 1 TO 16:
      PUT-BYTE(vrc4-128,l_Loop2) = bin_xor( GET-BYTE(p_OrigKey,L_Loop2), l_Loop).
    END.
...
PROCEDURE etttpdf_UI :
   EMPTY TEMP-TABLE TT_pdf_image NO-ERROR. 
END PROCEDURE.
