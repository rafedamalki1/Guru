...
DO WHILE AVAILABLE(PERSONALTAB):
   ...
   DO WHILE AVAILABLE(TIDREGITAB): 
      ...
      THEN DO:
         IF ( TIDREGITAB.OVERTIDTILL = "TIMANSTÄLLD" OR  TIDREGITAB.OVERTIDTILL = "SÄVSJÖ TIMANSTÄLLD" OR 
          TIDREGITAB.OVERTIDTILL = "LOKALVÅRDARE" OR 
          TIDREGITAB.OVERTIDTILL = "PRAKTIKANT") AND TIDREGITAB.TOTALT > 0 
         THEN DO:  
      ...
      ELSE DO :
         IF TIDREGITAB.LONTILLAGG NE " " AND TIDREGITAB.LONTILLANTAL NE 0
	      THEN DO TRANSACTION:
            CREATE pa90fil.
	         ...
	         pa90fil.PVECKONUMMER = TIDREGITAB.VECKONUMMER
	         pa90fil.PKOD = avtal.
            IF pa90fil.PLONTILLAGG = "ERSS" THEN DO: 
              ...
              IF AVAILABLE LONTILL THEN DO:
                 IF LONTILL.ENHET = "ST" THEN DO:    
                    /*hårdkodning , registrerat som st i LONTILL. se även GKALVE2.P */
                    ASSIGN 
                    pa90fil.PSORT = "KR"
                    pa90fil.PLONTILLANTAL = pa90fil.PLONTILLANTAL * LONTILL.ERSATTNING.
                 END.
              END.
           END.
	      END.          
        ...
        THEN DO:
           IF ( TIDREGITAB.OVERTIDTILL = "TIMANSTÄLLD" OR  TIDREGITAB.OVERTIDTILL = "SÄVSJÖ TIMANSTÄLLD" OR   TIDREGITAB.OVERTIDTILL = "LOKALVÅRDARE"
      	  OR  TIDREGITAB.OVERTIDTILL = "PRAKTIKANT") AND 
...
/*FOR EACH pa90fil where pa90fil.PLONTILLAGG = "881":                          ??? borde vara 781??
   ...
   END.
END.*/
/*Ta bort kostförmån lart 781 vilart KLUM lönesystem KOSTLM för endags och flerdygnförrättning. L
önesystemet genererar automatiskt
Skicka kostförmån bara för utlandsresa lart 742 vilart UTLT lönesystem TRAKTHEL*/
FOR EACH pa90fil where pa90fil.PLONTILLAGG = "781":
   FIND FIRST pabuff WHERE pabuff.PPERSKOD = pa90fil.PPERSKOD AND pabuff.PDATUM = pa90fil.PDATUM
   AND pabuff.PLONTILLAGG = "742" NO-LOCK NO-ERROR.
   IF NOT AVAILABLE pabuff THEN DO:
      /*kostförmån genereras automatiskt i lönesystemet för endag och flerdygn
      problem när man skickar 0.5 "MEND" = "727"- det genererar bara 0.5 "klum" (781).
      Komplettera med 0.5 klum (781)*/
      FIND FIRST pabuff WHERE pabuff.PPERSKOD = pa90fil.PPERSKOD AND pabuff.PDATUM = pa90fil.PDATUM
      AND pabuff.PLONTILLAGG = "727" AND pabuff.PLONTILLANTAL = 0.5 NO-LOCK NO-ERROR.
      IF AVAILABLE pabuff THEN DO:
         pa90fil.PLONTILLANTAL = 0.5.
      END.
      ELSE DELETE pa90fil.
   END.
END.

FOR EACH pa90fil where pa90fil.PLONTILLAGG = "847":
   /*ny lart Hogia för halv måltidsavdrag endag skickades som 0.5 lart 727 i personec*/
   ASSIGN
   pa90fil.PLONTILLAGG = "727"
   pa90fil.PLONTILLANTAL = 0.5.   
END.
/*översättning  VILART flyttad till gkalve2.p manuell översättning*/
/*FOR EACH pa90fil where pa90fil.PLONTILLAGG NE "":
   FIND FIRST LONTILL WHERE LONTILL.LONTILLAGG = pa90fil.PLONTILLAGG
...
RUN GKALVE2.P.      /* summerar veckans skörd till tidigare */
