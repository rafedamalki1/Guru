...
Date Created: 03/23/92
    Modified: Mario B. 11/17/98 Changed renumbering from using a fixed offset
...
form
   SKIP({&TFM_WID})
/* Commented this out because the text was getting truncated in Traditional 
   Chinese and Simplified Chinese.  This will be fixed in a later version
   the right way by using the no-box editor instead of view-as text */

   /*Fld_Order          label "Current field order" colon 28
   SKIP({&VM_WIDG})*/
...
do:
   ...
   do ON ERROR UNDO, LEAVE  ON STOP UNDO, LEAVE:
      ...
      /* We have to make sure that the new numbers don't conflict with any
      	 existing numbers.  Subtracting 10000 will put them all out of 
      	 range so that we can re-number without worry.
      */
      /* Unfortunately, doing the above made this fail on systems with order
	 numbers greater than 10000.  So, it has been changed.  Never assume.
      */
