/*SOKBEN1.I SOKNING AV BENAMNING HUVUDLEVERANTÖR*/
   ...
   FIND NEXT MTRL WHERE MTRL.BENAMNING MATCHES aosok AND MTRL.LEVKOD = vald_kundlev
   AND MTRL.KALKNR = 0 USE-INDEX BEN NO-LOCK NO-ERROR.
   ELSE 
   FIND NEXT MTRL WHERE MTRL.BENAMNING BEGINS aosok AND MTRL.LEVKOD = vald_kundlev
   AND MTRL.KALKNR = 0 USE-INDEX BEN NO-LOCK NO-ERROR.
   IF NOT AVAILABLE MTRL THEN DO:
      IF begvar = FALSE THEN      
      FIND FIRST MTRL WHERE MTRL.BENAMNING MATCHES aosok AND MTRL.LEVKOD = vald_kundlev
      AND MTRL.KALKNR = 0 USE-INDEX BEN NO-LOCK NO-ERROR.
      ELSE    
      FIND FIRST MTRL WHERE MTRL.BENAMNING BEGINS aosok AND MTRL.LEVKOD = vald_kundlev
      AND MTRL.KALKNR = 0 USE-INDEX BEN NO-LOCK NO-ERROR.
      IF NOT AVAILABLE MTRL THEN DO:
         ...
         ELSE DO:
            ...
            MTRL.LEVKOD NE vald_kundlev AND MTRL.KALKNR = 0 USE-INDEX BEN NO-LOCK.
            ELSE 
