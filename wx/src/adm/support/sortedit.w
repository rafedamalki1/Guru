...
&Scoped-Define ENABLED-OBJECTS RECT-8 brws-list Btn_OK c_sort-phrase ~
Btn_Cancel Btn_Help 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-LABELS NO-COLUMN-SCROLLING SEPARATORS MULTIPLE SIZE 21 BY 6.46.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME fr_sort
     brws-list AT ROW 2.08 COL 2
     Btn_OK AT ROW 1.54 COL 59
     c_sort-phrase AT ROW 2.08 COL 25 NO-LABEL
     Btn_Cancel AT ROW 2.88 COL 59
     b_Insert AT ROW 4.23 COL 59
     b_Remove AT ROW 5.58 COL 59
     b_quick-ins AT ROW 6.92 COL 59
     b_QB AT ROW 8.27 COL 26
     tg_dflt-case AT ROW 8.81 COL 3
     tg_freeform AT ROW 8.27 COL 45
     Btn_Help AT ROW 8.54 COL 59
     "Sort Case:" VIEW-AS TEXT
          SIZE 16 BY .77 AT ROW 1.27 COL 2
     " 'Sort By' Phrase" VIEW-AS TEXT
          SIZE 16 BY .77 AT ROW 1.27 COL 25
     RECT-8 AT ROW 1.54 COL 24
...
DO: /* Call Help Function (or a simple message). */
  ...
  RUN adecomm/_adehelp.p ("UIB":U, "CONTEXT":U, {&XFTR_SortBy_Options_Page}, "":U).
END.
...
------------------------------------------------------------------------------*/
  ...
  IF l_changes2save THEN DO:
    ...
    run-time-code = 
          ...
          + "    'SortBy-Options = " + case-list + ",":U + CHR(10) 
          + "     SortBy-Case = " + (IF AVAILABLE tt THEN tt.sort-case ELSE "")
          + "':U).":U + CHR(10) 
          +  CHR(10) 
