...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BRW_UT WINDOW-2 _STRUCTURED
  QUERY BRW_UT NO-LOCK DISPLAY
      tidut.ut FORMAT "X(132)":U WIDTH 82
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-LABELS NO-COLUMN-SCROLLING SIZE 87.5 BY 26.
...
DEFINE FRAME FRAME-VINST
     ...
     BTN_SKRIV AT ROW 27.96 COL 59
     BTN_AVS AT ROW 27.96 COL 75
...
"tidut.ut" ? ? "character" ? ? ? ? ? ? no ? no no "82" yes no no "U" "" ""
     _Query            is OPENED
*/  /* BROWSE BRW_UT */
&ANALYZE-RESUME
...
-------------------------------------------------------------*/
   ...
   DO TRANSACTION:   
      ...
      ELSE DO:                   
         CREATE tidut. 
         ASSIGN        
         SUBSTRING(tidut.UT,1) = "MATERIELBEREDNING"
...
-------------------------------------------------------------*/
   CREATE tidut. 
   FOR EACH bmtrl_mtrl BY bmtrl_mtrl.ENR:
      FIND FIRST levtemp WHERE levtemp.LEVKOD = bmtrl_mtrl.LEV NO-LOCK NO-ERROR.
      IF AVAILABLE levtemp THEN DO:
         CREATE tidut.      
         ASSIGN  
         SUBSTRING(tidut.UT,1) = bmtrl_mtrl.ENR
         SUBSTRING(tidut.UT,13) = SUBSTRING(bmtrl_mtrl.BENAMNING,1,25)
         SUBSTRING(tidut.UT,39) = SUBSTRING(levtemp.LEVNAMN,1,10).
         IF Guru.Konstanter:mtrlsekvar[6] = TRUE THEN.

         ELSE DO:
            IF bmtrl_mtrl.KPRIS < 0  THEN SUBSTRING(tidut.UT,50) = STRING(bmtrl_mtrl.KPRIS,"->>>99.99").
            ELSE SUBSTRING(tidut.UT,50) = STRING(bmtrl_mtrl.KPRIS,">>>>99.99").
         END.
         ASSIGN
         SUBSTRING(tidut.UT,61) = bmtrl_mtrl.ENHET.    
         IF bmtrl_mtrl.BERKVANT < 0 THEN SUBSTRING(tidut.UT,66) = STRING(bmtrl_mtrl.BERKVANT,"->>>9").
         ELSE SUBSTRING(tidut.UT,66) = STRING(bmtrl_mtrl.BERKVANT,">>>>9").
         IF Guru.Konstanter:mtrlsekvar[6] = TRUE THEN.
         ELSE DO:
            IF bmtrl_mtrl.SUMMA < 0 THEN SUBSTRING(tidut.UT,72) = STRING(bmtrl_mtrl.SUMMA,">>>>99").
...
/* -----------------------------------------------------------
  ...
  Notes:       
-------------------------------------------------------------*/
  /*SUMMERING AV VARJE ENR*/   
   FOR EACH bmtrl_mtrl:    
      bmtrl_mtrl.SUMMA = bmtrl_mtrl.KPRIS * bmtrl_mtrl.BERKVANT.      
   END.     
...
"======================================================================.======".      
   ...
   ELSE DO:   
      CREATE tidut.
      CREATE tidut.         
      SUBSTRING(tidut.UT,59) = "TOTAL SUMMA:".
      CREATE tidut.         
