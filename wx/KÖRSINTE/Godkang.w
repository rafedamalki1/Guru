...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BRW_TID-2 C-Win _STRUCTURED
  QUERY BRW_TID-2 NO-LOCK DISPLAY
      ...
      godmarkpers.DATUM COLUMN-LABEL "Godkänd till" FORMAT "99/99/99":U
  ENABLE
      godmarkpers.PERSONALKOD
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS MULTIPLE SIZE 76.5 BY 17.71.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME DEFAULT-FRAME
     FILL-IN-REGIS AT ROW 3.46 COL 5 COLON-ALIGNED NO-LABEL
     FILL-IN_FORNAMN AT ROW 3.46 COL 24.38 COLON-ALIGNED NO-LABEL
     FILL-IN_NAMN AT ROW 3.46 COL 24.5 COLON-ALIGNED HELP
          "ANGE ORAGNISTIONENSBENÄMNING" NO-LABEL
     FILL-IN_EFTERNAMN AT ROW 3.46 COL 46 COLON-ALIGNED NO-LABEL
     BRW_TID-2 AT ROW 6.42 COL 1.5
     FBTN_VISA AT ROW 8 COL 79.25
     RAD_ALLVAL AT ROW 13.83 COL 1.75 NO-LABEL
     BTN_GODA AT ROW 25.83 COL 10.25
     BTN_GODM AT ROW 25.83 COL 38.25
     BTN_AVB AT ROW 25.83 COL 79.25
     "Det finns inga tidsedlar att godkänna" VIEW-AS TEXT
          SIZE 57 BY 3.5 AT ROW 9.88 COL 7
          FONT 17
     "Godkänna:" VIEW-AS TEXT
          SIZE 15 BY .75 AT ROW 24.75 COL 1.5
     "Godkänna gamla tidsedlar för:" VIEW-AS TEXT
          SIZE 44.13 BY 1.33 AT ROW 1.38 COL 1.5
          FONT 17
...
"godmarkpers.DATUM" "Godkänd till" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
...
DO:
   ...
   IF soktemp.SOKLOG[1] = TRUE THEN DO:
      ...
      RETURN NO-APPLY.   
   END.
   MESSAGE "Vill du verkligen godkänna alla i bilden ?" 
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ...
   IF Guru.Konstanter:appcon THEN DO:                           
      ...
      (INPUT 5,INPUT Guru.Konstanter:globanv,INPUT "",
