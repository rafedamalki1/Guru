...
DEFINE VARIABLE new_lang AS CHARACTER EXTENT 18 NO-UNDO INITIAL [
  ...
  /*8,9*/ "Database", "is not connected.", /* used with 18 */
  /* 10*/ "You must select a current working database at this time.",
  /* 11*/ "is the only database connected - it is already selected.",
  /* 12*/ "You have been automatically switched to database",
  /* 13*/ "This V8 tool can't be used with a PROGRESS/V5, /V6 or /V7 database.",
  ...
  /* 18*/ "Would you like to connect it?"
].
/* LANGUAGE DEPENDENCIES END */ /*------------------------------------------*/

&IF "{&WINDOW-SYSTEM}" = "OSF/Motif" &THEN 
   &GLOBAL-DEFINE LSTHEAD_AT 2.5
&ELSE
...
&IF "{&WINDOW-SYSTEM}" = "OSF/Motif" &THEN 
ON ANY-PRINTABLE OF dlist IN FRAME schema_stuff
DO:
   DEFINE VAR lst_val AS CHAR  NO-UNDO. 

   RUN adecomm/_lstnav.p (INPUT dlist:HANDLE IN FRAME schema_stuff,
      	       	         OUTPUT lst_val).
END.
&ENDIF
...
do:
   ...
   do:
    ...
    /*(CAN-DO("PROGRESS/V5,PROGRESS/V6,PROGRESS/V7",cache_db_t[choice])*/
      then do:
      	 message new_lang[13] /* cannot use V5/V6/V7 db */
...
IF cache_db# = oldbs AND oldbs > 0 THEN DO:
  IF NOT is_dis THEN
    MESSAGE new_lang[5] SKIP new_lang[6] /* only non-v8 dbs connected */
            VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
  look = TRUE.
END.
...
IF is_dis THEN _discon: DO: /*-----------------------------*/ /* disconnect */
  ...
  IF fast_track THEN DO:
    MESSAGE new_lang[17] VIEW-AS ALERT-BOX ERROR BUTTONS OK.
    PAUSE.
    RETURN.
  END.
...
DO: /*---------------------------------------------------------*/ /* select */
  ...
  IF NOT CONNECTED(user_dbname) AND CAN-DO(GATEWAYS,user_dbtype) THEN DO: 
    answer = TRUE.
    MESSAGE new_lang[8] + ' "' + user_dbname + '"' new_lang[9] SKIP
      	    new_lang[18]
      	    VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO UPDATE answer.
