...
DO i = 1 to NUM-DBS:
  IF PDBNAME(i) = pik_first THEN DO:
    ASSIGN oldas4dict = LDBNAME("as4dict").
    CREATE ALIAS "as4dict" FOR DATABASE VALUE(ldbname(i)). /* NO-ERROR.*/
    RUN as4dict/dump/crtmptbl.p.
    CREATE ALIAS "as4dict" FOR DATABASE VALUE(oldas4dict).
    LEAVE  _dbloop.
  END.  
END.
