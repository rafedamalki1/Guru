...
IF UTRYCKNING.EXTID = TRUE THEN DO:
   ...
   ELSE IF regstart = regslut THEN slut4 = 0.
   ...
   OR WEEKDAY(ohjalp.ODATUM) = 6 THEN DO:
      ...
      IF NOT AVAILABLE otidtab THEN DO TRANSACTION:
         ...
         FELTEXT.FELTEXT = 'Kontakta Elpool! För nu är det något fel! Ange läge 32.'
         FELTEXT.PROGRAM = "OVERUTR4" + STRING(TODAY) + STRING(TIME,"HH:MM") + Guru.Konstanter:globanv.  
         RETURN.
      END.  
      nytid = ohjalp.OSTART.
      RUN TIMSEK.P.
...
IF Guru.Konstanter:globforetag = "MSK " AND ANSTFORMTAB.KOD = "K" THEN slut4 = 0.
IF UTRYCKNING.EXAKTM = TRUE THEN DO:
   ...
   OR WEEKDAY(ohjalp.ODATUM) = 6 THEN DO:
      ...
      IF NOT AVAILABLE otidtab THEN DO TRANSACTION:
         ...
         ASSIGN 
         FELTEXT.ANVANDARE = Guru.Konstanter:globanv         
         FELTEXT.FELTEXT = 'Kontakta Elpool! För nu är det något fel! Ange läge 33.'
         FELTEXT.PROGRAM = "OVERUTR4" + STRING(TODAY) + STRING(TIME,"HH:MM") + Guru.Konstanter:globanv.
         RETURN.
      END.                    
      nytid = ohjalp.OSLUT.
...
IF NOT AVAILABLE ovavtab THEN DO:
   ...
   otidtab.KOD = ANSTFORMTAB.KOD AND otidtab.OVERTIDUTTAG = ohjalp.OVERTIDUTTAG AND
   ...
   IF NOT AVAILABLE otidtab THEN DO:
      ...
      IF NOT AVAILABLE otidtab THEN DO TRANSACTION:            
         ...
         ASSIGN 
         FELTEXT.ANVANDARE = Guru.Konstanter:globanv         
         FELTEXT.FELTEXT = 'Kontakta Elpool! För nu är det något fel! Ange läge 34.' + string(slut4) + string(start4).
         FELTEXT.PROGRAM = "OVERUTR4" + STRING(TODAY) + STRING(TIME,"HH:MM") + Guru.Konstanter:globanv.
         RETURN.
      END.
...
ELSE DO:
   ...
   ovavtab.KOD = ANSTFORMTAB.KOD AND ovavtab.OVERTIDUTTAG = ohjalp.OVERTIDUTTAG AND
   ...
   IF NOT AVAILABLE ovavtab THEN DO:
      ...
      IF NOT AVAILABLE ovavtab THEN DO TRANSACTION:
         ...
         ASSIGN 
         FELTEXT.ANVANDARE = Guru.Konstanter:globanv         
         FELTEXT.FELTEXT = 'Kontakta Elpool! För nu är det något fel! Ange läge 35.'
         FELTEXT.PROGRAM = "OVERUTR4" + STRING(TODAY) + STRING(TIME,"HH:MM") + Guru.Konstanter:globanv.
         RETURN.
      END.
...
IF sluta LE stop1 THEN DO:
   ...
   ELSE IF ohjalp.OANT2 > 0 THEN DO:   /*SKIFTPERSONAL med annan arbetstid tex 14-22 men gräns 6*/
...
ELSE DO:
      ...
      IF NOT AVAILABLE ovavtab THEN DO:
	     ...
	     IF NOT AVAILABLE otidtab THEN DO TRANSACTION:
	        CREATE FELTEXT.
               ASSIGN 
               FELTEXT.ANVANDARE = Guru.Konstanter:globanv         
               FELTEXT.FELTEXT = 'Kontakta Elpool! För nu är det något fel! Ange läge 36.'
               FELTEXT.PROGRAM = "OVERUTR4" + STRING(TODAY) + STRING(TIME,"HH:MM") + Guru.Konstanter:globanv.
	        RETURN.
     	     END.
	     ...
	     ovavtab.STOPP2 NE ovavtab.START2 USE-INDEX OSTART2 NO-LOCK NO-ERROR.
 	     IF NOT AVAILABLE ovavtab THEN DO TRANSACTION:
	        CREATE FELTEXT.
               ASSIGN 
               FELTEXT.ANVANDARE = Guru.Konstanter:globanv         
               FELTEXT.FELTEXT = 'Kontakta Elpool! För nu är det något fel! Ange läge 37.'
               FELTEXT.PROGRAM = "OVERUTR4" + STRING(TODAY) + STRING(TIME,"HH:MM") + Guru.Konstanter:globanv.
	        RETURN.
	     END.
   ...
   ELSE DO:
      IF sluta GE stop1 THEN DO:
	     ...
	     IF NOT AVAILABLE otidtab THEN DO:
	        FIND FIRST otidtab WHERE otidtab.DAG = WEEKDAY(ohjalp.ODATUM) AND
	        otidtab.KOD = ANSTFORMTAB.KOD AND otidtab.OVERTIDUTTAG = ohjalp.OVERTIDUTTAG
	     ...
	     IF NOT AVAILABLE ovavtab THEN DO:
	        ...
	        IF NOT AVAILABLE ovavtab THEN DO TRANSACTION:
	           CREATE FELTEXT.
                  ASSIGN 
                  FELTEXT.ANVANDARE = Guru.Konstanter:globanv         
                  FELTEXT.FELTEXT = 'Kontakta Elpool! För nu är det något fel! Ange läge 38.'
                  FELTEXT.PROGRAM = "OVERUTR4" + STRING(TODAY) + STRING(TIME,"HH:MM") + Guru.Konstanter:globanv.
	           RETURN.
	        END.
...
ELSE DO:
   ...
   /*NYTT SAMMA KOD SKALL OCKSÅ TOLKAS PÅ OST2 OSL2 ANNARS BLIR 22-02 2 + 3 TIMMAR*/
      FIND FIRST ohjalp WHERE ohjalp.RECTIDVIS = reco NO-ERROR.
      IF NOT AVAILABLE ohjalp THEN regdatum = regdatum.        
      nytid = ohjalp.OSTART.
      RUN TIMSEK.P.
