...
&ENDIF
  source-file {&STDPH_FILL} FORMAT "x(80)" VIEW-AS FILL-IN SIZE {&FILLCH-1} BY 1
         LABEL "&Input File" COLON {&LINEUP-1}
  btn_File_Source SKIP({&VM_WIDG})
  target-file {&STDPH_FILL} FORMAT "x(80)" VIEW-AS FILL-IN SIZE {&FILLCH-1} BY 1
...
&ENDIF
  source-file {&STDPH_FILL} FORMAT "x(80)" VIEW-AS FILL-IN SIZE {&FILLCH-q} BY 1
         LABEL "&Input File" COLON {&LINEUP-q}
  btn_File_Source SKIP({&VM_WIDG})
  target-file {&STDPH_FILL} FORMAT "x(80)" VIEW-AS FILL-IN SIZE {&FILLCH-q} BY 1
...
&ENDIF
  source-file {&STDPH_FILL} FORMAT "x(80)" VIEW-AS FILL-IN SIZE {&FILLCH-c} BY 1
         LABEL "&Input File" COLON {&LINEUP-c}
  btn_File_Source SKIP({&VM_WIDG})
  target-file {&STDPH_FILL} FORMAT "x(80)" VIEW-AS FILL-IN SIZE {&FILLCH-c} BY 1
...
&ENDIF
  quoter-file {&STDPH_FILL} FORMAT "x(80)" VIEW-AS FILL-IN 
...
&ENDIF
  ...
  "OS2:"                                              TO 14
    "OS2 SILENT"                                      AT 16 
      msg[4] FORMAT "x(50)"                           AT 30 SKIP
  "BTOS/CTOS:"                                        TO 14
    "BTOS SILENT"                                     AT 16 
      msg[5] FORMAT "x(50)"                           AT 30 SKIP
  "VMS:"                                              TO 14
    "VMS SILENT"                                      AT 16 
      msg[6] FORMAT "x(50)"                           AT 30 SKIP
   {adecomm/okform.i
      &BOX    = rect_btns
      &STATUS = no
      &OK     = btn_OK
...
IF q-mode = "m" THEN DO:
  ...
  ELSE IF OPSYS = "OS2"   THEN c = "quoter -c ".
  ELSE IF OPSYS = "UNIX"  THEN c = "quoter -c ".
  ELSE IF OPSYS = "VMS"   THEN c = "PROGRESS/TOOLS=QUOTER/COLUMN=".
  ELSE IF OPSYS = "BTOS"  THEN c = SEARCH("Quoter.Run") + " Quoter -c ".
