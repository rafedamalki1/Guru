...
DEFINE VARIABLE antal AS DECIMAL FORMAT "-999.99" NO-UNDO.   /*LÖN*/
...
DEFINE VARIABLE overrapp1 AS CHARACTER FORMAT "X(101)" NO-UNDO.      /*LÖN*/
...
DEFINE VARIABLE ptal LIKE ANSTFORMTAB.KOD NO-UNDO.
DEFINE VARIABLE patal LIKE ANSTFORMTAB.KOD NO-UNDO.
...
DEFINE VARIABLE fminus AS CHARACTER FORMAT "X(1)" NO-UNDO.
...
DEFINE TEMP-TABLE pa90filen
   ...
   FIELD PVECKONUMMER LIKE TIDREGITAB.VECKONUMMER
   FIELD BOLAG AS CHARACTER FORMAT "X(2)"
   FIELD MINUS AS CHARACTER FORMAT "X(1)"
   INDEX PANSTNR IS PRIMARY PANSTNR ASCENDING.
...
/*FOR EACH pa90fil BY pa90fil.PANSTNR BY PLONTILLAGG:
   ...
   IF pa90fil.PLONTILLAGG = "FRUK" THEN DO TRANSACTION:
   ...
   END.
   IF pa90fil.PLONTILLAGG = "LUNC" THEN DO TRANSACTION:
      FIND FIRST LONTILL WHERE LONTILL.VILART = pa90fil.PLONTILLAGG
      USE-INDEX VILART NO-LOCK NO-ERROR.
      ASSIGN pa90fil.PLONTILLAGG = "847"
      pa90fil.PLONTILLANTAL = LONTILL.ERSATTNING * pa90fil.PLONTILLANTAL.
   END.
   IF pa90fil.PLONTILLAGG = "MIDD" THEN DO TRANSACTION:
      FIND FIRST LONTILL WHERE LONTILL.VILART = pa90fil.PLONTILLAGG
      USE-INDEX VILART NO-LOCK NO-ERROR.
      ASSIGN pa90fil.PLONTILLAGG = "847"
      pa90fil.PLONTILLANTAL = LONTILL.ERSATTNING * pa90fil.PLONTILLANTAL.
   END.
   IF pa90fil.PLONTILLAGG = "FRLU" THEN DO TRANSACTION:
      FIND FIRST LONTILL WHERE LONTILL.VILART = pa90fil.PLONTILLAGG
      USE-INDEX VILART NO-LOCK NO-ERROR.
      ASSIGN pa90fil.PLONTILLAGG = "847"
      pa90fil.PLONTILLANTAL = LONTILL.ERSATTNING * pa90fil.PLONTILLANTAL.
   END.
   IF pa90fil.PLONTILLAGG = "FRMI" THEN DO TRANSACTION:
      FIND FIRST LONTILL WHERE LONTILL.VILART = pa90fil.PLONTILLAGG
      USE-INDEX VILART NO-LOCK NO-ERROR.
      ASSIGN pa90fil.PLONTILLAGG = "847"
      pa90fil.PLONTILLANTAL = LONTILL.ERSATTNING * pa90fil.PLONTILLANTAL.
   END.
   IF pa90fil.PLONTILLAGG = "LUMI" THEN DO TRANSACTION:
      FIND FIRST LONTILL WHERE LONTILL.VILART = pa90fil.PLONTILLAGG
      USE-INDEX VILART NO-LOCK NO-ERROR.
      ASSIGN pa90fil.PLONTILLAGG = "847"
      pa90fil.PLONTILLANTAL = LONTILL.ERSATTNING * pa90fil.PLONTILLANTAL.
   END.
   IF pa90fil.PLONTILLAGG = "FLMI" THEN DO TRANSACTION:
      FIND FIRST LONTILL WHERE LONTILL.VILART = pa90fil.PLONTILLAGG
      USE-INDEX VILART NO-LOCK NO-ERROR.
      ASSIGN pa90fil.PLONTILLAGG = "847"
      pa90fil.PLONTILLANTAL = LONTILL.ERSATTNING * pa90fil.PLONTILLANTAL.
   END.
   IF pa90fil.PLONTILLAGG = "RFRU" THEN DO TRANSACTION:
      FIND FIRST LONTILL WHERE LONTILL.VILART = pa90fil.PLONTILLAGG
      USE-INDEX VILART NO-LOCK NO-ERROR.
      ASSIGN pa90fil.PLONTILLAGG = "847"
      pa90fil.PLONTILLANTAL = LONTILL.ERSATTNING * pa90fil.PLONTILLANTAL.
   END.   
   IF pa90fil.PLONTILLAGG = "RLUN" THEN DO TRANSACTION:
      FIND FIRST LONTILL WHERE LONTILL.VILART = pa90fil.PLONTILLAGG
      USE-INDEX VILART NO-LOCK NO-ERROR.
      ASSIGN pa90fil.PLONTILLAGG = "847"
      pa90fil.PLONTILLANTAL = LONTILL.ERSATTNING * pa90fil.PLONTILLANTAL.
   END.
   IF pa90fil.PLONTILLAGG = "RMID" THEN DO TRANSACTION:
      FIND FIRST LONTILL WHERE LONTILL.VILART = pa90fil.PLONTILLAGG
      USE-INDEX VILART NO-LOCK NO-ERROR.
      ASSIGN pa90fil.PLONTILLAGG = "847"
      pa90fil.PLONTILLANTAL = LONTILL.ERSATTNING * pa90fil.PLONTILLANTAL.
   END.
   IF pa90fil.PLONTILLAGG = "KFRU" THEN DO TRANSACTION:
      FIND FIRST LONTILL WHERE LONTILL.VILART = pa90fil.PLONTILLAGG
      USE-INDEX VILART NO-LOCK NO-ERROR.
      ASSIGN pa90fil.PLONTILLAGG = "808"
      pa90fil.PLONTILLANTAL = LONTILL.ERSATTNING * pa90fil.PLONTILLANTAL.
   END.
   IF pa90fil.PLONTILLAGG = "KLUN" THEN DO TRANSACTION:
      FIND FIRST LONTILL WHERE LONTILL.VILART = pa90fil.PLONTILLAGG
      USE-INDEX VILART NO-LOCK NO-ERROR.
      ASSIGN pa90fil.PLONTILLAGG = "808"
      pa90fil.PLONTILLANTAL = LONTILL.ERSATTNING * pa90fil.PLONTILLANTAL.
   END.
   IF pa90fil.PLONTILLAGG = "KMID" THEN DO TRANSACTION:
      FIND FIRST LONTILL WHERE LONTILL.VILART = pa90fil.PLONTILLAGG
      USE-INDEX VILART NO-LOCK NO-ERROR.
      ASSIGN pa90fil.PLONTILLAGG = "808"
      pa90fil.PLONTILLANTAL = LONTILL.ERSATTNING * pa90fil.PLONTILLANTAL.
...
ELSE DO:
   ...
   DO TRANSACTION:
      ...
      IF AVAILABLE pa90fil THEN DO:
         ...
         ELSE IF globforetag = "ESAN" AND pa90fil.PAVTAL = "TC" THEN globforetag = globforetag.
         ELSE IF globforetag = "ESAN" AND pa90fil.PAVTAL = "KA" THEN globforetag = globforetag.
         ELSE IF globforetag = "ESAN" AND pa90fil.PAVTAL = "TJ" THEN globforetag = globforetag.
         ELSE IF globforetag = "ESAN" AND pa90fil.PAVTAL = "TK" THEN globforetag = globforetag.
         ELSE IF globforetag = "ESAN" AND pa90fil.PAVTAL = "TL" THEN globforetag = globforetag.
         ELSE DELETE pa90fil.      
      END.         
   END.   
   REPEAT:
...
REPEAT:
   DO TRANSACTION:
      FIND NEXT pa90fil WHERE pa90fil.PLONTILLAGG = "883" EXCLUSIVE-LOCK NO-ERROR.
      IF NOT AVAILABLE pa90fil THEN LEAVE.
      ELSE DELETE pa90fil.
   END. 
END.  
...
REPEAT:
   DO TRANSACTION:
      FIND NEXT pa90fil WHERE pa90fil.PLONTILLAGG = "885" EXCLUSIVE-LOCK NO-ERROR.
      IF NOT AVAILABLE pa90fil THEN LEAVE.
      ELSE DELETE pa90fil.
   END. 
END.  
...
REPEAT:
   DO TRANSACTION:
      FIND NEXT pa90fil WHERE pa90fil.PLONTILLAGG = "886" EXCLUSIVE-LOCK NO-ERROR.
      IF NOT AVAILABLE pa90fil THEN LEAVE.
      ELSE DELETE pa90fil.
   END. 
END.  
...
REPEAT:
   DO TRANSACTION:
      FIND NEXT pa90fil WHERE pa90fil.PLONTILLAGG = "894" EXCLUSIVE-LOCK NO-ERROR.
      IF NOT AVAILABLE pa90fil THEN LEAVE.
      ELSE DELETE pa90fil.
   END. 
END.  
...
REPEAT:
   DO TRANSACTION:
      FIND NEXT pa90fil WHERE pa90fil.PLONTILLAGG = "895" EXCLUSIVE-LOCK NO-ERROR.
      IF NOT AVAILABLE pa90fil THEN LEAVE.
      ELSE DELETE pa90fil.
   END. 
END.  */
...
BY pa90fil.AONR  BY pa90fil.DELNR BY pa90fil.PVECKONUMMER:
   ...
   AND drnr = dnr AND vrnr = venr THEN DO:
      FIND FIRST LONTILL WHERE LONTILL.KOD = patal AND LONTILL.VILART = lon USE-INDEX VILART NO-LOCK NO-ERROR.
...
IF globforetag = "ESAN" THEN DO:
   OUTPUT TO /u10/guru/export/esan/ESFEL0606.
END.
FOR EACH pa90filen BY pa90filen.PANSTNR BY pa90filen.PTILLAGG:         
   ...
   END.
   ELSE IF pa90filen.PVECKONUMMER = 201 AND YEAR(pa90filen.PDATUM) = 2001 THEN DO: /*31 DEC 2001*/
      typdatum = "2" + STRING(pa90filen.PVECKONUMMER,"999").  
   END.
   ELSE DO:
   ...
   SUBSTRING(overrapp1,100,1) = pa90filen.MINUS.
