...
Modified:
    mcmann      98/06   changed logical name check 98-06-23-010
...
User_Id Pass_word Trig_Loc btn_filep Parm_File btn_filet Unix_Label ~
Unix_Parms 
...
DEFINE VARIABLE Network AS CHARACTER FORMAT "X(256)":U 
     ...
     LIST-ITEMS "(None)","AS400LFP","AS400SNA","CTOSCLUSTER","NETBIOS","SPX","TCP","TLI","WIPC" 
...
DEFINE FRAME Connect
     ...
     Unix_Label AT ROW 12.57 COL 3.8
...
ON CHOOSE OF btn_filen IN FRAME Connect /* Files... */
DO:
...
ON CHOOSE OF btn_filep IN FRAME Connect /* Files... */
DO:
...
ON CHOOSE OF btn_filet IN FRAME Connect /* Files... */
DO:
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ...
    assign Dlg_ShortH = FRAME {&FRAME-NAME}:HEIGHT -
                        (FRAME {&FRAME-NAME}:HEIGHT
                         ...
                         &IF "{&WINDOW-SYSTEM}" = "OSF/Motif":U &THEN
                         - 1
                         &ENDIF
