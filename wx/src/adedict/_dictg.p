...
PROCEDURE enable_widgets:
   ...
   */
   /*-------------------------------------------------------------------
      I commented out the following do-loop and if-condition, since
      both are not taking care of foreign DB's (which need not to be
      connected) and changed db's (number of in tool disconnected db's
      = number of in tool newly connected db's). I instead always clear
      the whole cache and start again  <hutegger 94/06> 
      
      do ix = 1 to s_lst_Dbs:NUM-ITEMS in frame browse while all_conn:
          all_conn = all_conn AND 
                     connected (s_lst_Dbs:entry(ix) in frame browse).  
       end.

       if save_numdbs <> NUM-DBS OR NOT all_conn then
       do: 
   ...
   */
   ...
   assign
      ...
      */ 
      ...
      /* s_icn_Doms:sensitive in frame browse = yes */
      ...
      */
      /*----
      s_icn_Doms:hidden   in frame browse = yes
      s_DomLbl:hidden     in frame browse = yes
      s_DomFill:hidden    in frame browse = yes
      s_lst_Doms:hidden   in frame browse = yes
      ----*/ 
      /* WIN95 s_SeqLbl:hidden     in frame browse = yes */
      s_SeqFill:hidden    in frame browse = yes
      s_lst_Seqs:hidden   in frame browse = yes
      s_icn_Flds:hidden   in frame browse = yes
     /* WIN95 s_FldLbl:hidden     in frame browse = yes */
      s_FldFill:hidden    in frame browse = yes
      s_lst_Flds:hidden   in frame browse = yes
      s_icn_Idxs:hidden   in frame browse = yes
      /* WIN95 s_IdxLbl:hidden     in frame browse = yes */
   ...
   IF SESSION:PIXELS-PER-COL = 6 AND SESSION:PIXELS-PER-ROW = 21 THEN
     ...
     ASSIGN s_icn_dbs:Y in frame browse  = s_icn_dbs:Y in frame browse - 10
            ...
            /* WIN95 s_DbLbl:Y in frame browse    = s_icn_dbs:Y in FRAME browse +
                               s_icn_dbs:HEIGHT-PIXELS in FRAME browse + 2        */
            /* WIN95 s_TblLbl:Y in frame browse   = s_DbLbl:Y in frame browse
                   s_SeqLbl:Y in frame browse   = s_DbLbl:Y in frame browse
                   s_FldLbl:Y in frame browse   = s_DbLbl:Y in frame browse
                   s_IdxLbl:Y in frame browse   = s_DbLbl:Y in frame browse
            */
            .
  ...
  /* WIN95
   assign
      ...
      */
      ...
      /* s_DomLbl:screen-value in frame browse = s_DomLbl */
   ...
   do:
      ...
      */
      ...
      assign
         ...
         /* WIN95 s_TblLbl:hidden   in frame browse = yes */
