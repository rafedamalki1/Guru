...
where     
  helpfile is a string which defines the help file to use with
           a call to SYSTEM-HELP. If "uib" is specified, special
...
DEFINE FRAME f_cuecard
     b_close AT ROW 1.23 COL 34
     b_Help AT ROW 1.23 COL 41.57
     e_cue-text AT ROW 2.54 COL 1.57 NO-LABEL
     t_show AT ROW 9.65 COL 2.57
...
IF NOT w_smbr-cue:LOAD-ICON("adeicon/cue") THEN
...
DO:
  IF hlpfile = "uib" THEN
    RUN adecomm/_adehelp.p ("uib", "CONTEXT", hlpcontext, ?).
  ELSE DO:
...
DO WHILE VALID-HANDLE(h):
  IF h:TYPE = "WINDOW" THEN IF h:TITLE = "User Interface Builder" THEN
    hUIB = h.
