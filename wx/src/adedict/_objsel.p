	 ...
	 if s_Lvl2Obj = {&OBJ_FLD} then
	    ...
	    run adedict/_brwlist.p (INPUT {&OBJ_FLD}).
      	    ...
      	    /* What I want is to refresh index window with first index for 
      	       ...
      	       current table and it isn't.  No time to fix.  For now, just
      	       close the window.
      	    */
      	    if s_win_Idx <> ? then
      	    do:
	    ...
	    run adedict/_brwlist.p (INPUT {&OBJ_IDX}).
      	    ...
      	    */
      	    ...
      	    do:
	       	  ...
	       	  find FIRST _Field of _File use-index _Field-name NO-ERROR.
      	       else
	       	  find FIRST _Field of _File use-index _Field-position NO-ERROR.
