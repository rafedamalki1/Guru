...
DEF INPUT PARAMETER BodyType        as CHARACTER  NO-UNDO.
...
/*
IF useffwdir THEN DO:
  RUN ffw/procs/gethostname.p(OUTPUT vLocalHostName).
END.  
ELSE DO:
   MESSAGE "smtp 1.5"
VIEW-AS ALERT-BOX. 
   RUN gethostname.p(OUTPUT vLocalHostName).
END.
*/ 
DEFINE VARIABLE outanvanv AS CHARACTER NO-UNDO.
RUN INLOAPI.P (OUTPUT outanvanv, OUTPUT vLocalHostName).   
...
ELSE IF SESSION:BATCH-MODE THEN DO:
  ...
  DO WHILE sending:
    ...
    IF start-etime + 60000 < MTIME THEN DO:
      sending = NO.
...
PROCEDURE readhandler:
 ...
 /* Used to Build MIME Message Body */
  ...
  IF vlength > 0 THEN DO:
    ...
    END CASE. /* vstate */
  /*Anders Olsson Elpool i Umeå AB  7 jun 2017 14:41:44 
  bra meddelande för felsökning 
  */
  /*   MESSAGE str vstate vMESSAGE v 
VIEW-AS ALERT-BOX.
*/
...
PROCEDURE WriteData:
  ...
  start-Etime = MTIME. /* Reset timer because we had activity */
  ASSIGN
...
PROCEDURE dofiles:
  ...
  IF cFileType = "Binary" THEN DO:
    ...
    DO while cBinaryFile <> ?:
      /* Use of MTIME in the name is to further minimize the 
      ...
      cLocalFile = EncodeDirectory + "en" + STRING(MTIME) + 
