...
DEFINE SHARED VARIABLE kto AS CHARACTER NO-UNDO.
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-LABELS NO-COLUMN-SCROLLING SIZE 93.5 BY 26.
...
DEFINE FRAME FRAME-VINST
     ...
     BTN_SKRIV AT ROW 27.96 COL 64.75
     BTN_AVS AT ROW 27.96 COL 81
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ...
   {muswait.i} 
   {ALLSTARTDYN.I} 
   FIND FIRST omrtemp WHERE omrtemp.OMRADE = valomr USE-INDEX OMR
   NO-LOCK NO-ERROR.
   {&WINDOW-NAME}:TITLE = "Budget för " + LC(Guru.Konstanter:gomrk) + ":" + omrtemp.NAMN + 
   ...
   DO TRANSACTION:         
      ...
      ELSE DO: 
         IF Guru.Konstanter:appcon THEN DO:                           
            RUN BUDTIDHMT.P ON Guru.Konstanter:apphand TRANSACTION DISTINCT 
            (INPUT kto,INPUT valomr,INPUT franar,OUTPUT TABLE tidut).
         END.
         ELSE DO:
            RUN BUDTIDHMT.P 
            (INPUT kto,INPUT valomr,INPUT franar,OUTPUT TABLE tidut).                 
...
/*          SUBSTRING(tidut.UT,8) = omrtemp.NAMN.             */
/*          CREATE tidut.                                     */
