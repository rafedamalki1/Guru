...
PROCEDURE DeleteMinMaxButtons :
  ...
  run GetWindowLong{&A} IN Guru.Konstanter:hpApi(hNonClient, 
                                 {&GWL_STYLE}, 
                                 output style).
  ...
  run SetWindowLong{&A} IN Guru.Konstanter:hpApi(hNonClient, 
                                {&GWL_STYLE}, 
                                style, 
                                output oldstyle).
...
PROCEDURE AddHelpButton :
  ...
  run FlashWindow IN Guru.Konstanter:hpApi (hNonClient,0, output ReturnValue).
...
PROCEDURE AddPaletteStyle :
  ...
  run ShowWindow IN Guru.Konstanter:hpApi(hNonClient, 0, output ReturnValue). /* 0 = SW_HIDE */
...
PROCEDURE NoSizingMenu :
  ...
  run GetSystemMenu IN Guru.Konstanter:hpApi (hNonClient, 0, output hSysmenu).
  run DeleteMenu IN Guru.Konstanter:hpApi (hSysmenu, {&SC_SIZE}    , {&MF_BYCOMMAND}, output ReturnValue).
  run DeleteMenu IN Guru.Konstanter:hpApi (hSysmenu, {&SC_MINIMIZE}, {&MF_BYCOMMAND}, output ReturnValue).
  run DeleteMenu IN Guru.Konstanter:hpApi (hSysmenu, {&SC_MAXIMIZE}, {&MF_BYCOMMAND}, output ReturnValue).
END PROCEDURE.
...
PROCEDURE FindCaptionParent :
  ...
  do while result=0 and hNonClient<>0 :
    run GetWindowLong{&A} IN Guru.Konstanter:hpApi(hClient, 
                                  {&GWL_STYLE}, 
                                  output style).
...
PROCEDURE FitFrame :
  ...
  run GetClientRect IN Guru.Konstanter:hpApi(hNonClient, 
