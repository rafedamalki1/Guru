...
DEFINE VARIABLE bodydoc AS CHARACTER INITIAL "Berlista2" LABEL "Berlista2" NO-UNDO.
...
FOR EACH tidut NO-LOCK: 
   ...
   END.
   /*Rubrik*/
   IF SUBSTRING(tidut.UT,1,13) = "Materiellista upplag" THEN DO: 
      boldblack = TRUE.      
   END.
...
IF Guru.GlobalaVariabler:plusaonr = "" OR Guru.GlobalaVariabler:plusaonr = ? THEN DO:
   utfil = utfil + STRING(TIME) + "ber2.htm".
END.
ELSE DO:
   utfil = utfil + TRIM (Guru.GlobalaVariabler:plusaonr) + TRIM(STRING(Guru.GlobalaVariabler:plusdnr,Guru.Konstanter:varforetypchar[1])) + STRING(TIME) + "ber2.htm".
END.
