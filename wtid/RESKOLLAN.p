...
DEFINE VARIABLE rskoll AS DECIMAL NO-UNDO.
      ...
      ELSE DO:              
         ...
         IF AVAILABLE TIDREGITAB THEN DO:
            IF regdatum = bdatum AND TIDREGITAB.ENFLERDAGS = enfle THEN DO:
               FIND LAST TIDREGITAB WHERE TIDREGITAB.PERSONALKOD = pkod AND
               TIDREGITAB.DATUM = regdatum AND
               TIDREGITAB.ENFLERDAGS NE " " AND TIDREGITAB.RESMAL BEGINS "Resans slut: " USE-INDEX PSTART NO-LOCK NO-ERROR.
               IF AVAILABLE TIDREGITAB THEN DO:                                                                                          
                  rskoll = decimal(SUBSTRING(TIDREGITAB.RESMAL,14,5)) NO-ERROR.
                  IF rstart GE  rskoll THEN   musz = TRUE.
               END.                 
               ELSE DO:
                  FIND LAST TIDREGITAB WHERE TIDREGITAB.PERSONALKOD = pkod AND
                  TIDREGITAB.DATUM = regdatum AND
                  TIDREGITAB.ENFLERDAGS NE " " AND TIDREGITAB.RESMAL BEGINS "Utlandsr.slut: " USE-INDEX PSTART NO-LOCK NO-ERROR.
                  IF AVAILABLE TIDREGITAB THEN DO:                     
                     rskoll = decimal(SUBSTRING(TIDREGITAB.RESMAL,16,5)) NO-ERROR.
                     IF  rstart ge  rskoll THEN  musz = TRUE.
                  END.                                                       
               END.
               ...
               ELSE DO:               
                  CREATE felmeddtemp.
                  felmeddtemp.FELMEDD = "Det finns redan en resa upplagd " + STRING(regdatum) + " med slut " +  string(rskoll).               
                  LEAVE.
               END.
            END.     
