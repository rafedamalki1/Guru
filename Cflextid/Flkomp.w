...
DEFINE INPUT PARAMETER alval AS INTEGER NO-UNDO.
...
&Scoped-Define ENABLED-OBJECTS BRW_UT BTN_EXCEL BTN_SKRIV BTN_AVS 
...
DEFINE FRAME FRAME-TIDS
     ...
     FILL-IN-REGIS AT ROW 14.25 COL 5 COLON-ALIGNED NO-LABEL
     FILL-IN_FORNAMN AT ROW 14.25 COL 25 COLON-ALIGNED NO-LABEL
     FILL-IN_NAMN AT ROW 14.33 COL 25.13 COLON-ALIGNED HELP
          "ANGE ORAGNISTIONENSBENÄMNING" NO-LABEL
     RAD_ALLVAL AT ROW 15.13 COL 2.38 NO-LABEL
     BTN_EXCEL AT ROW 28 COL 48 WIDGET-ID 2
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ...
   CREATE namntemp.
   IF manad = 160 OR MANAD = 161 THEN RUN tolk2_UI.   
   ELSE RUN tolk_UI.         
   BRW_UT:HIDDEN = FALSE.
   RUN enable_UI.  
   WINDOW-2:TITLE = listtext.
   {FRMSIZE.I}        
   ASSIGN
...
------------------------------------------------------------------------------*/
  ENABLE BRW_UT BTN_EXCEL BTN_SKRIV BTN_AVS 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE tolk2_UI WINDOW-2 
PROCEDURE tolk2_UI :
/* -----------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
-------------------------------------------------------------*/
   IF tomdat = ? THEN tomdat = TODAY.
   IF Guru.Konstanter:appcon THEN DO:
      RUN FLAKOMPAP.P ON Guru.Konstanter:apphand TRANSACTION DISTINCT
      (INPUT RAD_ALLVAL,INPUT manad,INPUT valaonr,INPUT tomdat,
      INPUT listtext,INPUT TABLE tidpers,INPUT-OUTPUT TABLE namntemp,
      OUTPUT TABLE tidut).
   END.
   ELSE DO:
      RUN FLAKOMPAP.P 
      (INPUT RAD_ALLVAL,INPUT manad,INPUT valaonr,INPUT tomdat,
      INPUT listtext,INPUT TABLE tidpers,INPUT-OUTPUT TABLE namntemp,OUTPUT TABLE tidut).
   END.
...
-------------------------------------------------------------*/   
   IF Guru.Konstanter:appcon THEN DO:
      RUN FLKOMPAP.P ON Guru.Konstanter:apphand TRANSACTION DISTINCT
      (INPUT RAD_ALLVAL, INPUT datar, INPUT manad,INPUT valaonr,INPUT tomdat,
      INPUT listtext,INPUT TABLE tidpers,INPUT-OUTPUT TABLE namntemp,
      OUTPUT TABLE tidut).
   END.
   ELSE DO:
