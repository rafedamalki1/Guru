...
ELSE IF FLEXAVT.FLEXTID = TRUE THEN DO:   
   ...
   NO-LOCK NO-ERROR.   
   DO TRANSACTION:
      /*Ta bort flexdagar som ligger tidigare än föregående körning*/
      FOR EACH FLEXDAG WHERE FLEXDAG.PERSONALKOD = PERSONALTAB.PERSONALKOD AND FLEXDAG.FELMED = "Ingen registrering gjord"
      AND FLEXDAG.DATUM LE FLEXREG.SALDOKORD USE-INDEX FLEX EXCLUSIVE-LOCK:
         DELETE FLEXDAG.
      END.
   END.
   IF TIME > 68400 THEN ASSIGN hjdatum = TODAY.
   ELSE hjdatum = TODAY - 1.
   OPEN QUERY flgam FOR EACH FLEXDAG WHERE FLEXDAG.PERSONALKOD = PERSONALTAB.PERSONALKOD AND
   FLEXDAG.KONTROLL = "Ejkontroll" AND FLEXDAG.DATUM LE hjdatum USE-INDEX FLEX NO-LOCK.     /*KONTROLL UPPLAGDA FLEXDAGAR*/    
   DO TRANSACTION:
...
PROCEDURE sub_UI:
      ...
      IF NOT AVAILABLE flexbuff2 THEN DO TRANSACTION:
         IF regdatum GE TODAY THEN.
         ELSE IF regdatum = TODAY AND TIME < 68400 THEN.
         ELSE DO:         
      ...
      /*   måste kolla framåt när man lägger in semester i förväg och vill färdigrapportera*/
      IF ejflextid = TRUE THEN regdatum = regdatum.
      ELSE DO TRANSACTION:
         /*ELSE DO:*/
         RUN REGVEC.P.
...
PROCEDURE andtid_UI :
   ASSIGN
   TIDREGITAB.PROGRAM = 'GAMKOLL.P' + STRING(TODAY) + STRING(TIME,"HH:MM")  
