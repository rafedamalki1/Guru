...
Modified: 
  98/06/23 mcmann   - Changed logical name check for spaces 98-06-23-010
...
form
   ...
   "Other CONNECT Statement Parameters:"              at      4   SKIP
   p_Unix_Parms    NO-LABEL  {&STDPH_EDITOR}    at      4
...
/*    input frame connect p_Type    */
      ...
      input frame connect p_Unix_Parms.
...
/*-------------------------------------------------------------------
   db-type no longer update-able  <hutegger> 94/05
&IF "{&WINDOW-SYSTEM}" <> "TTY" &THEN
   /* Set up the combo box of gateway types with only those types that 
      this executable supports.
   */
   do ix = 1 to 10:
      if CAN-DO(GATEWAYS, gates[ix]) then
         stat = lst_Db_Types:add-last(gates[ix]) in frame connect.
   end.
   lst_Db_Types:inner-lines in frame connect = lst_Db_Types:num-items.
    
   {adecomm/cbdrop.i &Frame  = "frame connect"
                     &CBFill = "p_Type"
                     &CBList = "lst_Db_Types"
                     &CBBtn  = "btn_DB_Types"
                     &CBInit = "p_Type"}
...
p_Unix_Parms:return-inserted = yes.
...
display 
   ...
   p_Multi_User
   p_UserId
/* p_Password     gets displayed as blank, so can be ommitted <hutegger> */
   p_Trig_Loc
   p_Parm_File
   p_Unix_Parms
   with frame connect.              
