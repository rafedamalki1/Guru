...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-LABELS NO-COLUMN-SCROLLING SIZE 76.5 BY 15.25.
...
DEFINE FRAME DIALOG-1
     ...
     BTN_AVS AT ROW 17.25 COL 64
     SPACE(0.99) SKIP(0.07)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Fellista frånvaro".
...
-------------------------------------------------------------*/
   /*HUVUD*/ 
   {AMERICANEUROPEAN.I}    
   IF Guru.Konstanter:globforetag = "NORD" OR Guru.Konstanter:globforetag = "ETA" OR Guru.Konstanter:globforetag = "ESAN" OR Guru.Konstanter:globforetag = "ESMA" THEN DO:
      IF NOT Guru.Konstanter:appcon THEN DO:      
         RUN appfel_UI.
         RETURN.
      END.            
      RUN SELONIN.P ON Guru.Konstanter:apphand TRANSACTION DISTINCT (INPUT "frfelma.d", OUTPUT TABLE tidut).      
      RETURN.
   END.          
   ...
   ELSE IF Guru.Konstanter:globforetag = "GRAN" THEN DO:
      INPUT FROM \\GRANGURU\guru_ser\server\PRO9S\gran\frfelma.d NO-ECHO.
   END.  
   ELSE IF Guru.Konstanter:globforetag = "GADM" THEN DO:
      INPUT FROM \\GRANGURU\guru_ser\server\PRO9S\gadm\frfelma.d NO-ECHO.
   END.      
   ELSE IF Guru.Konstanter:globforetag = "GSYD" THEN DO:
      INPUT FROM \\GRANGURU\guru_ser\server\PRO9S\gsyd\frfelma.d NO-ECHO.
   END.    
   ELSE IF Guru.Konstanter:globforetag = "GRIT" THEN DO:
      INPUT FROM \\GRANGURU\guru_ser\server\PRO9S\grit\frfelma.d NO-ECHO.
   END.    
   ELSE IF Guru.Konstanter:globforetag = "GKAL" THEN DO:
      INPUT FROM \\goliat\DELAD\server\PRO9S\gkal\frfelma.d NO-ECHO.
   END.    
   ELSE IF Guru.Konstanter:globforetag = "GSOL" THEN DO:
      INPUT FROM \\GRANGURU\guru_ser\server\PRO9S\gsol\frfelma.d NO-ECHO.
   END. 
   ELSE IF Guru.Konstanter:globforetag = "GKRVA" THEN DO:
      INPUT FROM \\GRANGURU\guru_ser\server\PRO9S\gkrva\frfelma.d NO-ECHO.
   END.      
    
   ELSE DO:
      INPUT FROM C:\GURU\frfelma.d NO-ECHO.
   END.     
   REPEAT TRANSACTION:
      CREATE tidut.
      ASSIGN.
