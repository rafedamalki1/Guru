...
DAGS FÖR SUMMERING:        
ekforst OCH ÖVRIGA TABELER SUMMERAS TILL  slutsum PER PERSON AONR ÅR VECKOKÖRD ELLER
EJ VECKOKÖRD OCH PRISTYP.
...
ALLTSÅ KAN DET FINNAS TRE SUMTID 
EN MED TIMMAR FÖR LÖNETILLÄGSRESTID
EN MED TIMMAR FÖR ÖVERTIDSTILLÄGG
EN MED KOSTNADER FÖR ÖVERTIDSTILLÄGG 
FÖR DE EJ VECKOKÖRDA.
FÖR DE VECKOKÖRDA SUMMERAS ÖVERTIDSKOSTNADERNA PER PERSON AONR ÅR PRISTYP
RESTIDSTIMMARNA SUMMERAS PER PERSON AONR ÅR PRISTYP
sumtill_UI SUMMERAR LÖNETILLÄGGSKOSTNDERNA.         
VARJE tilltab SÖKER UPP EN SUMTID MED SAMMA PERSON AONR ÅR VECKOKÖRD ELLER
EJ VECKOKÖRD OCH PRISTYP.
OCH SUMMERAR IN KOSTNADERNA.
ALLTSÅ KAN LÖNETILÄGGSKOSTNDER FÖR RESTID HAMNA PÅ NÅGON AV DE OVAN NÄMDA SUMTID
MEN ALLA VECKOKÖRDA POSTER SUMERAS IHOP OCH DET BÖR BARA BLI EN MED 'RESTID...'
TIMMAR ÖVERTIDSKOSTNAD OCH LÖNEKOSTNAD. 
*/
...
OUTPUT TO PRINTER.
FOR EACH XSUMTID USE-INDEX PERSONALKOD:
   DISPLAY XSUMTID.OMRADE PERSONALKOD FORNAMN EFTERNAMN TIMMAR BELOPP OTIMMAR
   OBELOPP TBELOPP LONKOST PRISTYP.
END.        
OUTPUT CLOSE.
...
PROCEDURE sumtid_UI:            
   ...
   DO WHILE AVAILABLE(TIDREGITAB):                      
      ...
      THEN DO TRANSACTION: 
         ...
         /*nattpris = TIDREGITAB.PRIS. sätts i nattpris */
         RUN TIMSEK.P.
   ...
   END.                
      FOR EACH slutsum:                 
         FIND FIRST PERSONALTAB WHERE PERSONALTAB.PERSONALKOD = slutsum.PERSONALKOD
         USE-INDEX PERSONALKOD NO-LOCK.
         CREATE XSUMTID.
         ASSIGN               
         XSUMTID.FORNAMN = PERSONALTAB.FORNAMN
         XSUMTID.EFTERNAMN = PERSONALTAB.EFTERNAMN
      ...
      END.
      FOR EACH restid:     
         FIND FIRST PERSONALTAB WHERE PERSONALTAB.PERSONALKOD = restid.PERSONALKOD
         USE-INDEX PERSONALKOD NO-LOCK.
         CREATE XSUMTID.
         ASSIGN               
         XSUMTID.FORNAMN = PERSONALTAB.FORNAMN
   ...
   FOR EACH tilltab USE-INDEX PERSONALKOD NO-LOCK:
      DO TRANSACTION:
         ...
         IF NOT AVAILABLE XSUMTID THEN DO:
            FIND FIRST PERSONALTAB WHERE PERSONALTAB.PERSONALKOD = tilltab.PERSONALKOD
            USE-INDEX PERSONALKOD NO-LOCK.
            CREATE XSUMTID.      
            ASSIGN
            XSUMTID.FORNAMN = PERSONALTAB.FORNAMN
