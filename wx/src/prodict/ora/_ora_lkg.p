...
for each DICTDBG.oracle_users 
  ...
  no-lock, 
  each DICTDBG.oracle_objects fields (owner# type name linkname obj#)
    where DICTDBG.oracle_users.user#      = DICTDBG.oracle_objects.owner#
/*  and   DICTDBG.oracle_objects.linkname = ? */
    no-lock
   ...
   */
     ...
     */
    if ENTRY(DICTDBG.oracle_objects.type + 1, oobjects) = "SYNONYM"
     then do:  /* synonym */
    ...
    else if ENTRY(DICTDBG.oracle_objects.type + 1, oobjects) = "SEQUENCE"
     then do:  /* sequence */
      find first DICTDB._Sequence
        where DICTDB._Sequence._Db-recid    = drec_db
        and   DICTDB._Sequence._Seq-misc[1] = DICTDBG.oracle_objects.name
    ...
    if   DICTDBG.oracle_objects.type   =    4
     ...
     then assign
       gate-work.gate-name = SUBSTR(DICTDBG.oracle_objects.name, 8, -1 ,"character")
  ...
  end. /* for each DICTDBG.oracle_users, each DICTDBG.oracle_objects */
