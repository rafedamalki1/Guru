...
$Archive:: /www-helpdesk/webspeed/wrk_dir/pro/pdftoo $
$Author:: Therese                                    $
$Date:: 05-03-04 10:37                               $
$Revision:: 5                                        $
...
*****************  Version 5  *****************
User: Therese      Date: 05-03-04   Time: 10:37
...
PROCEDURE BuildTable:
  ...
  h_DataQuery:ADD-BUFFER(h_DataBuffer:NAME).
...
PROCEDURE DisplayTableHeader:
  ...
  RUN pdf_set_tool_parameter IN pCallProc
      (pdfStream,pdfToolName,"MaxY",0,STRING(pdf_textY(pdfStream))).
...
PROCEDURE DisplayTableDetail:
  ...
  DO i_Field = 1 TO h_DataBuffer:NUM-FIELDS:
    ...
    RUN pdf_text_xy IN pCallProc
        (pdfStream, SUBSTR(h_DataField:BUFFER-VALUE(),1,i_ColWidth, "character":U),
                    pdf_get_tool_parameter (pdfStream,
                                            pdfToolname,
                                            "ColumnX",
                                            i_Field),
    ...
    IF i_Row = 5 THEN DO:
      ...
      IF c_DayHighlight <> "" THEN
        RUN DayHighLight (pdfStream,
                          ...
                          pdfYear,
                          i_X, 
                          pdfY + pdfHeight - i_DayLabelHeight - i_HeaderHeight - ((i_Row - 1) * d_DistY) - (d_DistY) + d_DayFontSize,
