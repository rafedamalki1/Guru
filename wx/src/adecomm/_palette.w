...
DEFINE VARIABLE palette-size      AS INT     NO-UNDO INITIAL 44.
...
DO:
  DEFINE VARIABLE rc   AS INTEGER INITIAL 0.
  DEFINE VARIABLE mode AS LOGICAL INITIAL no.

  ASSIGN toponly = SELF:CHECKED.
  IF toponly THEN mode = yes.
  ELSE mode = no. 
  RUN adecomm/_topmost.p (INPUT hWin:hWnd, INPUT mode, OUTPUT rc).
END.
...
-------------------------------------------------------------*/
  ...
  DO ON STOP UNDO, LEAVE:
    IF pName THEN
        RUN VALUE(pcFile) PERSISTENT (INPUT fName).
    ELSE
        RUN VALUE(pcFile) PERSISTENT.
  END.
END PROCEDURE.
