...
DEFINE VARIABLE RAD_MKTID AS INTEGER 
     ...
     RADIO-BUTTONS 
          "Endags- eller flerdygnsförättning", 1,
"Ändra tjänsteresa", 3,
"Utlandsresa", 4,
"Radera tjänsteresa", 2
...
DEFINE FRAME DIALOG-1
     ...
     "Tjänsteresan skall gälla:" VIEW-AS TEXT
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ...
   IF Guru.Konstanter:tidtsekvar[1] = FALSE THEN DO: 
      status-ok = RAD_MKTID:DELETE("Endags- eller flerdygnsförättning"). 
   END.
   IF Guru.Konstanter:tidtsekvar[2] = FALSE  THEN DO:
      status-ok = RAD_MKTID:DELETE("Radera tjänsteresa"). 
   END.
   IF Guru.Konstanter:tidtsekvar[5] = FALSE THEN DO:
      status-ok = RAD_MKTID:DELETE("Ändra tjänsteresa"). 
   END.
   ELSE DO:
      /*ingen ska ha ändra tjänsteresa*/
      status-ok = RAD_MKTID:DELETE("Ändra tjänsteresa"). 
   END.
