...
Define var trig_event      as char    format "x(8)"                 
					   view-as combo-box sort      NO-UNDO.
Define var trig_proc       as char    format  "x(32)"
...
FORM
   ...
   "Procedure:"   at 17 view-as TEXT
   "Options:"     at 55 view-as TEXT
   SKIP({&VM_WID})

   trig_event     at  2 NO-LABEL {&STDPH_FILL} SPACE(0)
   trig_proc      at 17 NO-LABEL {&STDPH_FILL} SPACE(.3)
   btn_File                     
   trig_crc       at 55 LABEL "&Check CRC" 
   trig_override        LABEL "Overrida&ble"
   SKIP ({&VM_WID})
   ...
   with frame trigedit 
      ...
      NO-LABELS SCROLLABLE VIEW-AS DIALOG-BOX.
...
Define var read       as logical NO-UNDO.  /* i.e., past tense of read */
   ...
   do:
      find b_ttrig where b_ttrig._File-Number = b_file._File-number
            AND b_ttrig._Event = trig_event NO-ERROR.
