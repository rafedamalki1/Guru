/*
   ...
   Parameters:
     ...
     Modified: 2005.10.04 13:40 ELPAO    
     Modified: 2005.12.20 14:17 ELPAO    
     Modified: 
*/
{ODBCTEMP.I}
DEFINE TEMP-TABLE ktemp NO-UNDO
   ...
   FIELD RINGT       AS CHARACTER                      
   FIELD RINGN       AS CHARACTER                      
   FIELD SENASTFAX   AS CHARACTER                      
   FIELD ANM          AS CHARACTER                     
   FIELD MINPRISBO    AS CHARACTER                     
   FIELD BPRIS AS CHARACTER                            
   FIELD MINPRISVI AS CHARACTER                        
   FIELD VPRIS     AS CHARACTER                        
   FIELD RUM       AS CHARACTER                        
   FIELD RUM2      AS CHARACTER                        
   FIELD MINBOBO AS CHARACTER                          
   FIELD MAXBOBO AS CHARACTER                          
   FIELD BOAREA AS CHARACTER                           
   FIELD MAXBOVI AS CHARACTER                          
...
DEFINE TEMP-TABLE otemp NO-UNDO
   FIELD PERSONALKOD AS CHARACTER
   FIELD BOTYP       AS CHARACTER
   FIELD OMRADE      AS CHARACTER
   FIELD SQLID       AS INTEGER
  INDEX PKOD IS PRIMARY PERSONALKOD.
DEFINE TEMP-TABLE pkodtemp NO-UNDO
   FIELD PERSONALKOD AS CHARACTER
   INDEX PKOD IS PRIMARY PERSONALKOD.

DEFINE INPUT PARAMETER TABLE FOR vadgora.
DEFINE INPUT  PARAMETER objdat AS DATE       NO-UNDO.
DEFINE VARIABLE prishj1 AS INTEGER    NO-UNDO.
DEFINE VARIABLE prishj2 AS INTEGER    NO-UNDO.
DEFINE VARIABLE kommando AS CHARACTER  NO-UNDO.
DEFINE VARIABLE odbch AS HANDLE     NO-UNDO.
DEFINE VARIABLE prishog AS DECIMAL    NO-UNDO.
DEFINE VARIABLE prislag AS DECIMAL    NO-UNDO.
DEFINE VARIABLE pkod AS CHARACTER  NO-UNDO.
FIND FIRST vadgora NO-ERROR.
{HUSOBJTEMP.I}
DEFINE TEMP-TABLE feltemp NO-UNDO
   FIELD feltext AS CHARACTER.
DEFINE TEMP-TABLE resultatobj NO-UNDO
   FIELD OMRADE AS  CHARACTER
   FIELD BOTYP  AS  CHARACTER
   FIELD RUM    AS DECIMAL
   FIELD ANTAL  AS INTEGER
   FIELD MINPRIS AS INTEGER
   FIELD MAXPRIS AS INTEGER
   FIELD MANAVG AS INTEGER
   INDEX OMR OMRADE BOTYP RUM
   INDEX MINPRIS MINPRIS
   INDEX MAXPRIS MAXPRIS.

DEFINE VARIABLE uttext AS CHARACTER  FORMAT "x(15)" NO-UNDO.
RUN startodbc_UI.
OUTPUT TO d:\delad\pro9S\autotid.txt  APPEND.
PUT "bg-sql "  TODAY " " STRING(TIME,"HH:MM:SS") SKIP.

FOR EACH feltemp :
   PUT feltemp.feltext SKIP.       
END.
OUTPUT CLOSE.
FIND FIRST feltemp NO-LOCK NO-ERROR.
IF AVAILABLE feltemp THEN RETURN.
