...
DEFINE VARIABLE thread AS CHARACTER EXTENT 5 NO-UNDO INITIAL [
...
DISPLAY
  ...
  "---------------------------" NO-ATTR-SPACE SKIP
  " A. Apply Changes "         @ msg[4] HELP "Apply Changes to Database" SKIP
  " U. Undo Changes "          @ msg[5] HELP "Undo Recent Changes" SKIP
  WITH COLOR DISPLAY VALUE(menu-fg) NO-LABELS
  ATTR-SPACE ROW 3 CENTERED WITH FRAME mod-scheme.

FORM
  c FORMAT "x(63)" SKIP
  WITH FRAME instr j DOWN ROW 11 CENTERED NO-LABELS NO-ATTR-SPACE.
...
ON CURSOR-DOWN    TAB.
NEXT-PROMPT msg[4] WITH FRAME mod-scheme.

answer = FALSE.
DO WHILE NOT answer:
  DO ON ERROR UNDO,LEAVE ON ENDKEY UNDO,LEAVE:
    CHOOSE FIELD msg[1 FOR 5] COLOR VALUE(menu-bg)
      AUTO-RETURN GO-ON(".") WITH FRAME mod-scheme.
  END.
  ASSIGN
    j         = (IF CHR(LASTKEY) = "." OR KEYFUNCTION(LASTKEY) = "END-ERROR"
                THEN 5 ELSE FRAME-INDEX)
    c         = SUBSTRING(INPUT FRAME mod-scheme msg[j],5)
    user_path = thread[j].
  IF user_path = "*R" THEN
