...
&Scoped-define FIELDS-IN-QUERY-BROWSE-19 KALKTYP.RUBRIK KALKTYP.TYP 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-19 
&Scoped-define QUERY-STRING-BROWSE-19 FOR EACH KALKTYP NO-LOCK
&Scoped-define OPEN-QUERY-BROWSE-19 OPEN QUERY BROWSE-19 FOR EACH KALKTYP NO-LOCK.
&Scoped-define TABLES-IN-QUERY-BROWSE-19 KALKTYP
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-19 KALKTYP
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-19 WINDOW-1 _STRUCTURED
  QUERY BROWSE-19 NO-LOCK DISPLAY
      KALKTYP.RUBRIK FORMAT "X(25)":U
      KALKTYP.TYP FORMAT "X(8)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-COLUMN-SCROLLING SIZE 52.5 BY 8.5.
...
DEFINE FRAME FRAME-A
     ...
     BTN_AVSL AT ROW 15.25 COL 54
     "KALKTYP" VIEW-AS TEXT
...
/* Query rebuild information for BROWSE BROWSE-19
     ...
     _Options          = "NO-LOCK"
     _FldNameList[1]   = RT9.KALKTYP.RUBRIK
     _FldNameList[2]   = RT9.KALKTYP.TYP
     _Query            is OPENED
*/  /* BROWSE BROWSE-19 */
&ANALYZE-RESUME
...
-------------------------------------------------------------*/
   {muswait.i}
   RUN KTYPNY.W (INPUT RECID(KALKTYP)). 
   {musarrow.i}
   IF musz = FALSE THEN DO: 
      OPEN QUERY {&BROWSE-NAME} FOR EACH KALKTYP NO-LOCK.
      FIND FIRST KALKTYP WHERE RECID(KALKTYP) = brec NO-LOCK NO-ERROR. 
      IF AVAILABLE KALKTYP THEN DO:
...
/* -----------------------------------------------------------
  ...
  Notes:       
-------------------------------------------------------------*/
   MESSAGE "VILL DU VERKLIGEN TA BORT DENNA KALKTYP?"
   VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO 
   UPDATE answer AS LOGICAL.
   IF answer THEN DO TRANSACTION:
      {muswait.i}
      FIND KALKTYP WHERE RECID(KALKTYP) = brec NO-LOCK NO-ERROR.
      FIND NEXT KALKTYP NO-LOCK NO-ERROR.
      IF AVAILABLE KALKTYP THEN borec = RECID(KALKTYP).
      ELSE DO:
         FIND KALKTYP WHERE RECID(KALKTYP) = brec NO-LOCK NO-ERROR.
         FIND PREV KALKTYP NO-LOCK NO-ERROR.
         IF AVAILABLE KALKTYP THEN borec = RECID(KALKTYP).
      END.
      FIND KALKTYP WHERE RECID(KALKTYP) = brec EXCLUSIVE-LOCK NO-ERROR.
