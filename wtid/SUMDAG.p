...
DEFINE VARIABLE nollkor AS LOGICAL NO-UNDO.
...
IF Guru.Konstanter:globforetag = "GKAL" THEN DO:   
    prognamnpers = "d:\DELAD\server\PRO9S\autotidpers.txt".                   
    prognamnque = "d:\DELAD\server\PRO9S\autotid.txt".                   
END.


ELSE IF Guru.Konstanter:globforetag = "sund" THEN DO:
   prognamnpers = "D:\delad\server\pro10S\autotidpers.txt".                   
   prognamnque = "D:\delad\server\pro10S\autotid.txt".
END.
ELSE IF Guru.Konstanter:globforetag = "SNAT" THEN DO:
   prognamnpers = "D:\delad\server\pro10S\autotidpersSNAT.txt".                   
   prognamnque = "D:\delad\server\pro10S\autotidSNAT.txt".
   /*SNATBERGET*/
   prognamnpers = REPLACE(prognamnpers,"D:\delad\server\pro10S\","D:\DELAD\PRO10S\").
   prognamnque = REPLACE(prognamnque,"D:\delad\server\pro10S\","D:\DELAD\PRO10S\").
       
END.
ELSE IF Guru.Konstanter:globforetag = "MISV" THEN DO:
   prognamnpers = "C:\elpool\delad\pro10s\autotidpers.txt".      
   prognamnpers = "D:\elpool\delad\pro10s\autotidpers.txt".            
   prognamnque = "C:\elpool\delad\pro10s\autotid.txt".
   /*Guru.Konstanter:AppSpringSet[1] = "misvstb"*/
   prognamnque = "D:\elpool\delad\pro10s\autotid.txt".
END.
...
ELSE IF Guru.Konstanter:globforetag = "LULE" THEN DO:
   prognamnpers = "D:\elpool\delad\PRO9S\autotidpers.txt".                   
...
IF nollkor = TRUE THEN DO:
   ...
   CLOSE QUERY sumq.
END.           
OPEN QUERY sumq FOR EACH SUMTIDDAG WHERE SUMTIDDAG.VECKOKORD = " " NO-LOCK.
DO TRANSACTION:
   GET FIRST sumq EXCLUSIVE-LOCK.
   IF AVAILABLE SUMTIDDAG THEN DELETE SUMTIDDAG.      
END.                                             
...
CLOSE QUERY sumq.        
/*SUMERING/ÅR*/
OPEN QUERY sumarq FOR EACH SUMTID WHERE SUMTID.VECKOKORD = " " NO-LOCK.
DO TRANSACTION:
...
END.       
CLOSE QUERY sumarq.  
/*nollkor ej perskoll_UI stoppar t.ex. flextid tidregitabbar*/
IF nollkor = TRUE THEN.
ELSE RUN perskoll_UI.
OUTPUT TO VALUE(prognamnque) APPEND.
PUT "DELETE EJ KÖRDA KLAR " TODAY " "  Guru.Konstanter:globforetag " " STRING(TIME,"HH:MM:SS") SKIP.
OUTPUT CLOSE.

FIND FIRST VECKONATT WHERE VECKONATT.DAG_AR = "START" NO-LOCK NO-ERROR.
IF VECKONATT.NYKORD = TRUE THEN DO:
   /*händer VID URSPORNING*/
   RUN rattfel_UI.
   OUTPUT TO VALUE(prognamnque) APPEND.
...
OUTPUT TO VALUE(prognamnque) APPEND.
PUT "SUMTIDDAG EJ KÖRDA KLAR" TODAY " " Guru.Konstanter:globforetag " " STRING(TIME,"HH:MM:SS") SKIP.
OUTPUT CLOSE.
...
OUTPUT TO VALUE(prognamnque) APPEND.
PUT "EJ KÖRDA KLAR " TODAY " " Guru.Konstanter:globforetag " " STRING(TIME,"HH:MM:SS") SKIP.
OUTPUT CLOSE.
/*nollkor EJ veckakord_UI
*/
IF nollkor = TRUE THEN.
ELSE RUN veckakord_UI.
OUTPUT TO VALUE(prognamnque) APPEND.
PUT "SUMTIDDAG  KÖRDA KLAR" TODAY " " Guru.Konstanter:globforetag " " STRING(TIME,"HH:MM:SS") SKIP.
OUTPUT CLOSE.
pkod = " ".
...
PROCEDURE pers_UI:
   ...
   IF AVAILABLE PERSONALTAB THEN DO:
      ...
      {SOKANROP.I}
      ASSIGN
      persmaskvar = PERSONALTAB.PERSMASK
...
PROCEDURE perskoll_UI:
   IF Guru.Konstanter:globforetag = "SUND" OR Guru.Konstanter:globforetag = "SNAT" OR Guru.Konstanter:globforetag = "MISV" THEN musz = musz.
   ELSE RETURN.
...
PROCEDURE sumtid_UI:    
   ...
   ELSE DO: 
      ...
      ELSE DO:
         IF Guru.Konstanter:globforetag = "lule" THEN DO:
            OPEN QUERY tidq FOR EACH TIDREGITAB WHERE 
            TIDREGITAB.VECKOKORD begins kollvecka AND    
            TIDREGITAB.TIDLOG = TRUE NO-LOCK BY TIDREGITAB.VECKOKORD BY
            TIDREGITAB.AONR BY TIDREGITAB.DELNR BY TIDREGITAB.PERSONALKOD.
         END.
         ELSE IF Guru.Konstanter:globforetag = "MISV" THEN DO:
   ...
   DO WHILE AVAILABLE(TIDREGITAB):
      ...
      /*ÄVEN ANDRA */               
      /*FAKTFORE*/
      IF Guru.Konstanter:globforetag = "GRAN" OR Guru.Konstanter:globforetag = "sund" OR Guru.Konstanter:globforetag = "SNAT" OR Guru.Konstanter:globforetag = "MISV" OR Guru.Konstanter:globforetag = "elpa" THEN DO:
         IF persmaskvar = FALSE THEN .
         ELSE DO:
...
PROCEDURE traktsumfel_UI.
   IF nollkor = TRUE THEN RETURN.
/*SUMTRAKT ENDAST FÖR FAKTURERING alla traktamenten*/   
   IF kollvecka = " " THEN RETURN.
   FIND FIRST TRAKTFLER WHERE TRAKTFLER.TRAAVTAL = traav AND 
   TRAKTFLER.TRAKTKOD = TIDFEL.TRAKTKOD  
   USE-INDEX FLTRAKT NO-LOCK NO-ERROR.                      
   CREATE SUMTRAKT.
   ASSIGN
   SUMTRAKT.PERSONALKOD = TIDFEL.PERSONALKOD      
   SUMTRAKT.AONR = TIDFEL.AONR
   SUMTRAKT.DELNR = TIDFEL.DELNR
   SUMTRAKT.DATUM = TIDFEL.DATUM
   SUMTRAKT.VECKOKORD = TIDFEL.VECKOKORD
   SUMTRAKT.TRAKTKOD = TIDFEL.TRAKTKOD
   SUMTRAKT.TRAKTANTAL = TIDFEL.TRAKTANTAL * debkredvar.

   IF AVAILABLE TRAKTFLER THEN SUMTRAKT.ENDAGS = FALSE.
   ELSE SUMTRAKT.ENDAGS = TRUE.    
END PROCEDURE.     

PROCEDURE lonsumfel_UI.
   IF nollkor = TRUE THEN RETURN.   
   /*SUMLON ENDAST FÖR FAKTURERING 
    endast de lönetillägg som skall till fakturering*/
   IF kollvecka = " " THEN RETURN.    
   ...
   CREATE SUMLON.
   ASSIGN
   SUMLON.PERSONALKOD = TIDFEL.PERSONALKOD      
   SUMLON.AONR = TIDFEL.AONR
   SUMLON.DELNR = TIDFEL.DELNR
   SUMLON.DATUM = TIDFEL.DATUM
   SUMLON.VECKOKORD = TIDFEL.VECKOKORD
   SUMLON.LONTILLAGG = TIDFEL.LONTILLAGG
   SUMLON.LONTILLANTAL = TIDFEL.LONTILLANTAL * debkredvar.   
END PROCEDURE.


PROCEDURE sumtidfel_UI:      
   /*endast för veckokorda*/
    /*nollkor*/
   IF nollkor = TRUE THEN DO: 
   ...
   END.
   ELSE DO:
      OPEN QUERY feltidq FOR EACH TIDFEL WHERE TIDFEL.FELKORD = kollvecka AND
      TIDFEL.TIDLOG = TRUE 
      NO-LOCK BY TIDFEL.FELKORD BY
      TIDFEL.AONR BY TIDFEL.DELNR BY TIDFEL.PERSONALKOD BY TIDFEL.DATUM.
   END.      
   ...
   DO WHILE AVAILABLE(TIDFEL):         
      ...
      END.                      
      /*ÄVEN ANDRA */               
      IF TIDFEL.TRAKTKOD NE "" THEN RUN traktsumfel_UI.
      IF TIDFEL.LONTILLAGG NE " " THEN RUN lonsumfel_UI.     
...
PROCEDURE sumtillfel_UI: 
   ...
   END.
   ELSE DO:
      OPEN QUERY tidfelq FOR EACH TIDFEL WHERE TIDFEL.FELKORD = kollvecka AND 
      TIDFEL.TIDLOG = FALSE NO-LOCK 
      BY TIDFEL.FELKORD BY  TIDFEL.AONR BY TIDFEL.DELNR BY TIDFEL.PERSONALKOD
      BY TIDFEL.DATUM.
   END.   
...
PROCEDURE sumeraallt_UI:
   ...
   END.
   OUTPUT TO VALUE(prognamnpers).
   PUT "summeraalt trakt2 ok".
   OUTPUT CLOSE.         
   DEFINE VARIABLE tran AS INTEGER NO-UNDO.
  
   REPEAT TRANSACTION :
      FOR EACH slutsum tran = 1 to 500 :
         CREATE SUMTIDDAG.
         ASSIGN     
         ...
         SUMTIDDAG.IKOSTNAD = slutsum.BELOPP. 
   		DELETE slutsum.
   	END.
   	IF NOT CAN-FIND ( FIRST slutsum ) THEN LEAVE.
   END.
   OUTPUT TO VALUE(prognamnpers).
   PUT "summeraalt tid ok".
   OUTPUT CLOSE.
