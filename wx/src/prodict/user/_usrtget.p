...
DEFINE VARIABLE point_flag AS LOGICAL   EXTENT  1024 NO-UNDO.
...
IF cache_dirty THEN RUN "prodict/_dctcach.p" (FALSE).
...
IF cache_file# = 0 THEN DO:
  FIND FIRST _File WHERE _Db-recid = drec_db AND _Hidden NO-ERROR.
...
IF p_some AND p_all OR (p_some AND p_allw) AND cache_file[1] <> "ALL" AND cache_file# >= 1 THEN DO:
...
ELSE IF p_some THEN 
  ...
  new_lang[11] @ h2 
  WITH FRAME t-help.
ELSE DISPLAY (IF p_all AND p_allw THEN new_lang[1] ELSE "") @ c WITH FRAME t-type.                                    /* ^-- (or "ALL" for all files) */
IF NOT p_option THEN DO:
  ...
  IF NOT p_option THEN _pick: DO WHILE TRUE:
    ...
    IF CAN-DO("GO" + (IF p_some THEN "" ELSE ",RETURN"),KEYFUNCTION(LASTKEY))
      ...
      ASSIGN 
        user_filename = "".
        DO p_recid = (IF p_all OR p_allw THEN 2 ELSE 1) TO cache_file#:
      ...
      IF cache_file[p_recid] BEGINS p_typed AND
        ...
        THEN do: user_filename = cache_file[p_recid].
        end.
      ELSE
      IF p_typed <> cache_file[p_recid] THEN DO:
        FIND _File
          WHERE _File._Db-recid = drec_db
          AND _File._File-name BEGINS p_typed NO-ERROR.
    ...
    IF CAN-DO("GET,PUT",KEYFUNCTION(LASTKEY)) AND p_some THEN DO:
      ...
      IF redraw AND lb = 0 AND l THEN DO:
        lb = cache_file#.
        FOR EACH _File
          WHERE _File._Db-recid = drec_db AND _File._Hidden
...
END. /* if optional */
 ...
 FIND FIRST _File WHERE _File._Db-recid = drec_db
  AND _File._File-name = SUBSTRING(user_filename,1,33) NO-ERROR.
...
IF user_filename MATCHES "*,*" THEN DO:
  IF p_all OR p_allw AND NOT p_some OR (count = cache_file# - 1)
...
ELSE 
IF user_filename <> "" THEN DO:
  IF count >= 1 AND (p_all OR p_allw) AND (count = cache_file# - 1)
    THEN DO:
      user_env[1] = "ALL".
      user_filename = "ALL".
    END.
