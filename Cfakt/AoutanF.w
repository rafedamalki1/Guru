...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-LABELS NO-ROW-MARKERS NO-COLUMN-SCROLLING SIZE 123.5 BY 26.25.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW WINDOW-2 ASSIGN
         HIDDEN             = YES
         TITLE              = "Aonr utan betalningsplan"
         HEIGHT             = 28.42
         WIDTH              = 125
         MAX-HEIGHT         = 28.42
         MAX-WIDTH          = 125
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ...
   IF lopfast = FALSE THEN DO:              
     WINDOW-2:TITLE = Guru.Konstanter:gaok + " utan fakturor".
   END.              
   ELSE DO:
      WINDOW-2:TITLE = Guru.Konstanter:gaol + " med fakturor".
   END.   
   IF valvar > 3 THEN RETURN.  
   IF Guru.Konstanter:appcon THEN DO:                           
      RUN AOUTANA.P ON Guru.Konstanter:apphand TRANSACTION DISTINCT 
      (INPUT valvar, INPUT valomr,INPUT lopfast,INPUT valfak,INPUT Guru.Konstanter:globanv,INPUT seallt,
       OUTPUT TABLE tidut).
   END.
   ELSE DO:
      RUN AOUTANA.P 
