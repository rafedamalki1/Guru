...
VIEW-AS COMBO-BOX
 ...
 DEFINE TEMP-TABLE ttOLine2 BEFORE-TABLE ttOlineBefore2
   FIELD Ordernum AS INTEGER LABEL "Order Num" FORMAT "zzzzzzzzz9" INITIAL "0"
...
DEFINE VARIABLE daOrderDate AS DATE FORMAT "99/99/99":U INITIAL ? 
     LABEL "Order Date" 
     VIEW-AS FILL-IN 
     SIZE 14 BY 1 NO-UNDO.
...
"OrderDate" ? ? "date" ? ? ? ? ? ? no ? no no "53" yes no no "U" "" ""
     _Query            is OPENED
*/  /* BROWSE OrderBrowse */
&ANALYZE-RESUME
...
ON MOUSE-SELECT-DBLCLICK OF OrderBrowse IN FRAME dsFrame /* Order Browse */
DO:
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  RUN enable_UI.
  RUN orderSupportCopy.p PERSISTENT SET hOrderProc.
