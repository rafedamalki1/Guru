...
/*
prognamnque = "d:\guru\PRO9S\autotid.txt". 
prognamn = "d:\guru\PRO9\guru\borl.CSH".
prognamnnovell ="j:\guru\guru\borl.CSH".
OUTPUT TO VALUE(prognamnque) APPEND.
PUT "START AUTOKÖRNING BORLÄNGE " TODAY " " STRING(TIME,"HH:MM:SS") SKIP.
OUTPUT CLOSE.
...
CONNECT -db borl -S 2701 -H 10.50.1.5 -N tcp NO-ERROR.    
IF CONNECTED("borl") THEN DO:   
   SAVE CACHE COMPLETE VALUE(LDBNAME(1)) TO VALUE(prognamn).
   DISCONNECT borl NO-ERROR. 
END.              
OS-COPY VALUE(prognamn) VALUE(prognamnnovell).
OUTPUT TO VALUE(prognamnque)  APPEND.
PUT "BACKUPP START " TODAY " " STRING(TIME,"HH:MM:SS") SKIP.
OUTPUT CLOSE.

RUN BORLBACK.P (INPUT prognamnque).
OUTPUT TO VALUE(prognamnque) APPEND.
PUT "SLUT AUTOKÖRNING BORLÄNGE " TODAY " " STRING(TIME,"HH:MM:SS") SKIP.
OUTPUT CLOSE.

/*
IF NOT CONNECTED("borl") THEN CONNECT -db borl -S 2701 -H 10.50.1.5 -N tcp NO-ERROR.    
IF CONNECTED("borl") THEN DO:   
   prognamn = "j:\guru\guru\borl.CSH".
   SAVE CACHE COMPLETE VALUE(LDBNAME(1)) TO VALUE(prognamn).
   DISCONNECT borl NO-ERROR. 
END.
*/              
