...
ON CHOOSE OF bHelp DO:
  DO WITH FRAME dialog-1:
    ...
    CASE TRIM(rsMain:screen-value):
      ...
      RUN adecomm/_adehelp.p ( "uib", "CONTEXT",{&Join_Builder_Dlg_Box}, ? ).
    WHEN "{&Where}" THEN
      /* ON HELP in the Where Builder dialog box */
      RUN adecomm/_adehelp.p ( "uib", "CONTEXT",{&Where_Builder_Dlg_Box}, ? ).
    WHEN "{&Sort}"  THEN
      /* ON HELP in the Sort Selection dialog box */
      RUN adecomm/_adehelp.p ( "uib", "CONTEXT",{&Sort_Selection_Dlg_Box}, ? ).
    WHEN "{&Field}" THEN
      /* ON HELP in the Field Selection dialog box */
      RUN adecomm/_adehelp.p ( "uib", "CONTEXT",{&Field_Selection_Dlg_Box}, ? ).
    WHEN "{&Table}" THEN
      /* ON HELP in the Table Builder dialog box */
      RUN adecomm/_adehelp.p ( "uib", "CONTEXT",{&Table_Builder_Dlg_Box}, ? ).
    END CASE.
    ****/
  END.
END.
