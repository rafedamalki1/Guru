...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-LABELS NO-COLUMN-SCROLLING SIZE 75.5 BY 15.5.
...
DEFINE FRAME DIALOG-1
     ...
     BTN_SKRIV AT ROW 17.5 COL 48
     BTN_AVS AT ROW 17.5 COL 63
...
-------------------------------------------------------------*/
   /*HUVUD*/                    
   /*IF Guru.Konstanter:globforetag = "NORD" OR Guru.Konstanter:globforetag = "ETA" OR Guru.Konstanter:globforetag = "ESMA"
   OR Guru.Konstanter:globforetag = "ESAN" THEN DO:
      IF NOT Guru.Konstanter:appcon THEN DO:      
         RUN appfel_UI.
         RETURN.
      END. 
      RUN SELONIN.P ON Guru.Konstanter:apphand TRANSACTION DISTINCT (INPUT "pa90.reg", OUTPUT TABLE tidut).
      RETURN.
   END.          */
   IF Guru.Konstanter:globforetag = "GRAN" THEN DO:
      ifil = "\\GRANGURU\guru_ser\server\PRO9S\gran\losyd.d".
   END.    
   ELSE IF Guru.Konstanter:globforetag = "GADM" THEN DO:
      ifil = "\\GRANGURU\guru_ser\server\PRO9S\gadm\lotrad.d".
   END. 
   ELSE IF Guru.Konstanter:globforetag = "GSYD" THEN DO:
      ifil = "\\GRANGURU\guru_ser\server\PRO9S\gsyd\lokoll.d".
   END.       
   ELSE IF Guru.Konstanter:globforetag = "GRIT" THEN DO:
      ifil = "\\GRANGURU\guru_ser\server\PRO9S\grit\lokoll.d".
   END.      
   ELSE IF Guru.Konstanter:globforetag = "GREN" THEN DO:
      ifil = "\\GRANGURU\guru_ser\server\PRO9S\gren\lokoll.d".
   END.     
   ELSE IF Guru.Konstanter:globforetag = "GKRVA" THEN DO:
      ifil = "\\GRANGURU\guru_ser\server\PRO9S\gkrva\lokoll.d".
   END. 
   ELSE IF Guru.Konstanter:globforetag = "GKAL" THEN DO:
      ifil = "\\goliat\DELAD\server\PRO9S\gkal\franutGKEAB.d".
   END. 
   ELSE DO:
      ifil = "C:\GURU\pa90.reg".
   END.     
   IF NOT Guru.Konstanter:appcon THEN DO:      
      RUN appfel_UI.
      RETURN.
