...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BRW_VDB C-Win _STRUCTURED
  QUERY BRW_VDB NO-LOCK DISPLAY
      valdbtemp.VALDB FORMAT "X(45)":U WIDTH 45.5
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-LABELS NO-ROW-MARKERS NO-COLUMN-SCROLLING SIZE 49 BY 6.83
...
DEFINE FRAME DEFAULT-FRAME
     ...
     BTN_START AT ROW 4.5 COL 52.5
     BTN_AVB AT ROW 9.58 COL 52.5
     "Välj databas för start av Guru !" VIEW-AS TEXT
          SIZE 35.25 BY 2 AT ROW 1.54 COL 1.5
          FONT 17
...
"VALDB" ? "X(45)" "character" ? ? ? ? ? ? no ? no no "45.5" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ...
   {VALDBSUND9.I}
   {VALDBSUNDNAT.I}
   OPEN QUERY  {&BROWSE-NAME} FOR EACH valdbtemp BY ORDNING.
   {VALDBMAIN.I}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE sundval_UI C-Win 
PROCEDURE sundval_UI :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
   ...
   OS-DELETE VALUE(cWorkDirectory + "foretagstart.sund").
   IF valdbtemp.GFORETAG = "SUNDN9" THEN DO:
      OUTPUT TO VALUE(cWorkDirectory + "foretagstart.SUNDEN") NO-ECHO.
      OUTPUT CLOSE.
   END.   
   ELSE DO: 
      OUTPUT TO VALUE(cWorkDirectory + "foretagstart.SUNDNAT") NO-ECHO.
      OUTPUT CLOSE.
   END.                 
