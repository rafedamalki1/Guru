...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ...
   FOR EACH tidut:
      DELETE tidut.
   END. 
   IF SEL_UPP = "Det.lista Tid-övertid-trakt./område" THEN DO: 
      MESSAGE "Vill du att benämningen till AONR skall vara med i listan. OBS! välj liggande utskrift om svaret är Ja."
      VIEW-AS ALERT-BOX
      QUESTION BUTTONS YES-NO-CANCEL TITLE "Benämning till AONR" UPDATE svar AS LOGICAL.         
      IF svar THEN DO:
         aoval = TRUE.
      END.
...
-------------------------------------------------------------*/ 
   ...
   arrhjsumlon = 0.   
         ...
         SUMTIDDAG.DATUM <= avdatum AND SUMTIDDAG.OMRADE = utomr
   ...
   arrhjsumlon = 0
   arrhjsumind = 0.      
   FOR EACH slutsum BREAK BY slutsum.INOMRADE:           
      ACCUMULATE 
      slutsum.BELOPP (TOTAL BY slutsum.INOMRADE). 
      ACCUMULATE 
      slutsum.TIMMAR (TOTAL BY slutsum.INOMRADE). 
      ACCUMULATE 
      slutsum.OTIMMAR (TOTAL BY slutsum.INOMRADE). 
      ACCUMULATE 
      slutsum.OBELOPP (TOTAL BY slutsum.INOMRADE).  
      ACCUMULATE 
      slutsum.TBELOPP (TOTAL BY slutsum.INOMRADE). 
      ACCUMULATE 
      slutsum.LONKOST (TOTAL BY slutsum.INOMRADE). 
      ACCUMULATE 
      slutsum.INDEREKT (TOTAL BY slutsum.INOMRADE).
      IF LAST-OF(slutsum.INOMRADE) THEN DO TRANSACTION:
         CREATE slutsum1.
         ASSIGN          
         slutsum1.OMRADE = slutsum.INOMRADE      
   ...
   arrhjsum = 0   
   arrhjsumtid = 0.
   /* TOTALT PÅ VARJE AONR*/  
   FOR EACH restid BREAK BY restid.AONR BY restid.DELNR:           
      ACCUMULATE 
