...
DEFINE VARIABLE lname  AS CHAR NO-UNDO.
...
&IF "{&WINDOW-SYSTEM}" = "OSF/Motif" &THEN  
   &GLOBAL-DEFINE NAMECHRS 47 
&ELSE
   &GLOBAL-DEFINE NAMECHRS 46 
&ENDIF
&GLOBAL-DEFINE EDCHRS 46 
...
FORM
   ...
   Dbb._Db-addr      AT 17 FORMAT "x({&NAMECHRS})" 
	       	     VIEW-AS FILL-IN NATIVE              SKIP({&VM_WIDG})
   "CONNECT-"        AT  2 VIEW-AS TEXT 	     
   args              AT 17 VIEW-AS EDITOR 
                     INNER-LINES 3 INNER-CHARS {&EDCHRS} SKIP({&VM_WIDG})
   ...
   "Physical Name:"  AT ROW-OF dbb._Db-addr + 0.2  COLUMN 2 VIEW-AS TEXT
...
FORM
   ...
   Dbb._Db-addr      AT 2 {&STDPH_FILL} FORMAT "x(80)" 
...
PROCEDURE Show_Db:
   ...
   IF AVAILABLE Dbb THEN DO:
      args = Dbb._Db-comm.
      DISPLAY 
	 Dbb._Db-addr
      	 args
	 WITH FRAME dbauto.
      /* Set lname to match name in buffer for next update */
      lname = Dbb._Db-name. 
   END.
   ELSE DO:
      args = "". /* you can't use @ with editor widgets so we need this */
      DISPLAY 
      	 "" @ Dbb._Db-addr
      	 args
      	 WITH FRAME dbauto.
