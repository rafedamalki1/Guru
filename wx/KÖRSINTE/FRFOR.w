...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-LABELS NO-COLUMN-SCROLLING SIZE 75.5 BY 13.5.
...
DEFINE FRAME DIALOG-1
     ...
     BTN_SKRIV AT ROW 15.75 COL 48
     BTN_AVS AT ROW 15.75 COL 63
...
-------------------------------------------------------------*/
   ...
   IF Guru.Konstanter:globforetag = "GRAN" THEN DO:
      INPUT FROM \\GRANGURU\guru_ser\server\PRO9S\gran\frforma.d NO-ECHO.
   END.  
   ELSE IF Guru.Konstanter:globforetag = "GADM" THEN DO:
      INPUT FROM \\GRANGURU\guru_ser\server\PRO9S\gadm\frforma.d NO-ECHO.
   END.      
   ELSE IF Guru.Konstanter:globforetag = "GRIT" THEN DO:
      INPUT FROM \\GRANGURU\guru_ser\server\PRO9S\grit\frforma.d NO-ECHO.
   END. 
   ELSE IF Guru.Konstanter:globforetag = "GKAL" THEN DO:
      INPUT FROM \\goliat\DELAD\server\PRO9S\gkal\frforma.d NO-ECHO.
   END.
   ELSE IF Guru.Konstanter:globforetag = "GKRVA" THEN DO:
      INPUT FROM \\GRANGURU\guru_ser\server\PRO9S\gkrva\frforma.d NO-ECHO.
   END.      
   ELSE DO:
      INPUT FROM C:\GURU\frforma.d NO-ECHO.
   END.     
   REPEAT TRANSACTION:
      CREATE tidut.
      ASSIGN.
