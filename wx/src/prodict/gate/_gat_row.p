...
form
  l_ttb_idx.detail at 2 view-as editor scrollbar-v
    &IF "{&WINDOW-SYSTEM}" = "TTY"
     &THEN size 70 by 13
    &ELSEIF "{&WINDOW-SYSTEM}" begins "MS-WIN"
     &THEN inner-chars 57 inner-lines 13 /*scrollbar-h large*/
 ...
 &THEN
  on HELP of frame l_frm_idx OR CHOOSE of btn_Help in frame l_frm_idx
     RUN "adecomm/_adehelp.p" (INPUT "dict", INPUT "CONTEXT", 
                               INPUT 0{&RowID_Idx_Dlg_Box},
                               INPUT ?).
  on HELP of frame l_frm_dtl OR CHOOSE of btn_Help in frame l_frm_idx
     RUN "adecomm/_adehelp.p" (INPUT "dict", INPUT "CONTEXT", 
                               INPUT 0{&RowID_Idx_Dtl_Dlg_Box},
                               INPUT ?).
  &ENDIF
