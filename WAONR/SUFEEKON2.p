...
/*   FIELD EGEO LIKE EKRAPPRESULT.EGEO       */
   ...
   FIELD ELONBELOPP LIKE EKRAPPRESULT.ELONBELOPP         
   FIELD FELDEBKRED AS LOGICAL
   FIELD ERESULTENH AS CHARACTER
   INDEX ORG IS PRIMARY PERSONALKOD EVERDATUM EORG EPROJEKT EKOSTNADSSLAG ASCENDING.   
...
DO WHILE AVAILABLE(ekoforst):
   ...
   /*extern konsult*/
   IF PERSONALTAB.BEFATTNING = "Extern konsult" THEN DO:
      FIND FIRST omrkosttemp WHERE omrkosttemp.OMRADE = ekoforst.EORG AND
      omrkosttemp.PERSTYP = "KONSULT" AND omrkosttemp.NRSERIE = SUBSTRING(ekoforst.EPROJEKT,1,2) NO-LOCK NO-ERROR.
      IF NOT AVAILABLE omrkosttemp  THEN DO:   
         FIND FIRST omrkosttemp WHERE omrkosttemp.OMRADE = ekoforst.EORG AND
         omrkosttemp.PERSTYP = "KONSULT" AND omrkosttemp.NRSERIE = SUBSTRING(ekoforst.EPROJEKT,1,1) NO-LOCK NO-ERROR.
         IF NOT AVAILABLE omrkosttemp  THEN DO:   
            FIND FIRST omrkosttemp WHERE omrkosttemp.OMRADE = ekoforst.EORG AND
            omrkosttemp.PERSTYP = "KONSULT" AND omrkosttemp.NRSERIE = "" NO-LOCK NO-ERROR.
         END.      
      END.
      IF NOT AVAILABLE omrkosttemp THEN DO:
         RUN sammut_UI (INPUT 3).
         OUTPUT TO VALUE(samvarglb) APPEND.
         PUT UNFORMATTED "FEL KONTAKTA ELPOOL  konsult "   ekoforst.EPERSONALKOD " " ekoforst.EORG SKIP.
         OUTPUT CLOSE.
      END.
      ELSE DO:   
         ASSIGN
...
DO WHILE AVAILABLE(eko):
   ...
   DO WHILE AVAILABLE(AONRKONTKOD):
      IF Guru.Konstanter:globforetag = "SUND" THEN DO:
         ...
         /*santal*/
         slutut.ERESULTENH = eko.ERESULTENH AND
         ...
         slutut.K5 = AONRKONTKOD.K5 AND
         slutut.BIL = eko.ELONTILLAGG AND
      ...
      /*santal*/
      slutut.ERESULTENH = eko.ERESULTENH 
...
FOR EACH foretemp:
   ...
   IF foretemp.FTG = "ELNÄT" THEN DO:
      kommando2 = "d:\DELAD\SERVER\PRO10S\BACKEXPORT\ekoF" + STRING(TODAY,"99999999"). 
      kommando = "d:\DELAD\SERVER\PRO10S\EXPORT\EKONOMI\ekoF" + STRING(TODAY,"99999999"). 
         
   END.
   ELSE DO:
      kommando2 = "d:\DELAD\SERVER\PRO10S\BACKEXPORT\" + "ekoF" + foretemp.FTG + STRING(TODAY,"99999999"). 
      kommando = "d:\DELAD\SERVER\PRO10S\EXPORT\EKONOMI\" + "ekoF" + foretemp.FTG + STRING(TODAY,"99999999").                
   END.
...
PROCEDURE ut_UI:
   ...
   */
   ...
   IF slutut.DEBKRED = TRUE THEN DO:
      SUBSTRING(str,100,10) = slutut.K4.
      SUBSTRING(str,110) = STRING(INTEGER(slutut.ANTAL)).  
      IF Guru.Konstanter:globforetag = "SUND" THEN DO:
         SUBSTRING(str,125) = STRING(slutut.K5) + " " + STRING(slutut.MOTPART).
      END.       
   END.   
   IF str = ? THEN DO:
      OUTPUT TO VALUE(samvarglb) APPEND.
...
PROCEDURE kost_UI:
   ...
   eko.ENY = TRUE AND
   /*santal*/
   eko.ERESULTENH = ekoforst.ERESULTENH AND 
   ...
   /*santal*/
   eko.ERESULTENH = ekoforst.ERESULTENH
   ...
   ELSE DO:
      ...
      IF eko.ERESULTENH = "OVE" THEN eko.EANTAL = eko.EANTAL + ekoforst.EANTAL.   
