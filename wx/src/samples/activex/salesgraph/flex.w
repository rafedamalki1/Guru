...
------------------------------------------------------------------------------*/
    ...
    FOR EACH order of bcust, each order-line of order BREAK BY MONTH(order.order-date):
        ACCUMULATE order-line.extended-price (TOTAL BY MONTH(order.order-date)).
        IF LAST-OF(MONTH(order.order-date)) THEN DO:
            currmth = MONTH(order.order-date).
            monthData[currmth] = monthData[currmth] + 
                                 ACCUM TOTAL BY MONTH(order.order-date) order-line.extended-price.
        END.    
    END.    
END PROCEDURE.
