...
/*------------------------------------------------------------------------
    ...
    Purpose     : 
                    ...
                    run Modules\Register\AllaDatabaserStart.p.
    Syntax      :
...
PROCEDURE AllaDatabaser_UI :
   ...
   DEFINE VARIABLE AllaDB   AS Modules.Register.AllaDatabaser NO-UNDO.
    
   /*får windows,dbmanager mm*/
   startalladbroot = NEW Guru.Root().
   AllaDB = NEW Modules.Register.AllaDatabaser(INPUT startalladbroot).
...
PROCEDURE AllaDatabaserU_UI :
   ...
   AllaDB = NEW Modules.Register.AllaDatabaser().
   WAIT-FOR AllaDB:ShowDialog().
   /*EMPTY TEMP-TABLE rubrikTT NO-ERROR.*/ 
