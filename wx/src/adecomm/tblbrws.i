...
IF p_multi THEN  
  ASSIGN FRAME ffrm-m:FRAME  = p_dlg
         ...
         FRAME ffrm-m:COLUMN = p_tc +
                               &IF "{&WINDOW-SYSTEM}" = "OSF/Motif" &THEN 22
                               &ELSE 20  &ENDIF
...
IF NOT AVAILABLE _FILE THEN DO:
  IF cur-rec NE ? THEN FIND _FILE NO-LOCK
                           WHERE RECID(_FILE) = cur-rec NO-ERROR.
