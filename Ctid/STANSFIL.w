...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-LABELS NO-COLUMN-SCROLLING SIZE 78.5 BY 25.25.
...
DEFINE FRAME DIALOG-1
     ...
     BTN_SKRIV AT ROW 27.33 COL 36
     BTN_EXCEL AT ROW 27.33 COL 51
     BTN_AVS AT ROW 27.33 COL 66
     SPACE(2.24) SKIP(0.87)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Senaste löneöverföring".
...
DO:
   IF Guru.Konstanter:globforetag = "SUND" OR Guru.Konstanter:globforetag = "SNAT" THEN DO:
      MESSAGE "Vill du skriva ut en person per sida?" 
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ...
   {muswait.i}    
   {ALLSTARTDYN.I} 
   FRAME {&FRAME-NAME}:TITLE = "Senaste tidfil ".
   ...
   END.
   RUN enable_UI.       
   IF Guru.Konstanter:globforetag = "SUND" OR Guru.Konstanter:globforetag = "SNAT" OR Guru.Konstanter:globforetag = "GKAL" OR Guru.Konstanter:globforetag = "ELPA"  THEN musz = musz.
   ELSE BTN_EXCEL:HIDDEN IN FRAME {&FRAME-NAME} = TRUE .
   {FRMSIZED.I}  
...
-------------------------------------------------------------*/
   ...
   IF Guru.Konstanter:globforetag = "GKAL" THEN DO:
      ifil = "\\goliat\DELAD\server\PRO9S\gkal\stanstid.d".
   END.    
   ELSE IF Guru.Konstanter:globforetag = "LULE" THEN DO:      
      ifil = "D:\elpool\DELAD\PRO9s\EXPORT\LON\Lonback\stanstid.d".
   END.  
   ELSE IF Guru.Konstanter:globforetag = "elpa" THEN DO:   
      ifil = "c:\pro10\stanstid.d".
   END. 
   IF Guru.Konstanter:appcon THEN RUN SELONIN.P ON Guru.Konstanter:apphand TRANSACTION DISTINCT (INPUT ifil, OUTPUT TABLE tidut).
   ELSE RUN SELONIN.P (INPUT ifil, OUTPUT TABLE tidut).
