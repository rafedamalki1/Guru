...
PROCEDURE Choose_Properties:
   ...
   case s_CurrObj:
      ...
      when {&OBJ_DOM} then
      do:
      	 run adedict/_objsel.p (INPUT {&OBJ_DOM}).
      	 run Do_Dbl_Click (INPUT ({&OBJ_DOM})).
      end.
      ---------------------*/
      when {&OBJ_SEQ} then
      do:
...
/*----- SELECTION of DOMAINS BUTTON -----*/
/*-------------------------------
On mouse-select-down of s_icn_Doms in frame browse
   run Push_Obj_Button (INPUT {&OBJ_DOM}, 
      	       	     	INPUT s_icn_Doms:HANDLE in frame browse,
      	       	     	INPUT false).
--------------------------------*/
...
/*--------------------------------------------
on  U1 of s_lst_Doms in frame browse, s_DomFill in frame browse
   run adedict/_objsel.p (INPUT {&OBJ_DOM}).
---------------------------------------------*/
