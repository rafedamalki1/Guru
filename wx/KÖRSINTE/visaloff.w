...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BRW_UT WINDOW-2 _STRUCTURED
  QUERY BRW_UT NO-LOCK DISPLAY
      tidut.ut FORMAT "X(132)":U WIDTH 81
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME FRAME-VINST
     ...
     CMB_LEV AT ROW 27.96 COL 36.25 COLON-ALIGNED NO-LABEL
     BTN_SKRIV AT ROW 27.96 COL 57.5
     BTN_AVS AT ROW 27.96 COL 74
...
"tidut.ut" ? ? "character" ? ? ? ? ? ? no ? no no "81" yes no no "U" "" ""
     _Query            is OPENED
*/  /* BROWSE BRW_UT */
&ANALYZE-RESUME
...
DO:
   ...
   RUN visloff_UI IN kundoffproch (INPUT valkalknr,INPUT vald_lev,INPUT TABLE bmtrl_mtrl,OUTPUT TABLE tidut).                 
   RUN enable_UI.   
END.
