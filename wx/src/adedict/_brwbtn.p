...
case p_Obj:
   ...
   /*---------------
   when {&OBJ_DOM} then
      s_Res = p_Icn:LOAD-IMAGE("adeicon/domdown").
   -----------------*/
   when {&OBJ_SEQ} then
...
case s_CurrObj:
   ...
   when {&OBJ_DOM} then
      assign
   	 s_btn_Create:label in frame browse = "C&reate Domain..."
   	 s_btn_Props:label in frame browse = "Domain &Properties..."
   	 s_btn_Delete:label in frame browse = "De&lete Domain"
   	 s_btn_Delete:hidden in frame browse = no.
   ---------------------*/
   when {&OBJ_SEQ} then
      assign
