...
FOR EACH eko:
   FIND FIRST sumkont WHERE sumkont.DEBKRED = eko.EDEBKRED AND 
...
FOR EACH sumkont:
  ...
  /*iedimi1*/
   ...
   /*iedimi4*/ /*aonrkont.k1*/
   IF INTEGER(sumkont.AONR) = 0 THEN SUBSTRING(str,utnrIBT[nrcolIBT[5]]) = "".
   ELSE SUBSTRING(str,utnrIBT[nrcolIBT[5]]) = sumkont.AONR.       
   

     
   /* aonrkontkod.k2 skall ej med på projekten Lena 20030521*/
   IF sumkont.DEBKRED = TRUE THEN SUBSTRING(str,utnrIBT[nrcolIBT[4]]) = "7000".
   ...
   /*KREDIT*/
   IF sumkont.DEBKRED = FALSE THEN SUBSTRING(str,utnrIBT[nrcolIBT[15]],1) = "-".
   ASSIGN
   filut = ""
   filutkopia = "". 
   ASSIGN
