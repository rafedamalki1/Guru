...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-LABELS NO-COLUMN-SCROLLING SIZE 91.5 BY 18.5.
...
DEFINE FRAME FRAME-TIDS
     ...
     BTN_SKRIV AT ROW 20.5 COL 63.5
     BTN_AVS AT ROW 20.5 COL 79
...
-------------------------------------------------------------*/
   ...
   IF manad = 1 THEN DO:
      ASSIGN
      SUBSTRING(tidut.UT,4) = "Senaste registrering" 
   ...
   ELSE DO:
      ASSIGN
      SUBSTRING(tidut.UT,4) = "Annan- över- eller flex-tid för senast körda registreringar" 
      SUBSTRING(tidut.UT,60) = STRING(TODAY)
      SUBSTRING(tidut.UT,70) = STRING(TIME,"HH:MM:SS").
      CREATE tidut. 
      ASSIGN
   ...
   SUBSTRING(tidut.UT,61) = CAPS(Guru.Konstanter:gaok)     
   str =                                                                                                                            
   "=======.=========.===============.========.==========.=====.======.".             
   CREATE tidut.                  
   ASSIGN
...
-------------------------------------------------------------*/ 
   IF Guru.Konstanter:appcon THEN DO:                           
      RUN ALLSIAP.P ON Guru.Konstanter:apphand TRANSACTION DISTINCT 
      (INPUT pkod,OUTPUT TABLE sista).         
   END.
   ELSE DO:
      RUN ALLSIAP.P 
      (INPUT pkod,OUTPUT TABLE sista).         
...
/* -----------------------------------------------------------
  ...
  Notes:       
-------------------------------------------------------------*/     
   FOR EACH sista USE-INDEX PERSONALKOD: 
      CREATE tidut. 
      ASSIGN
      SUBSTRING(tidut.UT,1) = sista.PERSONALKOD
      SUBSTRING(tidut.UT,9) = sista.FORNAMN
      SUBSTRING(tidut.UT,19) = sista.EFTERNAMN
      SUBSTRING(tidut.UT,35) = STRING(sista.DATUM)          
      SUBSTRING(tidut.UT,44) = SUBSTRING(sista.KNAPP,1,10)
      SUBSTRING(tidut.UT,55) = STRING(sista.TID,">9.99").          
   END.            
   CREATE tidut. 
   ASSIGN 
   tidut.UT = str.  
