...
PROCEDURE etiketer_UI :
   ...
   FOR EACH marktmp:
      ...
      */
      ...
      END.
      ASSIGN
      SUBSTRING(tidut.UTRAD1,utnr[nrcol[i]]) = TRIM(marktmp.MARKAGARE) 
      SUBSTRING(tidut.UTRAD2,utnr[nrcol[i]]) = TRIM(marktmp.GATUADRESS) 
      SUBSTRING(tidut.UTRAD3,utnr[nrcol[i]]) = TRIM(STRING(marktmp.POSTNUMMER,"999 99") + " " +  marktmp.POSTADRESS).
      i = i + 1.
...
PROCEDURE startin_UI:
   ...
   slutc = "D"
   iRad = 2.    
   RUN readexcelstart_UI (INPUT "D:\delad\gg\TESTLENAETIKETT.xls").
