...
Define var trig_txt as char NO-UNDO
	 ...
	 SIZE 74 BY 10. /* 94/02/10 hutegger: cursor-offset not supported */
			/*  with lareg editor's anymore */
...
/*----- SELECTION of CLOSE BUTTON -----*/
/*-----------------
   This is no longer the behavior of close, it now cancel's
   any unsaved changes.  Leave this here in comments in
   case this changes again.

on choose of btn_Close in frame trigedit
do:
   Define var changed as logical NO-UNDO.
   Define var answer  as logical NO-UNDO.
   Define var success as logical NO-UNDO.
   
   /* We will not re-compute crc on close unless something has 
      changed since we entered dialog.
   */
   run Trig_Changed (OUTPUT changed).
   if NOT changed then return.

   /* Ask the user if he wants to save changes. */
   message "You have made changes to the" trig_event SKIP
	   "trigger that haven't been saved." SKIP(1)
	   "Do you want to save these changes?"
	   view-as ALERT-BOX QUESTION 
	   buttons YES-NO-CANCEL update answer.

   if answer = no then
      return.
   else if answer = ? then /* user hit cancel */
      return NO-APPLY.
   else do:
      run Check_Syntax (INPUT false, OUTPUT success).
      if success then
	 run Save_Trigger (OUTPUT success).
      if NOT success then
	 return NO-APPLY.
   end.
end.
...
/* Deal with privileges - this is way too hairy to do accurately.  To 
   simplify we will do the following.  If the user does not have read
