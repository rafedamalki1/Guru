...
do:
   ...
   run as4dict/_leavnam.p (INPUT b_File._File-Name, INPUT s_win_Tbl,
   ...
   if can-find (as4dict.p__File where as4dict.p__File._File-Name = name
                         AND as4dict.p__File._File-number <> b_File._File-number) then
   do:
...
do:
   ...
   run as4dict/_leavnam.p (INPUT  b_Field._Field-Name, INPUT win, 
                                             OUTPUT name, OUTPUT okay).
   if okay = ? then return.
   if NOT okay then do:
      s_Valid = no.
   ...
   /* Make sure there isn't already an object with this name.   */
   if can-find (as4dict.p__Field where as4dict.p__Field._File-number = s_TblForNo AND
                             as4dict.p__Field._Field-Name = name AND
                             as4dict.p__Field._Fld-number <> b_field._Fld-number )
                             then  do:
...
do:                                          
 ...
 /*
  if s_Adding and 
    can-find(as4dict.p__Field where as4dict.p__Field._AS4-file = b_field._AS4-File
                                                          and as4dict.p__Field._AS4-Library = b_Field._AS4-Library
                                                          and as4dict.p__Field._For-name = b_Field._For-name)
    then do:
...
do:
   ...
   */
   if can-find (as4dict.p__Seq where as4dict.p__Seq._Seq-Name = name) then
   do:
...
do:
   ...
   /* Make sure there isn't already an object with this name.   */
   IF LENGTH(name) > 0 THEN DO:
     if can-find (as4dict.p__Index where as4dict.p__Index._File-number = s_TblForNo AND
                             as4dict.p__Index._Index-Name = name) then
     do:
