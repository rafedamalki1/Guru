...
PROCEDURE ProcessRootDictionary:
  ...
  DO WHILE TRUE:
    ...
    IF INDEX(text-ptr,"~/Pages") > 0 THEN DO:
      text-ptr = SUBSTR(text-ptr,INDEX(text-ptr,"~/Pages")).
    ...
    IF INDEX(text-ptr,"~/AcroForm") > 0 THEN DO:
      text-ptr = SUBSTR(text-ptr,INDEX(text-ptr,"~/AcroForm")).
      form-obj = INT( ENTRY(2, text-ptr, " ") ).
      form-gen = INT( ENTRY(3, text-ptr, " ") ).
        ...
        IF INDEX(text-ptr,"~/P ") > 0 THEN DO:
          ...
          FIND FIRST B_Par_object
               WHERE B_par_object.obj_Stream = pStream
                 ...
                 AND B_par_object.gen_id     = par-gen NO-LOCK NO-ERROR.
          IF AVAIL B_par_object THEN
...
PROCEDURE ProcessEntry:
  ...
  IF L_Array THEN DO L_Loop = 1 TO NUM-ENTRIES(pEntry," ") BY 4:
    ...
    ASSIGN TT_Resource.obj_stream = pStream
           ...
           TT_Resource.res_obj    = L_Obj
           TT_Resource.res_gen    = L_Gen
           TT_Resource.res_text   = "~/" + ENTRY(L_Loop, pEntry, " ").
...
PROCEDURE ProcessFontDictionary:
    ...
    IF FIRST(B_TT_object.obj_Seq) THEN DO:
      ...
      DO WHILE TRUE:
        ...
        IF INDEX(text-ptr,"~/FontDescriptor") > 0 THEN DO:
          ...
          descriptor = TRUE.
          curr-ptr = SEEK(INPUT).
          RUN ProcessFontDescriptorDictionary (pTag, pObject, pGen, obj-ptr, gen-ptr, enc-obj, enc-gen, OUTPUT font_name).
          SEEK INPUT TO curr-ptr.
        END.
        ...
        IF INDEX(text-ptr,"~/BaseFont") > 0 THEN DO:
  ...
  ELSE DO:
    FIND FIRST TT_Font WHERE TT_Font.obj_stream = pStream
                         ...
                         AND TT_Font.font_name  = font_name NO-ERROR.
...
PROCEDURE ProcessPageContent:
  ...
  FOR EACH B_TT_Object WHERE B_TT_object.obj_stream = pStream
    ...
    IF FIRST(B_TT_object.obj_Seq) THEN DO:
      ...
      IF pCounter = 1 THEN DO:
        OS-DELETE VALUE(".\" + pID + STRING(pPage) + ".txt") NO-ERROR.
      ...
      IF l_Length > 0 THEN DO:
        ...
        IF NOT L_Filter THEN DO:
          OUTPUT TO VALUE(".\" + pID + STRING(pPage) + ".txt") BINARY NO-MAP NO-CONVERT APPEND.
            EXPORT m_Contents.
          OUTPUT CLOSE.
        END. /* No Filter */
        ...
        ELSE DO:
          ...
          OUTPUT TO VALUE(".\" + pID + STRING(pPage) + "-" + STRING(B_TT_Object.obj_id) + ".txt") BINARY NO-MAP NO-CONVERT.
            EXPORT m_Contents.
          OUTPUT CLOSE.

          /* Now Decompress the File */
          decompressfile ( ".\" + pID + STRING(pPage) + "-" + STRING(B_TT_Object.obj_id) + ".txt",
                           ".\" + "decompressed.txt").

          /* Remove the individual Object file */
          OS-DELETE VALUE(".\" + pID + STRING(pPage) + "-" + STRING(B_TT_Object.obj_id) + ".txt").

          /* Now re-import the decompressed file */
          SET-SIZE(m_Contents) = 0.
          FILE-INFO:FILE-NAME = ".\" + "decompressed.txt".
          ...
          OUTPUT TO VALUE(".\" + "filter.txt") BINARY NO-MAP NO-CONVERT APPEND.
            EXPORT m_Contents.
          OUTPUT CLOSE.

          OS-DELETE VALUE(".\" + "decompressed.txt").
        END. /* Filtered */
  ...
  IF l_Filter THEN DO:
    ...
    OS-COPY VALUE(".\" + "filter.txt")
            VALUE(".\" + pID + STRING(pPage) + ".txt").

    /*
    compressfile(".\" + "filter.txt",
                 ".\" + pID + STRING(pPage) + ".txt").
    */
  END.
