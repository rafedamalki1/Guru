...
History:
    D McMann    98/06/24 Added NOT _File._Hidden when all files have been
                         selected so that the DataServer Hidden files would
                         not try to be loaded with data.
...
FORM SKIP({&TFM_WID})
  ...
  btn_File  SKIP ({&VM_WIDG})
    ...
    err-to-file VIEW-AS TOGGLE-BOX LABEL "Output Errors to Fi&le"
        COLON {&LINEUP} SKIP({&VM_WIDG})
...
IF io-frame = "df" THEN DO:
  ...
  DO ON ERROR UNDO,RETRY ON ENDKEY UNDO,LEAVE WITH FRAME read-df:
     ENABLE stop_flg err-to-file err-to-screen &IF "{&WINDOW-SYSTEM}" <> "TTY" &THEN warntxt &ENDIF WITH FRAME read-df.
   ASSIGN
      stop_flg = user_env[4] BEGINS "y".
    
    UPDATE user_env[2] btn_File stop_flg err-to-file err-to-screen
