DEFINE VARIABLE vSmtpServer AS CHARACTER FORMAT "X(50)" 
LABEL "Smtp Server" INITIAL "smtp.company.com" NO-UNDO.
DEFINE VARIABLE vTo         AS CHARACTER FORMAT "X(50)"
LABEL "To" INITIAL "toaddress@tocompany.com" NO-UNDO.
DEFINE VARIABLE vFrom       AS CHARACTER FORMAT "X(50)"
LABEL "From" INITIAL "myaddress@mycompany.com" NO-UNDO.
DEFINE VARIABLE vSubject    AS CHARACTER FORMAT "X(50)"
LABEL "Subject" 
INITIAL "test email using Progress 4GL sockets" NO-UNDO.
DEFINE VARIABLE vBody AS CHARACTER
   VIEW-AS EDITOR INNER-LINES 20 INNER-CHARS 70
   SCROLLBAR-VERTICAL INITIAL "test email" NO-UNDO.
...
/* FORM definition */
FORM
vSmtpServer COLON 15 HELP "Enter the address of            
an smtpserver, example: smtp.company.com" SKIP
vTo         COLON 15 HELP "Enter recipient's email address,
example: toaddress@tocompany.com" SKIP
   vFrom       COLON 15 HELP "Enter your email            
address,example:
myaddress@mycompany.com" SKIP
   ...
   DO ON ENDKEY UNDO, LEAVE:
      ...
      DISPLAY vSmtpServer vTo vFrom vSubject vdebugmode vBody 
         WITH FRAME sendmail.
   ...
   THIS-PROCEDURE).
   vstatus = hSocket:CONNECT("-S 25 -H " 
      + vSmtpServer)NO-ERROR.
 ...
 IF vdebugmode THEN MESSAGE "newState: " newState " pstring: "pstring VIEW-AS ALERT-BOX. vState = newState.
 ...
 PROCEDURE readHandler:
   ...
   IF vlength > 0 THEN DO:
      ...
      IF vdebugmode THEN MESSAGE "server:" str VIEW-AS ALERT-BOX.
      SET-SIZE(vbuffer) = 0.
      v = INTEGER(ENTRY(1, str," ")).
      CASE vState:
      WHEN 1 THEN IF v = 220 THEN 
      RUN newState(2, "HELO how are you? ~r~n").
      ELSE vState = -1.
      WHEN 2 THEN IF v = 250 THEN
         RUN newState(3, "MAIL From: " + vFrom + "~r~n").
         ELSE vState = -1.
         WHEN 3 THEN IF v = 250 THEN
         RUN newState(4, "RCPT TO: " + vTo + "~r~n").
         ELSE vState = -1.
         WHEN 4 THEN IF v = 250 THEN 
         RUN newState(5, "DATA ~r~n").
         ELSE vState = -1.
         WHEN 5 THEN IF v = 354 THEN
         ...
         ".~r~n").
         ELSE vState = -1.
         WHEN 6 THEN IF v = 250 THEN
         RUN newState(7,"QUIT~r~n").
         ELSE vState = -1.
         END CASE.
      END.
      IF vState = 7 THEN MESSAGE "Email has been accepted
      for delivery.".
      IF vState < 0 THEN MESSAGE "Email has been aborted".
  END.
