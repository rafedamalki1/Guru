...
&IF DEFINED(DONTDEFINE-HPAPI)=0 &THEN
  ...
  /* IF NOT VALID-HANDLE(hpApi) THEN*/ 
  DEFINE VARIABLE pa64 AS LOGICAL NO-UNDO.
    IF PROVERSION BEGINS "11.2" THEN RUN Windows.p PERSISTENT SET hpApi.
   ELSE DO: 
      RUN CHECK3264.p (OUTPUT pa64).
      IF pa64 = TRUE THEN RUN Windows64.p PERSISTENT SET hpApi.
      ELSE RUN Windows.p PERSISTENT SET hpApi.
    END.  
