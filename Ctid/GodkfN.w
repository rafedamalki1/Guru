...
godmarkpers.FORNAMN godmarkpers.EFTERNAMN godmarkpers.KOLLDATUM ~
...
vgodmarkpers.FORNAMN vgodmarkpers.EFTERNAMN vgodmarkpers.DATUM ~
vgodmarkpers.GODKAND 
...
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-REGIS TOG_FARDIG FILL-IN-INGENGOD 
...
DEFINE VARIABLE FILL-IN-INGENGOD AS CHARACTER FORMAT "X(256)":U 
      VIEW-AS TEXT 
     SIZE 43.5 BY 1.5
     FONT 17 NO-UNDO.
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS MULTIPLE SIZE 54.5 BY 20
         TITLE "Färdigrapporterade tidsedlar".
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS MULTIPLE SIZE 62.88 BY 20
         TITLE "Godkänn tidsedel".


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME DEFAULT-FRAME
     FILL-IN-REGIS AT ROW 1.83 COL 1.63 NO-LABEL
     FILL-IN_NAMN AT ROW 1.83 COL 20.63 COLON-ALIGNED HELP
          "ANGE ORAGNISTIONENSBENÄMNING" NO-LABEL
     FILL-IN_FORNAMN AT ROW 1.83 COL 21 COLON-ALIGNED NO-LABEL
     FILL-IN_EFTERNAMN AT ROW 1.83 COL 38.63 COLON-ALIGNED NO-LABEL
     FILL-IN-AR AT ROW 3.33 COL 4.5 COLON-ALIGNED NO-LABEL
     FILL-IN-MANAD AT ROW 3.33 COL 13.75 COLON-ALIGNED NO-LABEL
     BRW_TID AT ROW 4.83 COL 1.5
     BRW_VTID AT ROW 4.83 COL 62.5
     BTN_OVER AT ROW 13.17 COL 56.38
     BTN_BACK AT ROW 15.38 COL 56.38
     FBTN_VISA AT ROW 25.38 COL 9
     FBTN_VISAM AT ROW 25.38 COL 28.5
     BTN_GODM AT ROW 25.38 COL 81.5
     BTN_AVB AT ROW 25.38 COL 111.38
     RAD_ALLVAL AT ROW 26.71 COL 56.5 NO-LABEL
     TOG_FARDIG AT ROW 26.96 COL 1.5
     FILL-IN-INGENGOD AT ROW 12.5 COL 4 COLON-ALIGNED NO-LABEL WIDGET-ID 2
...
/* SETTINGS FOR FRAME DEFAULT-FRAME
   FRAME-NAME                                                           */
/* BROWSE-TAB BRW_TID FILL-IN-MANAD DEFAULT-FRAME */
...
/* SETTINGS FOR FILL-IN FILL-IN-INGENGOD IN FRAME DEFAULT-FRAME
   NO-ENABLE                                                            */
...
"godmarkpers.EFTERNAMN" "Efternamn" "X(256)" "character" ? ? ? ? ? ? no ? no no "15" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > Temp-Tables.godmarkpers.KOLLDATUM
...
"vgodmarkpers.FORNAMN" "Förnamn" "X(256)" "character" ? ? ? ? ? ? no ? no no "12" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.vgodmarkpers.EFTERNAMN
"vgodmarkpers.EFTERNAMN" "Efternamn" "X(256)" "character" ? ? ? ? ? ? no ? no no "21" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > Temp-Tables.vgodmarkpers.DATUM
"vgodmarkpers.DATUM" "Färdig! till" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > Temp-Tables.vgodmarkpers.GODKAND
...
ON END-ERROR OF C-Win /* Godkänna tidsedlar */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Godkänna tidsedlar */
DO:
...
ON MOUSE-SELECT-DBLCLICK OF BRW_TID IN FRAME DEFAULT-FRAME /* Färdigrapporterade tidsedlar */
DO:
...
DO:      
   EMPTY TEMP-TABLE evgodmarkpers  NO-ERROR. 
   hjpkod = "".
   ...
   ELSE DO:   
      ...
      APPLY "CHOOSE" TO FBTN_VISA.      
      IF AVAILABLE godmarkpers THEN DO:
         IF sppkod = godmarkpers.PERSONALKOD THEN DO:         
            CREATE vgodmarkpers.
            BUFFER-COPY godmarkpers TO vgodmarkpers.
         END.
      END.
...
DO:
   IF AVAILABLE godmarkpers THEN DO:     
      ...
      IF tillochmeddatum NE ? THEN DO:
         ASSIGN 
         godmarkpers.DATUM = tillochmeddatum.
...
ON CHOOSE OF FBTN_VISAM IN FRAME DEFAULT-FRAME /* Visa månadssamm. */
DO:
   IF AVAILABLE godmarkpers THEN DO:
      {muswait.i}   
      ASSIGN    
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FBTN_VISAM C-Win
ON GO OF FBTN_VISAM IN FRAME DEFAULT-FRAME /* Visa månadssamm. */
DO:
   RETURN.
END.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ...
   EMPTY TEMP-TABLE vgodmarkpers NO-ERROR. 
   hjpkod = "".
   ...
   IF Guru.Konstanter:appcon THEN DO:                           
      ...
      (INPUT 1,INPUT Guru.Konstanter:globanv,INPUT "F",
   ...
   IF RAD_ALLVAL = 2 THEN DO:
      ...
      FIND FIRST omrtemp WHERE omrtemp.OMRADE = tidpers.OMRADE NO-LOCK NO-ERROR.
      ASSIGN
   ...
   FILL-IN-INGENGOD = "Det finns inga tidsedlar att godkänna".
   RUN enable_UI.   
   {FRMSIZE.I}  
   ...
   IF NOT AVAILABLE godmarkpers THEN DO:
      ASSIGN
      BRW_TID:HIDDEN = TRUE.
      BRW_VTID:HIDDEN = TRUE.
      DISABLE BTN_GODM BTN_OVER BTN_BACK FBTN_VISA FBTN_VISAM TOG_FARDIG WITH FRAME {&FRAME-NAME}.
      FILL-IN-INGENGOD:HIDDEN = FALSE.
   END.
   ELSE DO:    
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-REGIS TOG_FARDIG FILL-IN-INGENGOD 
   ...
   ELSE DO:        
      ...
      DO WHILE antal_raknare LE antal_valda :
         ...
         IF vgodmarkpers.GODKAND = "" THEN DO:         
            ...
            FIND FIRST godmarkpers WHERE godmarkpers.PERSONALKOD = vgodmarkpers.PERSONALKOD NO-ERROR.
