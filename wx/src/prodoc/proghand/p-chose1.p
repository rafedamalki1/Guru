...
REPEAT COUNTER = 1 TO 5:
    DISPLAY cust-num name address city postal-code WITH FRAME cust-frame.
...
REPEAT:
    STATUS DEFAULT "Use up and down arrows. Enter C to create, D to delete".
   ...
   THEN DO:              /* Open a space in the frame. */
	...
	CREATE customer.
	UPDATE cust-num name address city postal-code WITH FRAME cust-frame.
   ...
   END.
	...
	WHILE FRAME-LINE(cust-frame) < FRAME-DOWN(cust-frame).
	    ...
	    DOWN WITH FRAME cust-frame.
	    IF INPUT cust-num = ""
	    THEN DISPLAY cust-num name address city postal-code WITH FRAME cust-frame.
	END.
