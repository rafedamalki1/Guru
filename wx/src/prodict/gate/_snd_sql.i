...
History:    
    ...
    28 Jan 1998  DLG      Commented out type != 10 for Oracle Version       
                          Compatability (added in 8.2C and 90A).
...
FORM 
    ...
    WITH row 4 centered
    ATTR-SPACE overlay side-labels
...
if   batch_mode 
 ...
 then do: /* we need to open the output to the log-file */
  assign
...
case l_edbtyp:
  ...
  when "SYBASE"    then assign
    separate_term   = true 
    separate_drop   = true
    drop_indicator  = "Note To Myself"
    skip_use_clause = true.
  when "SYBASE-10" then assign
    separate_term   = true 
    separate_drop   = true
    drop_indicator  = "Note To Myself"
    change_chained_mode = true
    skip_use_clause = true.
  when "MS SQL Server" then assign
    ...
    skip_use_clause = true
    comit1_per_stmt = FALSE.
  when "DB2" then assign
    separate_term   = false 
    separate_drop   = false
    change_chained_mode = false
    skip_use_clause = true
    add_cr          = false
    comit1_per_stmt = true
    drop_terminator = true
    clear_buffers   = false .
  when "DB2-DRDA" then assign
    separate_term   = false 
    separate_drop   = false
    change_chained_mode = false
    skip_use_clause = true
...
REPEAT ON STOP UNDO, LEAVE:  /* repeat reading lines from sql-file */
    ...
    REPEAT:  /* get an entire sql statement */
      ...
      else if (oracle_drop = TRUE)
       and    (tmp_line begins "drop ")
       then do:
      ...
      if clear_buffers
       then for each DICTDBG.proc-text-buffer
        ...
        if check_for_drop
         then assign 
            check_for_drop = false
            dropit         = ( index(DICTDBG.proc-text-buffer.proc-text
                                    ,drop_indicator
                                    )   <>  0
                             ).
