...
PROCEDURE pdf_new :
  ...
  /* igc - don't need this here 
  IF NOT VALID-HANDLE(h_Zlib) THEN DO:
    &IF PROVERSION BEGINS "8" &THEN
      RUN {&PDFDIR}zlib8.p PERSISTENT SET h_zlib (INPUT "{&PDFDIR}", OUTPUT h_zlibbind).
    &ELSE
      RUN {&PDFDIR}zlib.p PERSISTENT SET h_zlib (INPUT "{&PDFDIR}", OUTPUT h_zlibbind).
    &ENDIF
  END.
  */
  ...
  IF OPSYS = "UNIX" THEN
    OS-COMMAND SILENT VALUE("rm " + SESSION:TEMP-DIR + pdfStream + "*.txt").
  ELSE
...
PROCEDURE pdf_text_to:
  ...
  /* igc - DEFINE VARIABLE L_LeftMargin   AS CHARACTER NO-UNDO. */
...
FUNCTION pdf_get_wrap_length RETURNS INTEGER ( INPUT pdfStream   AS CHARACTER,
  ...
  DO i = 1 TO NUM-ENTRIES(pdfText,"|"):
    ...
    /* IF PDF_text_width(pdfStream,TRIM(v-thisline)) >= (pdfWidth * v-fontSize) */
...
PROCEDURE pdf_init_param:  /* PRIVATE */
  ...
  RUN pdf_set_PaperType(pdfStream,"A4").  /* This also default the PageWidth
                                             and PageHeight Parameters */ /* det var letter här /ELPKL  */
...
PROCEDURE pdf_load_images: /* PRIVATE */
  ...
  &IF PROVERSION BEGINS "8" &THEN
    DEFINE VARIABLE m_EncryptKey  AS RAW NO-UNDO.
    LENGTH(mImage) = 0. /* Un-Initialize Memory */
  &ELSE
    DEFINE VARIABLE m_EncryptKey  AS MEMPTR NO-UNDO.
  ...
  FOR EACH TT_pdf_image WHERE TT_pdf_image.obj_stream = pdfStream:
    ...
    IF pdf_get_parameter(pdfStream,"Compress") = "TRUE" THEN DO:
      ...
      decompressfile( SESSION:TEMP-DIR + TT_pdf_font.font_name + ".cmp",
                      SESSION:TEMP-DIR + TT_pdf_font.font_name + ".dmp").
      */
...
PROCEDURE pdf_load_external: /* PRIVATE */
  ...
  FOR EACH TT_pdf_external WHERE TT_pdf_external.obj_stream = pdfStream:
    ...
    */
    ...
    IF TT_pdf_external.ext_rotate = 90 THEN
      PUT STREAM S_pdf_inc UNFORMATTED
        "/Matrix [0 -1 1 0 0 " pdf_PageHeight(pdfStream) "]" {&pdfSKIP}
        "/BBox [0 0 " pdf_PageHeight(pdfStream) " " pdf_PageWidth(pdfStream) "]" {&pdfSKIP}.
    ELSE
      PUT STREAM S_pdf_inc UNFORMATTED
    ...
    ELSE DO: 
      DO WHILE TRUE:
        IF iFileSize >= 15000 THEN
          iMemSize = 15000.
        ELSE
          iMemSize = iFileSize.

        SET-SIZE(mHolder) = 0.
        SET-SIZE(mHolder) = iMemSize.

        cContent = GET-BYTES(mFile, iPointer, iMemSize).
        FOR EACH TT_Resource 
           WHERE TT_Resource.obj_stream = pdfStream
             AND TT_Resource.res_type   = "Xobject"
             AND TT_Resource.res_old   <> "":
          ...
          IF "~/" + TT_resource.pdf_id + STRING(TT_resource.page_id ) = TT_pdf_external.ext_tag THEN DO:
            ...
            SET-SIZE(mHolder) = 0.
            SET-SIZE(mHolder) = LENGTH(cContent).
            PUT-STRING(mHolder,1, LENGTH(cContent)) = cContent.
          END.
        END. /* each Resource */

        IF GET-SIZE(mTotal) > 0 THEN DO:

          SET-SIZE(mTemp) = 0.
          SET-SIZE(mTemp) = GET-SIZE(mTotal).
          mTemp = mTotal.
          SET-SIZE(mTotal) = 0.
          SET-SIZE(mTotal) = GET-SIZE(mHolder) + GET-SIZE(mTemp).
          PUT-BYTES(mTotal,1) = GET-BYTES(mTemp,1, GET-SIZE(mTemp)).
          PUT-BYTES(mTotal,1 + GET-SIZE(mTemp)) = GET-BYTES(mHolder,1, GET-SIZE(mHolder)).
    
        END.
        ELSE DO:
          SET-SIZE(mTotal) = GET-SIZE(mHolder).
          mTotal = mHolder.
        END.

        iFileSize = iFileSize - 15000.
        iPointer = iPointer + 15000.
...
PROCEDURE pdf_reset_all: 
  ...
  FOR EACH TT_pdf_ext:
    IF OPSYS = "UNIX" THEN
      OS-COMMAND SILENT VALUE("rm " + SESSION:TEMP-DIR + TT_pdf_ext.pdf_id + "*.txt").
    ELSE
...
PROCEDURE pdf_close :
  ...
  */
  ...
  /* To Debug page content - comment this code out 
     Delete any pre-existing temp files for stream */
  IF OPSYS = "UNIX" THEN
    OS-COMMAND SILENT VALUE("rm " + SESSION:TEMP-DIR + TT_pdf_stream.obj_UniqueID + "*.txt").
  ELSE
...
PROCEDURE ChangePageText: /* PRIVATE */
  ...
  DEFINE VARIABLE iDiffLen  AS INTEGER NO-UNDO.
  */
  ...
  /*
  iDiffLen = LENGTH(pdfFromText) - LENGTH(pdfToText).
  IF iDiffLen > 0 THEN
    cFill = FILL(" ",iDiffLen).
  */
  ...
  message "pause" SKIP
          SESSION:TEMP-DIR 
                      + B_TT_stream.obj_UniqueID  
                      + "-Content-" 
                      + STRING(pdfPage) + ".txt" view-as alert-box.
  */
END. /* ChangePageText */
