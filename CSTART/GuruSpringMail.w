...
DEFINE VARIABLE app_server_info14 AS CHARACTER NO-UNDO.
...
&Scoped-Define ENABLED-OBJECTS TOG_LOS FILL-IN_ANVANDARE BTN_LOSENMAIL ~
FILL-IN_AV-LOSEN BTN_LOSEN BTN_OK BTN_AVS 
...
DEFINE BUTTON BTN_LOSEN 
     LABEL "Byt lösenord!":L 
     SIZE 14 BY 1.

DEFINE BUTTON BTN_LOSENMAIL 
     LABEL "Begär nytt lösenord":L 
     SIZE 14 BY 1.
...
DEFINE FRAME FRAME-C
     ...
     BTN_LOSENMAIL AT ROW 4.5 COL 48.75
     FILL-IN_AV-LOSEN AT ROW 6.42 COL 19.25 COLON-ALIGNED BLANK 
     BTN_LOSEN AT ROW 6.5 COL 48.75 WIDGET-ID 2
...
ON CHOOSE OF BTN_LOSEN IN FRAME FRAME-C /* Byt lösenord! */
DO:   
   RUN nytt_UI (INPUT "Bytlösen").
...
&Scoped-define SELF-NAME BTN_LOSENMAIL
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BTN_LOSENMAIL GURUW
ON CHOOSE OF BTN_LOSENMAIL IN FRAME FRAME-C /* Begär nytt lösenord */
DO:   
  
   app_server_info14 = Guru.Konstanter:AppSpringSet[14].
   Guru.Konstanter:AppSpringSet[14] = "Rmail".
   RUN nytt_UI (INPUT "Mail").
   Guru.Konstanter:AppSpringSet[14] = app_server_info14.
   IF musz = TRUE THEN DO:
      musz = FALSE.
      APPLY "ENTRY" TO FILL-IN_ANVANDARE IN FRAME {&FRAME-NAME}.
      RETURN NO-APPLY.
   END.
   ELSE DO:
      APPLY "ENTRY" TO FILL-IN_AV-LOSEN IN FRAME {&FRAME-NAME}.
      RETURN NO-APPLY.
   END.   
...
/*
   ...
   */
   ...
   status-ok = BTN_LOSENMAIL:MOVE-AFTER-TAB-ITEM(my1hand).
   my1hand = BTN_LOSENMAIL:HANDLE IN FRAME {&FRAME-NAME}.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE TOG_LOS FILL-IN_ANVANDARE BTN_LOSENMAIL FILL-IN_AV-LOSEN BTN_LOSEN 
         BTN_OK BTN_AVS 
      WITH FRAME FRAME-C IN WINDOW GURUW.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
   DEFINE INPUT  PARAMETER bytmail AS CHARACTER NO-UNDO.
   DEFINE VARIABLE AppSpringFel AS CHARACTER NO-UNDO.
   ASSIGN
   ...
   END.
   IF FILL-IN_ANVANDARE = "" THEN DO:
      MESSAGE "Du måste ange någon användare!" VIEW-AS ALERT-BOX.       
      musz = TRUE.       
      RETURN.
   END.
   IF bytmail = "Mail" THEN DO:
      IF Guru.Konstanter:AppSpringSet[5] NE FILL-IN_ANVANDARE THEN DO:
         MESSAGE "Du kan inte begära nytt lösen för: " FILL-IN_ANVANDARE VIEW-AS ALERT-BOX.       
         musz = TRUE.       
         RETURN.
      END.
   ...
   IF AppSpringFel = {LOSENKOLLFEL13.I} OR AppSpringFel = {LOSENKOLLFEL14.I} OR AppSpringFel = {LOSENKOLLFEL15.I} THEN DO:
      MESSAGE AppSpringFel
      VIEW-AS ALERT-BOX.   
   END.
   IF AppSpringFel = {LOSENKOLLFEL13.I} THEN DO:
      MESSAGE AppSpringFel
      VIEW-AS ALERT-BOX.   
      QUIT.
   END.   
