...
{OMRTEMPW.I}
DEFINE SHARED TEMP-TABLE kompbettemp
...
DEFINE VARIABLE FILL-IN_ACCKOMP AS DECIMAL FORMAT "->>9.99" INITIAL 0 
     LABEL "Kompsaldo" 
     VIEW-AS FILL-IN 
     SIZE 9 BY 1.
...
DEFINE FRAME DIALOG-3
     ...
     FILL-IN_ACCKOMP AT ROW 7.17 COL 13 COLON-ALIGNED
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         TITLE "Ny/ändra komp justering":L.
...
/* SETTINGS FOR FILL-IN FILL-IN-TEXT IN FRAME DIALOG-3
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN FILL-IN_ACCKOMP IN FRAME DIALOG-3
   NO-DISPLAY NO-ENABLE                                                 */
ASSIGN 
       FILL-IN_ACCKOMP:HIDDEN IN FRAME DIALOG-3           = TRUE.
...
"personaltemp.PERSONALKOD" "Enhet/Sign" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.personaltemp.FORNAMN
"personaltemp.FORNAMN" "Förnamn" ? "character" ? ? ? ? ? ? no ? no no "29.75" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.personaltemp.EFTERNAMN
"personaltemp.EFTERNAMN" "Efternamn" ? "character" ? ? ? ? ? ? no ? no no "31.25" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON END-ERROR OF FRAME DIALOG-3 /* Ny/ändra komp justering */
DO:
  {muswait.i}
   musz = TRUE. 
END.
...
DO:
   ...
   ELSE DO:      
      ...
      NO-LOCK NO-ERROR.      
   END.   
   FIND FIRST kompbettemp WHERE kompbettemp.PERSONALKOD = FILL-IN-PKOD AND kompbettemp.DATUM = FILL-IN_DATUM
   AND RECID(kompbettemp) NE brec NO-LOCK NO-ERROR.
   IF AVAILABLE kompbettemp THEN DO:
   ...
   AND RECID(kompbettemp) NE brec NO-LOCK NO-ERROR.
   IF AVAILABLE kompbettemp THEN DO:
      MESSAGE "Kompsaldo finns redan upplagt för senare datum " kompbettemp.DATUM VIEW-AS ALERT-BOX.   
      status-mus2 = SESSION:SET-WAIT-STATE("").
   ...
   IF AVAILABLE felmeddtemp THEN DO:
      ...
      RETURN NO-APPLY.      
   END.   
   FIND kompbettemp WHERE RECID(kompbettemp) = brec NO-ERROR.
   hjtim = kompbettemp.TIMMAR.
   ASSIGN 
   kompbettemp.PERSONALKOD = FILL-IN-PKOD
   kompbettemp.DATUM = FILL-IN_DATUM
   kompbettemp.TIMMAR = FILL-IN_TIMMAR
   kompbettemp.ANVANDARE = Guru.Konstanter:globanv.
   IF hjvart = "NYA" THEN DO:
      kompbettemp.FREC = ?.
   END.
   IF Guru.Konstanter:appcon THEN DO:
      RUN KOMPUTBETH.P ON Guru.Konstanter:apphand TRANSACTION DISTINCT
      (INPUT 2,INPUT kompbettemp.FREC,INPUT hjtim,INPUT-OUTPUT TABLE kompbettemp).
   END.
   ELSE DO:
      RUN KOMPUTBETH.P 
      (INPUT 2,INPUT kompbettemp.FREC,INPUT hjtim,INPUT-OUTPUT TABLE kompbettemp).
   END.              
   FIND FIRST kompbettemp WHERE kompbettemp.PERSONALKOD = FILL-IN-PKOD AND
   kompbettemp.DATUM = FILL-IN_DATUM AND kompbettemp.TIMMAR = FILL-IN_TIMMAR NO-LOCK NO-ERROR.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ...
   FIND kompbettemp WHERE RECID(kompbettemp) = brec NO-LOCK NO-ERROR.
   IF kompbettemp.PERSONALKOD = "" THEN DO:
      ASSIGN FILL-IN-PKOD = ""
      FILL-IN_DATUM = TODAY
      FILL-IN_TIMMAR = 0.
   END.
   ELSE DO:
      ASSIGN FILL-IN-PKOD = kompbettemp.PERSONALKOD
      FILL-IN_DATUM= kompbettemp.DATUM
      FILL-IN_TIMMAR = kompbettemp.TIMMAR.      
   END.   
   ...
   IF kompbettemp.PERSONALKOD NE "" THEN DO:
      FIND FIRST personaltemp WHERE personaltemp.PERSONALKOD = kompbettemp.PERSONALKOD NO-LOCK NO-ERROR.
      IF AVAILABLE(personaltemp) THEN DO:         
