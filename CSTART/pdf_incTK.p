...
$Archive:: /www-helpdesk/webspeed/wrk_dir/pro/pdf_in $
$Author:: Patrik                                     $
$Date:: 05-04-07 15:48                               $
$Revision:: 12                                       $
...
*****************  Version 12  *****************
User: Patrik       Date: 05-04-07   Time: 15:48
Updated in $/www-helpdesk/webspeed/wrk_dir/pro
uppdaterat och tagit bort lite utav G.Cs debugmedd. =)

*****************  Version 11  *****************
User: Patrik       Date: 05-03-29   Time: 13:46
Updated in $/www-helpdesk/webspeed/wrk_dir/pro
Sett till att felmedd. faktiskt hamnar i loggfilen och
inte försvinner i tomma intet.
...
*****************  Version 9  *****************
User: Therese      Date: 05-03-04   Time: 10:48
Updated in $/www-helpdesk/webspeed/wrk_dir/pro
Ny version
...
DEFINE TEMP-TABLE TT_pdf_external NO-UNDO
    ...
    FIELD ext_width     AS INTEGER
    FIELD ext_height    AS INTEGER
INDEX obj_image AS PRIMARY
      obj_stream
      ext_tag.
...
/*
DEFINE VARIABLE pdf_DoingText       AS LOGICAL   NO-UNDO.
DEFINE VARIABLE pdf_DoingGraphic    AS LOGICAL   NO-UNDO.
*/
...
&IF PROVERSION BEGINS "8" &THEN
  DEFINE VARIABLE mContent              AS RAW NO-UNDO.
  DEFINE VARIABLE mHolder               AS RAW NO-UNDO.
&ELSE
...
PROCEDURE pdf_new :
  ...
  IF NOT VALID-HANDLE(h_Zlib) THEN DO:
  ...
  TT_pdf_stream.obj_UniqueID = ENCODE(STRING(TODAY) + STRING(TIME) + TT_pdf_stream.obj_file).

  /* Delete any pre-existing temp files for stream */
  IF OPSYS = "UNIX" THEN
    OS-COMMAND SILENT VALUE("rm " + ".\" + pdfStream + "*.txt").
  ELSE
    OS-COMMAND SILENT VALUE("del " + ".\" + pdfStream + "*.txt").
...
PROCEDURE pdf_set_Page : /* PRIVATE */
  DEFINE INPUT PARAMETER pdfStream          AS CHARACTER NO-UNDO.
  DEFINE INPUT PARAMETER pdfValue           AS INTEGER NO-UNDO.
...
PROCEDURE pdf_text_charxy :
  ...
  RUN OutputTextContent(pdfStream,
                        "TEXTXY",
                          CHR(10) + L_Font + " " + L_PointSize + " Tf" + CHR(10)
                                    + L_color
...
PROCEDURE pdf_text_boxed_xy :
  ...
  RUN OutputTextContent(pdfStream,
    ...
    /* Add the dynamic justification tags
   IF LOOKUP(pdfJustify,"Right,Center") GT 0 THEN DO:
     /* Initialize dynamic object justification parameters
        delimited by chr(1) */
        tt_pdf_content.obj_type = tt_pdf_content.obj_type + chr(1) +
                                  pdfJustify + chr(1) +
                                  l_font + chr(1) +
                                  l_pointSize + chr(1) +
                                  string(pdfColumn) + chr(1) +
                                  string(pdfColumn + pdfWidth) + chr(1) +
                                  L_orig_text.
   END.
  */
...
PROCEDURE pdf_text_to:
  ...
  RUN pdf_replace_text (INPUT-OUTPUT pdfText).
    ...
    RUN OutputTextContent(pdfStream,
                          ...
                          + "(" + FILL(" ", pdfColumn - LENGTH(L_orig_text, "character":u) - 1) + pdfText + ") Tj ").
    ...
    ELSE DO:
      L_X = pdf_text_width(pdfStream,FILL(" ",pdfColumn))
  ...
  CASE pdfAlign:
    WHEN "RIGHT" THEN DO:
      RUN OutputTextContent(pdfStream,
                            ...
                            + L_Font + " " + L_PointSize + " Tf" + CHR(10)
                                        + L_color
                                        + "1 0 0 1 " + dec2string(pdfX - L_Width) + " "
                            + STRING(pdfY) + " Tm" + CHR(10)
                            + "(" + pdfText + ") Tj").
...
/* ---- Updated pdf_get_wrap_length Function ---- */
FUNCTION pdf_get_wrap_length RETURNS INTEGER ( INPUT pdfStream   AS
CHARACTER,
                                               INPUT pdfText AS CHARACTER,
...
pdf_text_width(pdfstream,FILL("W",pdfWidth))).
  ...
  DO i = 1 TO NUM-ENTRIES(pdfText,"|"):
    ...
    /* IF PDF_text_width(pdfStream,TRIM(v-thisline)) >= (pdfWidth *
v-fontSize) */
    IF pdf_getnumfittingchars(pdfStream,
                              TRIM(v-thisline),
                              0,
                              pdf_text_width(pdfstream,TRIM(v-thisline))) >
v-maxwidth
    THEN DO:
...
PROCEDURE pdf_Content: /* PRIVATE */
  ...
  &IF PROVERSION BEGINS "8" &THEN
    DEFINE VARIABLE vSource       AS RAW NO-UNDO.
    DEFINE VARIABLE vDest         AS RAW NO-UNDO.
  &ELSE
  ...
  DO L_Loop = 1 TO pdf_Page(pdfStream):
    ...
    FILE-INFO:FILE-NAME = ".\" + TT_pdf_stream.obj_UniqueID  + "-Content-" + STRING(L_Page) + ".txt".

    &IF PROVERSION BEGINS "8" &THEN
      LENGTH(mContent) = 0.
      LENGTH(mContent) = GetFileSize().
    &ELSE
      SET-SIZE(mContent) = 0.
  ...
  OS-DELETE VALUE(".\" + TT_pdf_stream.obj_UniqueID + "-C.txt").
...
PROCEDURE pdf_definition: /* PRIVATE */
  ...
  PUT STREAM S_pdf_inc UNFORMATTED
      "/Rotate " STRING(TT_pdf_page.page_rotate) {&pdfSKIP}.
...
PROCEDURE pdf_Load_fonts : /* PRIVATE */
  ...
  &IF PROVERSION BEGINS "8" &THEN
    DEFINE VARIABLE mFont         AS RAW NO-UNDO.
    DEFINE VARIABLE m_EncryptKey  AS RAW  NO-UNDO.
  &ELSE
  ...
  FOR EACH TT_pdf_font WHERE TT_pdf_font.obj_stream = pdfStream
    ...
    IF TT_pdf_font.font_embed THEN DO:
      ...
      IF pdf_get_parameter(pdfStream,"Compress") = "TRUE" THEN DO:
        ...
        compressfile( TT_pdf_font.font_file,
                      ".\" + TT_pdf_font.font_name + ".cmp").

        FILE-INFO:FILE-NAME = ".\" + TT_pdf_font.font_name + ".cmp".
      END.
    END. /* Embed Font */
    ...
    PUT STREAM S_pdf_inc UNFORMATTED
        ...
        "/Ascent 878" {&pdfSKIP}
        "/CapHeight 878" {&pdfSKIP}
        "/Descent 0" {&pdfSKIP}
        "/Flags 32" {&pdfSKIP}
        "/FontBBox[0 0 797 878]" {&pdfSKIP}
        "/FontName/Code#2039" {&pdfSKIP}
        "/ItalicAngle 0" {&pdfSKIP}
        "/StemV 87" {&pdfSKIP}
        "/XHeight 579" {&pdfSKIP}.
        **/
        "<< /Type /FontDescriptor" {&pdfSKIP}
    ...
    IF TT_pdf_font.font_embed THEN DO:
      ...
      &IF PROVERSION BEGINS "8" &THEN
        LENGTH(mFont) = GetFileSize(). /* Initialize Memory */
      &ELSE
      ...
      /**
      INPUT FROM VALUE(SEARCH(FILE-INFO:FILE-NAME)) NO-ECHO NO-CONVERT BINARY.
        IMPORT mFont.
      INPUT CLOSE.
      **/
      ...
      IF pdf_get_parameter(pdfStream,"Compress") = "TRUE" THEN
        OS-DELETE VALUE(".\" + TT_pdf_font.font_name + ".cmp").
...
PROCEDURE pdf_load_images: /* PRIVATE */
  ...
  &IF PROVERSION BEGINS "8" &THEN
    DEFINE VARIABLE m_EncryptKey  AS RAW NO-UNDO.
    DEFINE VARIABLE mImage        AS RAW  NO-UNDO.
  &ELSE
  DEFINE VARIABLE m_EncryptKey  AS MEMPTR NO-UNDO.
  ...
  FOR EACH TT_pdf_image WHERE TT_pdf_image.obj_stream = pdfStream:
    ...
    IF pdf_get_parameter(pdfStream,"Compress") = "TRUE" THEN DO:
      compressfile( TT_pdf_image.image_file,
                    ".\" + TT_pdf_image.image_name + ".cmp").

      /* Debug -- decompress the file to see if we get the same file size etc
      decompressfile( ".\" + TT_pdf_font.font_name + ".cmp",
                      ".\" + TT_pdf_font.font_name + ".dmp").
      */

      FILE-INFO:FILE-NAME = ".\" + TT_pdf_image.image_name + ".cmp".
    END.
    ELSE
    ...
    &IF PROVERSION BEGINS "8" &THEN
      LENGTH(mImage) = GetFileSize(). /* Initialize Memory */
    &ELSE
    ...
    &IF PROVERSION BEGINS "8" &THEN
      LENGTH(mImage) = 0. /* Release memory */
    &ELSE
    ...
    /* Remove Compressed File */
    IF pdf_get_parameter(pdfStream,"Compress") = "TRUE" THEN
      OS-DELETE VALUE(".\" + TT_pdf_image.image_name + ".cmp").
  ...
  FOR EACH TT_pdf_external WHERE TT_pdf_external.obj_stream = pdfStream:
    ...
    */
    PUT STREAM S_pdf_inc UNFORMATTED
        ...
        "/BBox [0 0 " TT_pdf_external.ext_width " " TT_pdf_external.ext_height "]" {&pdfSKIP}
        "/Resources <<" {&pdfSKIP}.
        /*
        "/ColorSpace /DeviceRGB" {&pdfSKIP}
        */
...
PROCEDURE pdf_reset_all:
  ...
  &ELSE
    ...
    FOR EACH TT_pdf_bookmark:
      DELETE TT_pdf_bookmark.
    END.

    FOR EACH TT_pdf_ReplaceTxt:
    ...
    FOR EACH TT_pdf_tool_param:
      DELETE TT_pdf_tool_param.
    END.

    FOR EACH TT_pdf_external:
  ...
  FOR EACH TT_pdf_ext:
    IF OPSYS = "UNIX" THEN
      OS-COMMAND SILENT VALUE("rm " + ".\" + TT_pdf_ext.pdf_id + "*.txt").
    ELSE
      OS-COMMAND SILENT VALUE("del " + ".\" + TT_pdf_ext.pdf_id + "*.txt").
...
------------------------------------------------------------------------------*/
  ...
  DO i = 1 TO NUM-ENTRIES(pdfText,"|"):
    ...
    /* IF LENGTH(v-thisline,"character":u) >= v-maxwidth THEN DO: */
...
PROCEDURE pdf_close :
  ...
  &IF PROVERSION BEGINS "8" &THEN
    RUN pdf_set_parameter(pdfStream,"Compress","FALSE").
    RUN pdf_set_parameter(pdfStream,"Encrypt","FALSE").
  &ENDIF
  ...
  /* IF pdf_DoingText THEN */
  IF TT_pdf_stream.obj_DoingText THEN
    PUT STREAM S_pdf_out UNFORMATTED "ET" {&pdfskip}.
  /* ELSE IF pdf_DoingGraphic THEN */
  ...
  OUTPUT STREAM S_pdf_inc TO VALUE( TT_pdf_stream.obj_file ) UNBUFFERED NO-CONVERT.
    ...
    IF pdf_get_parameter(pdfStream,"Encrypt") = "TRUE" THEN DO:
      ...
      RUN pdfencryptTK.p PERSISTENT SET h_PDF-Encrypt (INPUT "pro/").

      RUN BuildDocID IN h_PDF-encrypt (INPUT  TT_pdf_stream.obj_UniqueID,
      ...
      RUN DetermineOwnerKey IN h_PDF-Encrypt (INPUT TT_pdf_stream.obj_UniqueID,
                                              INPUT TT_pdf_stream.obj_user,
                                              INPUT-OUTPUT TT_pdf_stream.obj_master) .

      RUN DetermineUserKey  IN h_PDF-Encrypt (INPUT TT_pdf_stream.obj_UniqueID,
                                              INPUT TT_pdf_stream.obj_ID,
                                              INPUT INT(pdf_get_parameter(pdfStream,"EncryptKey")),
  ...
  IF VALID-HANDLE(h_Zlibbind) THEN DO:
    RUN ReleaseZlib IN h_Zlibbind.
    DELETE PROCEDURE h_Zlibbind.
  END.

  IF VALID-HANDLE(h_Zlib) THEN
    DELETE PROCEDURE h_Zlib.

  /* To Debug page content - comment this code out
     Delete any pre-existing temp files for stream */
  IF OPSYS = "UNIX" THEN
    OS-COMMAND SILENT VALUE("rm " + ".\" + TT_pdf_stream.obj_UniqueID + "*.txt").
  ELSE
    OS-COMMAND SILENT VALUE("del " + ".\" + TT_pdf_Stream.obj_UniqueID + "*.txt").
...
PROCEDURE pdf_set_parameter:
  ...
  IF LOOKUP(pdfParameter,L_valid_params) = 0 THEN DO:
  ...
  CASE pdfParameter:
    ...
    WHEN "EncryptKey" THEN DO:
      ...
      IF pdfValue <> "40" /* AND pdfValue <> "128" */ THEN
        ASSIGN L_Error    = TRUE
               L_ErrorMsg = "Only a value of 40 is allowed for '" + pdfParameter + "' Parameter!".
    END.
...
PROCEDURE pdf_merge_stream_content : /* PRIVATE */
    ...
    /* Also close the Parent Stream Display path - Text or Graphic */
    IF B_TT_pdf_Stream.obj_DoingGraphic THEN
      PUT STREAM S_pdf_out UNFORMATTED "Q" {&pdfskip}.
    ELSE IF B_TT_pdf_Stream.obj_DoingText THEN
      PUT STREAM S_pdf_out UNFORMATTED "ET" {&pdfskip}.
...
PROCEDURE OutputMemPtr: /* PRIVATE */
  &IF PROVERSION BEGINS "8" &THEN
    DEFINE INPUT PARAMETER pMemPtr AS RAW NO-UNDO.
  &ELSE
  DEFINE INPUT PARAMETER pMemPtr AS MEMPTR NO-UNDO.
  &ENDIF
...
PROCEDURE ConvChar2MemPtr : /* PRIVATE */
  ...
  &IF PROVERSION BEGINS "8" &THEN
    DEFINE VARIABLE mWork     AS RAW NO-UNDO.
  &ELSE
  ...
  &IF PROVERSION BEGINS "8" &THEN
    LENGTH(mWork) = 0.
    LENGTH(mWork) = LENGTH(pText, "character":u).
  &ELSE
  SET-SIZE(mWork) = 0.
  ...
  &IF PROVERSION BEGINS "8" &THEN
    LENGTH(mContent) = 0.
    LENGTH(mContent) = LENGTH(mWork) + 1.
  &ELSE
  SET-SIZE(mContent) = 0.
...
PROCEDURE pdf_tool_add :
  ...
  IF NOT VALID-HANDLE(h_PDF-Tool) THEN
    RUN pdftoolTK.p PERSISTENT SET h_PDF-Tool
...
PROCEDURE pdf_set_tool_parameter :
  ...
  CASE TT_pdf_tool.tool_type:
    WHEN "TABLE" THEN DO:
      ...
      IF LOOKUP(pdfToolParam,"ColumnWidth,ColumnX,MaxX,MaxY,Outline") > 0 THEN DO:
...
PROCEDURE pdf_load_template:
  ...
  IF NOT VALID-HANDLE(h_PDF-template) THEN
    RUN pro/pdfTemplate.p PERSISTENT SET h_PDF-Template
        (INPUT THIS-PROCEDURE:HANDLE).
...
PROCEDURE GetFileContent:  /* PRIVATE */
  ...
  &IF PROVERSION BEGINS "8" &THEN
    DEFINE INPUT-OUTPUT PARAMETER pMemPtr AS RAW NO-UNDO.
  &ELSE
  ...
  &IF PROVERSION BEGINS "8" &THEN
    LENGTH(pMemPtr) = i_size.
    DO i_cnt = 1 TO i_size:
      READKEY.
      PUT-BYTE(pMemPtr,i_cnt) = LASTKEY.
    END.
  &ELSE
...
PROCEDURE PutFileContent: /* PRIVATE */
  ...
  &IF PROVERSION BEGINS "8" &THEN
    i_size = LENGTH(pMemPtr).
    LENGTH(r_Byte) = 1.

    DO L_Loop = 1 TO i_Size:
      PUT-BYTE(r_Byte,1) = GET-BYTE(pMemPtr,L_Loop).
      PUT STREAM S_PDF_inc CONTROL r_Byte.
    END.
...
PROCEDURE pdf_open_PDF:
  ...
  IF AVAIL TT_pdf_ext THEN DO:
    RUN pdf_error("pdf_open_PDF","PDF ID (" + pdfID + ") has been used already").
    RETURN.
  END.
  ...
  IF AVAIL TT_pdf_ext THEN DO:
    RUN pdf_error("pdf_open_PDF","PDF File Name (" + pdfName + ") has already been"
                                + " opened with PDF ID = " + TT_pdf_ext.pdf_ID).
    RETURN.
  END.

  RUN pdfextractTK.p (INPUT pdfStream,
                             INPUT pdfName,
                             INPUT pdfID).
...
PROCEDURE pdf_use_PDF_page:
  ...
  THEN DO:
    ...
    ASSIGN TT_pdf_external.obj_stream  = pdfStream
           TT_pdf_external.ext_tag     = "~/" + pdfID + STRING(pdfPage)
           TT_pdf_external.ext_file    = ".\" + pdfID + STRING(pdfPage) + ".txt".
           TT_pdf_external.ext_height  = pdf_PageHeight(pdfstream).
           TT_pdf_external.ext_width   = pdf_PageWidth(pdfstream).
  END.
...
PROCEDURE LoadExternalFonts: /* PRIVATE */
  ...
  DEFINE VARIABLE l_Desc  AS INTEGER NO-UNDO.
  DEFINE VARIABLE l_Enc   AS INTEGER NO-UNDO.

  DEFINE VARIABLE l_FoundDesc AS LOGICAL NO-UNDO.
        ...
        FOR EACH TT_Resource
          ...
          IF FIRST-OF(TT_Resource.res_text) THEN DO:
            ...
            THEN DO:
              ...
              INPUT FROM VALUE(".\" + TT_Resource.pdf_id + "-"
                ...
                REPEAT:
                  ...
                  IF INDEX(l_Text,"~/FontDescriptor " + STRING(TT_Font.desc_obj) + " " + STRING(TT_Font.desc_gen)) > 0 THEN DO:
                    ...
                    L_Desc = L_Seq.
                  END.
                  ELSE IF INDEX(l_Text,"~/Encoding " + STRING(TT_Font.enc_obj) + " " + STRING(TT_Font.enc_gen)) > 0 THEN DO:
                    L_Seq = l_Seq + 1.
                    l_Text = REPLACE(l_Text,
                                     "~/Encoding "
              ...
              IF L_FoundDesc AND L_FoundEnc THEN
                L_Seq = 2.
              ELSE
                L_Seq = 1.
              ...
              IF TT_Font.desc_obj <> 0 THEN DO:
                ObjectSequence(pdfStream, L_Desc, "FontDescriptor", 0).
                PUT STREAM S_pdf_inc UNFORMATTED
                    pdf_inc_ObjectSequence " 0 obj" {&pdfSKIP}.
              ...
              IF TT_Font.file2_obj <> 0 THEN DO:
                FILE-INFO:FILE-NAME = ".\" + TT_Resource.pdf_id + "-"
                                    + STRING(TT_Font.file2_obj) + "-"
              ...
              FIND FIRST TT_pdf_Font
                   WHERE TT_pdf_Font.obj_Stream = pdfStream
                     AND TT_pdf_Font.font_tag   = TT_Resource.res_text NO-ERROR.
              IF AVAIL TT_pdf_Font THEN
  ...
  DEFINE BUFFER B_TT_Resource FOR TT_Resource.
        ...
        FOR EACH TT_Resource
          ...
          FOR EACH B_TT_Resource
            ...
            FILE-INFO:FILE-NAME = ".\" + B_TT_Resource.pdf_id + "-"
                                + STRING(B_TT_Resource.res_obj) + "-"
...
PROCEDURE OutputTextContent: /* PRIVATE */
  ...
  IF pdf_Page(pdfStream) > 0 THEN DO:
    ...
    IF pdfType BEGINS "IMAGE" THEN DO:
      ...
      /*
      ASSIGN pdf_DoingText    = FALSE
             pdf_DoingGraphic = TRUE. */
      ASSIGN TT_pdf_stream.obj_DoingText    = FALSE
             TT_pdf_stream.obj_DoingGraphic = TRUE.
    END. /* End Image */

    /* ELSE IF pdfType BEGINS "TEXT" AND NOT pdf_DoingText THEN DO: */
    ELSE IF pdfType BEGINS "TEXT" AND NOT TT_pdf_Stream.obj_DoingText THEN DO:
      ...
      PUT STREAM S_pdf_out UNFORMATTED
                           CHR(10) + "BT" + CHR(10)
                           + "~/BF1 " + dec2string(pdf_PointSize(pdfStream)) + " Tf" + CHR(10)
                           + " 1 0 0 1 " + STRING(pdf_LeftMargin(pdfStream)) + " "
                           /* + STRING((TT_pdf_Page.Page_Height - pdf_TopMargin(pdfStream))) */
                           + STRING(pdf_TextY(pdfStream))
                           + " Tm"
                           {&pdfSKIP}.

      /*
      ASSIGN pdf_DoingGraphic = FALSE
             pdf_DoingText    = TRUE. */
      ASSIGN TT_pdf_Stream.obj_DoingGraphic = FALSE
             TT_pdf_Stream.obj_DoingText    = TRUE.
    END.

    /* ELSE IF pdf_DoingText AND NOT pdfType BEGINS "Text" THEN DO: */
    ...
    /* ELSE IF pdf_DoingGraphic AND pdfType BEGINS "TEXT" THEN DO: */
    ELSE IF TT_pdf_Stream.obj_DoingGraphic AND pdfType BEGINS "TEXT" THEN DO:
      ...
      /*
      ASSIGN pdf_DoingText    = TRUE
             pdf_DoingGraphic = FALSE. */
      ASSIGN TT_pdf_Stream.obj_DoingText    = TRUE
             TT_pdf_Stream.obj_DoingGraphic = FALSE.
    END.

    /* ELSE IF NOT pdf_DoingGraphic AND NOT pdfType BEGINS "TEXT" THEN DO: */
    ...
    FOR EACH TT_pdf_FillTxt
      ...
      FOR EACH TT_Widget
        ...
        CASE L_Align:
          WHEN "Right" THEN DO:
            IF L_Font <> "" THEN
              L_Width = pdf_text_fontwidth(pdfStream, L_Font, L_Text).
            ELSE
              L_Width = pdf_text_fontwidth(pdfStream, "Courier", L_Text).

            /*
            message TT_pdf_FillTxt.Fill_from
                    INT(ENTRY(3,TT_Widget.widget_rect," "))
                    L_Width SKIP
                    INT(ENTRY(3,TT_Widget.widget_rect," ")) - L_Width
                    view-as alert-box.
            */
            RUN OutputTextContent(pdfStream,
                                  "TEXTXY",
