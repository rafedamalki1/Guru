...
form
   ...
   SKIP({&VM_WID})
/*   "Case Insensitive"      view-as TEXT         at     2
   SKIP({&VM_WID})
   b_Field._For-xpos 	   LABEL "Position"  	colon 15 {&STDPH_FILL} */
...
form
   ...
   SKIP({&VM_WID})
   b_Field._For-scale	   LABEL "&Scale"  	       colon 20 {&STDPH_FILL}
   SKIP({&VM_WID})
   b_Field._Fld-misc1[1]   LABEL "&Precision"  	       colon 20 {&STDPH_FILL}
   SKIP({&VM_WID})
  /* b_Field._For-retrieve   LABEL "&Retrieve"  	       at    19
      	       	     	   view-as TOGGLE-BOX */ /* commented out for bug 95-01-25-021 */ 
...
do:
   run Validate_Offset 
      (INPUT input frame rdbfld b_Field._Fld-stoff,
       INPUT 0). /* why doesn't rdb have length? */
   if RETURN-VALUE = "error" then return NO-APPLY.
end.
...
on HELP of frame odbfld OR choose of s_btn_Help in frame odbfld
...
/* Copy gateway info to work file in case we need to undo changes, if
