...
&Scoped-Define ENABLED-OBJECTS RECT-1 BtnOK BtnGet BtnDone 
...
DEFINE FRAME CustFrame
     BtnOK AT ROW 18.75 COL 10
     BtnGet AT ROW 18.75 COL 34
     BtnDone AT ROW 18.75 COL 58
     RECT-1 AT ROW 18.25 COL 6
...
ON END-ERROR OF C-Win /* Materiel TempTable */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Materiel TempTable */
DO:
...
DO:
  ...
  if not valid-handle(hTableProgram) then do:
    run xTempTable.p persistent set hTableProgram no-error.    
    dynamic-function("SetTableName" in hTableProgram, "{&TableName}").
  end.
/*   DEFINE VARIABLE comQuery AS CHARACTER NO-UNDO.                                   */
/*   comQuery = " where KALKNR = " + STRING(0) + " AND LEVKOD = '" + STRING(1) + "'". */
...
/*     run GetData     in hTableProgram (FieldList,ValueList,INPUT comQuery, */
/*                                     output table-handle hTempTable).      */
  end.
...
/*     then do:                                                                  */
/*     run xTempTable.p persistent set hTableProgram.                             */
...
------------------------------------------------------------------------------*/
  ENABLE RECT-1 BtnOK BtnGet BtnDone 
