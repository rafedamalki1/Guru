...
DEFINE SHARED VARIABLE imph AS HANDLE NO-UNDO.
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-LABELS NO-COLUMN-SCROLLING SIZE 82.5 BY 20.
...
DEFINE FRAME FRAME-VINST
     ...
     BTN_SKRIV AT ROW 21.79 COL 54.5
     BTN_AVS AT ROW 21.79 COL 70
...
-------------------------------------------------------------*/
   ...
   DO TRANSACTION:   
      ...
      ELSE DO:                   
         RUN bbenmning_UI IN imph (INPUT typkod, OUTPUT TABLE bbtemp).
         FIND FIRST bbtemp NO-LOCK NO-ERROR.
         CREATE tidut.         
         ASSIGN        
         SUBSTRING(tidut.UT,1) = "RADER SOM EJ IMPORTERAS FÖR " + typkod.          
         CREATE tidut.
         ASSIGN
...
-------------------------------------------------------------*/   
    ...
    FOR EACH tidin WHERE tidin.FEL = TRUE:    
       CREATE tidut.
       ASSIGN    
       SUBSTRING(tidut.UT,1) = tidin.KTYPKOD
       SUBSTRING(tidut.UT,10) = tidin.ENR
       SUBSTRING(tidut.UT,22) = SUBSTRING(tidin.BENAMNING,1,25)
       SUBSTRING(tidut.UT,48) = SUBSTRING(tidin.ENHET,1,3)
       SUBSTRING(tidut.UT,54) = SUBSTRING(tidin.LEVKOD,1,5)
       SUBSTRING(tidut.UT,61) = STRING(tidin.ANTAL,">>>>9")
       SUBSTRING(tidut.UT,67) = tidin.F1
       SUBSTRING(tidut.UT,76) = tidin.F2
       SUBSTRING(tidut.UT,85) = tidin.F3
       SUBSTRING(tidut.UT,94) = tidin.F4
       SUBSTRING(tidut.UT,103) = tidin.F5
       SUBSTRING(tidut.UT,112) = STRING(tidin.LINKAB2,"Ja/Nej")
       SUBSTRING(tidut.UT,116) = STRING(tidin.DIAMETER,">>9")
       SUBSTRING(tidut.UT,120) = STRING(tidin.MODUL,">>>9")
       SUBSTRING(tidut.UT,125) = STRING(tidin.TYPBER2,"Ja/Nej").                  
    END.       
...
"========.===========.=========================.=====.======.=====.========.========.========.========.========.===.===.====.===".
   ...
   CREATE tidut.
   ASSIGN  
   SUBSTRING(tidut.UT,1) = SUBSTRING(bbtemp.B1,1,8)
   SUBSTRING(tidut.UT,10) = CAPS(Guru.Konstanter:genk)
   SUBSTRING(tidut.UT,22) = "BENÄMNING"
   SUBSTRING(tidut.UT,48) = "ENHET"
   SUBSTRING(tidut.UT,54) = "LEVKOD"
   SUBSTRING(tidut.UT,61) = "ANTAL"
   SUBSTRING(tidut.UT,67) = SUBSTRING(bbtemp.B2,1,8)
   SUBSTRING(tidut.UT,76) = SUBSTRING(bbtemp.B3,1,8)
   SUBSTRING(tidut.UT,85) = SUBSTRING(bbtemp.B4,1,8)
   SUBSTRING(tidut.UT,94) = SUBSTRING(bbtemp.B5,1,8)
   SUBSTRING(tidut.UT,103) = SUBSTRING(bbtemp.B6,1,8)
   SUBSTRING(tidut.UT,112) = "L/K"
   SUBSTRING(tidut.UT,116) = "DIA"
   SUBSTRING(tidut.UT,120) = "MODU"
   SUBSTRING(tidut.UT,125) = "BER".        
   CREATE tidut.    
   SUBSTRING(tidut.UT,1) = str.                                                                                        
END PROCEDURE.
