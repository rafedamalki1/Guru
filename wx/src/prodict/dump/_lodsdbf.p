...
ASSIGN
  dbfutl = SEARCH(IF OPSYS = "UNIX"  THEN "dbf"
           ELSE   IF OPSYS = "BTOS"  THEN "dbf.run"
           ELSE   IF CAN-DO("MSDOS,WIN32,OS2",OPSYS) THEN "dbf.exe"
           ELSE   ?)
  errmsg = ?
  dbffil = SEARCH(user_env[1])
  nuxi   = STRING(user_env[2] = "y","1/0").

IF OPSYS = "VMS"   THEN errmsg = new_lang[1].
ELSE IF dbfutl = ? THEN errmsg = new_lang[2].
...
ELSE IF OPSYS = "OS2"   THEN OS2  SILENT VALUE(dbfutl) VALUE(c).
ELSE IF OPSYS = "BTOS"  THEN BTOS SILENT VALUE(dbfutl) VALUE(c).
