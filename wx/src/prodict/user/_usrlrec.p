...
&ENDIF
  dot_d_file {&STDPH_FILL} FORMAT "x(80)" VIEW-AS FILL-IN SIZE {&FILLCH} BY 1 
        LABEL "Original Data File" COLON {&LINEUP}
  btn_File_d SKIP ({&VM_WIDG})

  dot_e_file {&STDPH_FILL} FORMAT "x(80)" VIEW-AS FILL-IN SIZE {&FILLCH} BY 1
         LABEL "Error File" COLON {&LINEUP}
 btn_File_e SKIP ({&VM_WIDG})
  third_file {&STDPH_FILL} FORMAT "x(80)" VIEW-AS FILL-IN SIZE {&FILLCH} BY 1
