...
IF respers.DATUM = bdatum THEN DO:
   ...
   LONFLER.RESSTART2 GE startresa AND LONFLER.DAGTYP = "UTRESA"
   USE-INDEX FLON NO-LOCK NO-ERROR.
   IF AVAILABLE LONFLER THEN DO:
      CREATE TIDREGITAB.
      ASSIGN TIDREGITAB.PERSONALKOD = PERSONALTAB.PERSONALKOD
      SUBSTRING(TIDREGITAB.PROGRAM,1,158) = "FLLON2" + STRING(TODAY) + STRING(TIME,"HH:MM") + Guru.Konstanter:globanv
...
IF respers.DATUM = avdatum THEN DO:   
   ...
   LONFLER.RESSTART2 GE slutresa AND LONFLER.DAGTYP = "HEMRESA"
   USE-INDEX FLON NO-LOCK NO-ERROR.
   IF AVAILABLE LONFLER THEN DO:
      CREATE TIDREGITAB.
      ASSIGN TIDREGITAB.PERSONALKOD = PERSONALTAB.PERSONALKOD
