...
&SCOPED-DEFINE VER_PLATFORM_WIN32_NT      2
  ...
  IF PlatformId = {&VER_PLATFORM_WIN32_NT} THEN
    ASSIGN OSstr = "Windows NT " + STRING(MajorVersion) + "." + 
                   STRING(MinorVersion).
  ELSE IF PlatformId = {&VER_PLATFORM_WIN32_WINDOWS} THEN
    CASE MinorVersion:
      WHEN 10   THEN OSstr = "Windows 98".
      OTHERWISE      OSStr = "Windows 95". 
    END CASE.
  
  pLabel1 = OSstr + " running on an " + ProcType.
  pLabel2 = "Phys. memory: " + STRING(TotPhysMem, ">>>,>>>K") + " with "
