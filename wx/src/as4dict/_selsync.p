...
form
   SKIP(1)
    ...
    "Select files to process:"               at 2
                        view-as TEXT                               
   "Files to be processed:"                     at 47 
                        view-as TEXT                               
   SKIP
   ...
   s_btn_FileRmv                                             at     33 
 &IF "{&WINDOW-SYSTEM}" <> "TTY" &THEN SKIP(4)
 &ELSE SKIP(8)
 &ENDIF
...
FIND as4dict.p__db NO-LOCK.
   ...
   FOR EACH as4dict.p__file NO-LOCK by as4dict.p__File._File-name :
     if list-contents = "" THEN
       ASSIGN list-contents = STRING(as4dict.p__File._File-name, "x(20)").
     else  
       assign list-contents = list-contents + "," + STRING(as4dict.p__File._File-name,"x(20)").
   END. 
...
DO ON ERROR UNDO,LEAVE _uploop ON ENDKEY UNDO,LEAVE _uploop 
    ...
    DO fnum = 1 to num_fls:
      assign name = SUBSTR(ENTRY(fnum, fls), 1, 32).
      IF name = "" OR name = ? THEN NEXT _fileloop.
      
      FIND as4dict.p__File WHERE as4dict.p__File._File-name = name NO-LOCK.                                                            
