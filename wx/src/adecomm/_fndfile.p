...
**
**                 pFilter
**                    Filetypes that the user can select.  The 1st file type
**                    is user as the initial filter.  Pipe character "|" is
**                    used as a delimeter character (and not commas).  Commas
**                    are used to separate filetypes as in:
**
**                        *.ico,*.bmp | *.w,*.wx | *.p,*.i
**
...
&Scoped-Define ENABLED-OBJECTS Container TextDisplay FileNameField ~
ImageDisplay FileList FileType PreviewType PreviewFile DirList ~
EditPathButton BrowseButton FullFileNameField FileNameLabel 
...
DEFINE FRAME f_findfile
     TextDisplay AT ROW 2.19 COL 34.2 NO-LABEL
     FileNameField AT ROW 2.24 COL 2.4 NO-LABEL
     FileList AT ROW 3.52 COL 2.4 NO-LABEL
     FileType AT ROW 11.33 COL 2.4 NO-LABEL
     PreviewType AT ROW 12.29 COL 47.8 NO-LABEL
     PreviewFile AT ROW 12.33 COL 34.2
     DirList AT ROW 13.38 COL 2.4 NO-LABEL
     EditPathButton AT ROW 13.38 COL 34.2
     BrowseButton AT ROW 13.38 COL 51.2
     FullFileNameField AT ROW 1 COL 33 COLON-ALIGNED NO-LABEL
     FileNameLabel AT ROW 1.33 COL 2.4 NO-LABEL
     Container AT ROW 2.14 COL 34.2
     ImageDisplay AT ROW 2.33 COL 34.8
     "File &Type:" VIEW-AS TEXT
          SIZE 9.2 BY .67 AT ROW 10.52 COL 2.4
     "&Directory:" VIEW-AS TEXT
          SIZE 9 BY .67 AT ROW 12.62 COL 2.4
     SPACE(71.73) SKIP(1.23)
...
DO:
  ...
  */
  ...
  /*
  ** Upon exit, then make these assigns to pass to the calling procedure
  ** Back slashes are recoded into forward slashes for Unix portability
  */
  ASSIGN xDirName = RIGHT-TRIM(REPLACE(DirList:SCREEN-VALUE, "~\":U, "/":U),"/").
  ...
  /* Strip the extention off image files. */
  IF pMode EQ "IMAGE" THEN DO:
    RUN adecomm/_osfext.p (INPUT pFileName, OUTPUT fext).
    /* Strip out know extensions (and don't bother about no extension). */
    IF NOT CAN-DO(",bmp,ico,xbm,xpm", fext) THEN
      pFileName = SUBSTRING(pFileName,1,
                    LENGTH(pFileName,"CHARACTER") - LENGTH(fext,"CHARACTER"),
                           "CHARACTER").
  END.
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ...
    */
    ...
    IF PreviewType THEN DO:
      /*
      ** load as an image: PreviewType = True (do this by removing the file 
      ** extension and loading it.
      */
      RUN adecomm/_osfext.p (INPUT FullFileName, OUTPUT fext).
      ImageFileName = SUBSTRING(FullFileName, 1,
         LENGTH(FullFileName,"CHARACTER") - LENGTH(fext,"CHARACTER"),"CHARACTER").
      ASSIGN Result = ImageDisplay:LOAD-IMAGE
        (ImageFileName,0,0,ImageDisplay:WIDTH-P,ImageDisplay:HEIGHT-P) NO-ERROR.
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ...
    */
    ASSIGN
      ...
      pOK                             = FALSE
      FileType:delimiter              = "|"
      FileType:LIST-ITEMS             = TRIM(pFilter)
